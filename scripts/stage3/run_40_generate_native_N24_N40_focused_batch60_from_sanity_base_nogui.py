...
RUN_ID = "run_40_native_N24_N40_focused_batch60_cae_inp_generation"
...
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run40_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run40_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run40_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run40_native_N24_N40_focused_batch60_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run40_native_N24_N40_focused_batch60_cae_generation_manifest.json"
...
def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run40 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "run38_native_N24_N40_focused_batch60":
                raise run40GenerationError("non-run38_native_N24_N40_focused_batch60 row selected: {}".format(row.get("handoff_strategy_name")))
    ...
    if len(rows) != 60:
        raise run40GenerationError("generation plan row count {} != 60".format(len(rows)))
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run40_case_metadata.json"
    metadata_keys = [
        "run_id", "batch_name", "selected_batch", "n", "handoff_strategy_name",
        "original_run38_candidate_id", "original_run38_strategy_name",
        ...
        "N24_N40_focused", "overnight_batch60", "native_validation_N",
    ]
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "Run40 native N24/N40 focused batch60 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN40_NATIVE_N24_N40_FOCUSED_BATCH60_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN40_NATIVE_N24_N40_FOCUSED_BATCH60_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN40_NATIVE_N24_N40_FOCUSED_BATCH60_GENERATION")
        print(str(exc))
