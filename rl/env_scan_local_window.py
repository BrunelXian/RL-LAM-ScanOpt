"""Gymnasium environment for truly local window scan planning."""
...
class ScanPlanningLocalWindowEnv(ScanPlanningEnv):
    """Local-window variant using compact 3-cell scan primitives.

    One action is defined by:
    - center cell (row, col), restricted to target cells
    - orientation: horizontal or vertical

    The executed cells are the target-masked cells covered by the centered
    3-cell line within the current orientation.
    """

    ORIENTATION_NAMES = ("horizontal", "vertical")
    ...
    def _validate_window(self) -> None:
        """Validate local-window parameters."""
        if self.window_size <= 0:
            raise ValueError("window_size must be positive")
        if self.window_size % 2 == 0:
            raise ValueError("window_size must be odd to preserve a centered local action")

    def _window_offsets(self, orientation_index: int) -> list[tuple[int, int]]:
        """Return centered offsets for one orientation."""
        radius = self.window_size // 2
        if orientation_index == 0:
            return [(0, delta) for delta in range(-radius, radius + 1)]
        return [(delta, 0) for delta in range(-radius, radius + 1)]
    ...
    def _build_action_cells(
        ...
        """Return target-masked cells covered by one local window action."""
        row, col = center
        cells: list[tuple[int, int]] = []
        for row_offset, col_offset in self._window_offsets(orientation_index):
    ...
    def _rebuild_local_actions(self) -> None:
        """Recompute the local action catalog."""
        ...
        for row, col in zip(target_rows, target_cols):
            ...
            for orientation_index, orientation_name in enumerate(self.ORIENTATION_NAMES):
                cells = self._build_action_cells(center, orientation_index)
                ...
                self.action_lookup[(int(row), int(col), int(orientation_index))] = action_index
    ...
    def _remaining_cells_for_action(self, action: int) -> list[tuple[int, int]]:
        """Return currently executable cells for one local action."""
        return [
            cell
    ...
    def reset(
        ...
        *,
        ...
        """Reset the environment and rebuild the local action catalog."""
        observation, info = super().reset(seed=seed, options=options)
        self._rebuild_local_actions()
        info.update(
            {
                "action_mode": "local_window",
    ...
    def decode_action(self, action: int) -> tuple[int, int, int]:
        """Return ``(center_row, center_col, orientation_index)`` for one action."""
        spec = self.action_specs[int(action)]
        return int(spec["center_row"]), int(spec["center_col"]), int(spec["orientation_index"])

    def is_valid_action(self, action: int) -> bool:
        """Return True only when the local window can still affect unscanned target cells."""
    ...
    def action_masks(self) -> np.ndarray:
        """Return a flat mask over the local action catalog."""
    ...
    def _base_info(
        ...
        *,
        ...
        """Return Stage A logging fields plus local-action metadata."""
    ...
    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        """Apply one local window action and return the Stage A composite reward."""
        self.steps_taken += 1
