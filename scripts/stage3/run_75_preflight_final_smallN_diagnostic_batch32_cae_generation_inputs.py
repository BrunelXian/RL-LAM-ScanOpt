"""Preflight Stage 3 run75 final_smallN_diagnostic_batch32 CAE/INP generation inputs."""
...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_74_run73_final_smallN_diagnostic_batch32_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run74_final_smallN_diagnostic_batch32_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run75_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run75_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run75_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run75_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN75_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN75_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "final_smallN_diagnostic_batch32":
            errors.append("row {} is not final_smallN_diagnostic_batch32: {}".format(idx, row.get("batch_option")))
        ...
        if "final small-n diagnostic batch24" in lowered or "stop-and-freeze" in lowered:
            errors.append("row {} references forbidden Run73 Option B/C text".format(idx))
    ...
    verdict = "PASS_RUN75_FINAL_SMALLN_DIAGNOSTIC_BATCH32_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN75_FINAL_SMALLN_DIAGNOSTIC_BATCH32_INPUT_INVALID"
    summary = {
        "verdict": verdict,
