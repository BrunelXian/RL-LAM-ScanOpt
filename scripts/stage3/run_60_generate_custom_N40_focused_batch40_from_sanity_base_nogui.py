"""Generate Stage 3 run40 native_N24_N40_focused_batch60 CAE/INP artifacts from sanity bases.

Run with:
abaqus cae noGUI="E:\Projects\RL-LAM-ScanOpt\scripts\stage3\run_40_generate_native_N24_N40_focused_batch60_from_sanity_base_nogui.py"
...
RUN_ID = "run_60_custom_N40_focused_calibrated_penalty_repair_batch40_cae_inp_generation"
BATCH_NAME = "stage3_run59_N40_focused_calibrated_penalty_repair_batch40_v01"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_60_custom_N40_focused_calibrated_penalty_repair_batch40_cae_inp_generation"
PLAN_CSV = OUTPUT_DIR / "run60_generation_plan.csv"
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run60_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run60_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run60_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run60_custom_N40_focused_batch40_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run60_custom_N40_focused_batch40_cae_generation_manifest.json"
...
class run40GenerationError(RuntimeError):
    pass


def validate_scan_order(n: int, order: object) -> None:
    if not isinstance(order, list):
        raise run40GenerationError("scan_order is not a list")
    if len(order) != n:
        raise run40GenerationError("scan_order length {} != N {}".format(len(order), n))
    if not all(isinstance(v, int) for v in order):
        raise run40GenerationError("scan_order contains non-integer entries")
    if sorted(order) != list(range(n)):
        raise run40GenerationError("scan_order is not a permutation of 0..{}".format(n - 1))


def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run60 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "custom_run58_N40_focused_calibrated_penalty_repair_batch40":
                raise run40GenerationError("non-custom_run58_N40_focused_calibrated_penalty_repair_batch40 row selected: {}".format(row.get("handoff_strategy_name")))
            if n not in EXPECTED_COUNTS:
                raise run40GenerationError("forbidden or unexpected N{} row selected".format(n))
            rows.append(dict(row, n=n, scan_order=scan_order))
    if len(rows) != 40:
        raise run40GenerationError("generation plan row count {} != 40".format(len(rows)))
    counts = {n: len([row for row in rows if int(row["n"]) == n]) for n in EXPECTED_COUNTS}
    if counts != EXPECTED_COUNTS:
        raise run40GenerationError("per-N generation counts {} != {}".format(counts, EXPECTED_COUNTS))
    if any(int(row["n"]) in (12, 16, 32) for row in rows):
        raise run40GenerationError("forbidden N12/N16/N32 row selected")
    return rows


def validate_base_audit(rows: list[dict[str, object]]) -> None:
    if not BASE_AUDIT_SUMMARY.exists():
        raise run40GenerationError("missing base mesh audit summary: {}".format(BASE_AUDIT_SUMMARY))
    ...
    for n in sorted({int(row["n"]) for row in rows}):
        ...
        if float(row.get("total_node_count", 0)) <= 0 or float(row.get("total_element_count", 0)) <= 0:
            failed.append(n)
    if failed:
        raise run40GenerationError("base mesh audit not passing for N values: {}".format(failed))
...
def validate_paths(rows: list[dict[str, object]]) -> None:
    for row in rows:
        ...
        if any(token in path for token in BAD_SCHEMA_TOKENS for path in paths):
            raise run40GenerationError("bad path schema token in {}".format(row["handoff_strategy_name"]))
        if Path(str(row["case_dir"])).parent != CASE_ROOT:
            raise run40GenerationError("case_dir not under run40 case root: {}".format(row["case_dir"]))
        for key in ("cae_path", "inp_path", "jnl_path", "scan_order_json", "generation_log_path"):
            if Path(str(row[key])).parent != Path(str(row["case_dir"])):
                raise run40GenerationError("{} is not inside case_dir".format(key))
        existing = [path for path in (Path(str(row["cae_path"])), Path(str(row["inp_path"])), Path(str(row["jnl_path"]))) if path.exists()]
        if existing:
            raise run40GenerationError("generated CAE/INP/JNL already exists for {}: {}".format(row["handoff_strategy_name"], existing))
...
def validate_no_solver_outputs(case_dir: Path) -> None:
    ...
    if found:
        raise run40GenerationError("solver output files unexpectedly exist: {}".format(found))
...
def validate_heat_sets_exist_in_model(model, n: int) -> dict[int, object]:  # noqa: ANN001
    ...
    for track in range(n):
        ...
        if track not in found:
            raise run40GenerationError("missing heat set {}".format(name))
    ...
    if len(set(all_heat_names)) != n:
        raise run40GenerationError("expected exactly {} heat sets, found {}".format(n, sorted(set(all_heat_names))))
    return found
...
def infer_templates(model) -> dict[str, object]:  # noqa: ANN001
    ...
    if missing:
        raise run40GenerationError("missing template objects: {}".format(missing))
    load = model.loads[MAPPING["template_load"]]
    if load.__class__.__name__ != MAPPING["load_type"]:
        raise run40GenerationError("template load type {} != {}".format(load.__class__.__name__, MAPPING["load_type"]))
...
def create_step_from_template(model, template_step, name: str, previous: str, time_period: float, initial_inc=None, max_inc=None) -> None:  # noqa: ANN001,E501
    ...
    if step_type == "CoupledTempDisplacementStep":
        ...
        raise run40GenerationError("unsupported step template type {}".format(step_type))
...
def set_existing_step_time(model, step_name: str, duration: float) -> None:  # noqa: ANN001
    try:
        ...
        raise run40GenerationError("failed setting {} duration: {}".format(step_name, exc))
...
def set_existing_step_time_and_initial(model, step_name: str, duration: float, initial_inc=None) -> None:  # noqa: ANN001
    ...
    try:
        ...
        raise run40GenerationError("failed setting {} step values: {}".format(step_name, exc))
    ...
    for seq, track in enumerate(scan_order):
        ...
        if seq == 0:
            if scan not in model.steps or cool not in model.steps:
                raise run40GenerationError("template scan/cool step missing for seq0")
        ...
        if cool_initial_inc is not None:
            ...
            if observed != cool_initial_inc:
                raise run40GenerationError("{} initialInc verification failed: {}".format(cool, observed))
...
def create_heat_load(model, seq: int, track: int, heat_regions: dict[int, object]):  # noqa: ANN001
    ...
    if seq == 0:
        ...
        try:
            ...
            if track != 0:
                raise run40GenerationError("failed to retarget template load to track {}: {}".format(track, exc))
...
def append_final_cooling(model, records: list[dict[str, object]]) -> dict[str, object]:  # noqa: ANN001
    ...
    if final in model.steps:
        raise run40GenerationError("final cooling step already exists")
    ...
    if checks["final_cooling_timePeriod"] != 1200.0:
        raise run40GenerationError("final cooling timePeriod verification failed")
    if checks["final_cooling_initialInc"] != 0.01:
        raise run40GenerationError("final cooling initialInc verification failed")
    if checks["final_cooling_maxInc"] != 60.0:
        raise run40GenerationError("final cooling maxInc verification failed")
    return checks
...
def validate_model_before_write(model, n: int, scan_order: list[int], records: list[dict[str, object]], deactivations: list[dict[str, str]], final_record: dict[str, object]) -> None:  # noqa: ANN001,E501
    ...
    if len(records) != n or len(deactivations) != n:
        raise run40GenerationError("sequence/deactivation count != N")
    for seq in range(n):
        if MAPPING["scan_step_pattern"].format(seq=seq) not in model.steps:
            raise run40GenerationError("missing scan step {}".format(seq))
        if MAPPING["cool_step_pattern"].format(seq=seq) not in model.steps:
            raise run40GenerationError("missing cool step {}".format(seq))
        if MAPPING["load_pattern"].format(seq=seq) not in model.loads:
            raise run40GenerationError("missing heat load {}".format(seq))
    if final_record["final_cooling_previous"] != records[-1]["cool_step"]:
        raise run40GenerationError("final cooling is not after last cool step")
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run60_case_metadata.json"
    metadata_keys = [
        "run_id", "batch_name", "selected_batch", "n", "handoff_strategy_name",
        "original_run48_candidate_id", "original_run48_strategy_name",
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "run60 constrained N24/N40 batch32 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN60_CUSTOM_N40_FOCUSED_BATCH40_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN60_CUSTOM_N40_FOCUSED_BATCH40_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN60_CUSTOM_N40_FOCUSED_BATCH40_GENERATION")
        print(str(exc))
