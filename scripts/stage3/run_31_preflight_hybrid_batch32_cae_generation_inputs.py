"""Preflight Stage 3 run31 hybrid_batch32 CAE/INP generation inputs."""
...
RUN30_DIR = PROJECT_ROOT / "outputs" / "stage3_run_30_run29_hybrid_policy_batch32_handoff_package"
INPUT_CSV = RUN30_DIR / "stage3_run30_hybrid_policy_batch32_candidate_orders.csv"
SCAN_ORDER_DIR = RUN30_DIR / "scan_orders"
...
SUMMARY_PATH = OUTPUT_DIR / "run31_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run31_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run31_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run31_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN31_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN31_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "hybrid_policy_batch32":
            errors.append("row {} is not hybrid_policy_batch32: {}".format(idx, row.get("batch_option")))
        ...
        if "batch64" in lowered or "batch48" in lowered or "shortlist64" in lowered:
            errors.append("row {} references forbidden hybrid batch64/focused batch48/shortlist64 text".format(idx))
        ...
        normalized.append(
            {
                ...
                "original_run23_strategy_name": row.get("original_run23_strategy_name", ""),
                ...
                "predicted_gap_vs_combined108_best": row.get("predicted_gap_vs_combined108_best", ""),
    ...
    verdict = "PASS_RUN31_HYBRID_BATCH32_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN31_HYBRID_BATCH32_INPUT_INVALID"
    summary = {
        "verdict": verdict,
