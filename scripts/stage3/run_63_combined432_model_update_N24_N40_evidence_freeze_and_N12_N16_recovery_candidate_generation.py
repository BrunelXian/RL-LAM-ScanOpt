...
RUN_ID = "run_63_combined432_model_update_N24_N40_evidence_freeze_and_N12_N16_recovery_candidate_generation"
RUN_NAME = "combined432 model update N24/N40 evidence freeze and N12/N16 recovery candidate generation"
SCRIPT_PATH = ROOT / "scripts" / "stage3" / "run_63_combined432_model_update_N24_N40_evidence_freeze_and_N12_N16_recovery_candidate_generation.py"

RUN62_DIR = ROOT / "outputs" / "stage3_run_62_custom_N40_focused_batch40_teacher_metrics_ingestion_and_combined432_ranking"
COMBINED432_READY = RUN62_DIR / "combined432_RL_ready_dataset.csv"
COMBINED432_PLUS_N32_READY = RUN62_DIR / "combined432_plus_N32_RL_ready_dataset.csv"
RUN61_ENRICHED = RUN62_DIR / "run61_custom_N40_focused_batch40_teacher_dataset_enriched.csv"
RUN61_COMPARISON = RUN62_DIR / "run61_vs_combined392_best_comparison.csv"
RUN61_EFFECTIVENESS = RUN62_DIR / "run61_custom_N40_focused_batch40_effectiveness_audit.csv"
RUN61_PRED_AUDIT = RUN62_DIR / "run61_prediction_audit_for_run58_custom_batch40.csv"
RUN61_PRED_SUMMARY = RUN62_DIR / "run61_prediction_audit_for_run58_custom_batch40_summary.json"
RUN62_MATURITY = RUN62_DIR / "n24_n40_updated_maturity_and_claim_boundary_summary.json"
RUN62_MATURITY_MD = RUN62_DIR / "n24_n40_updated_maturity_and_claim_boundary_audit.md"
...
RUN62_MANIFEST = ROOT / "artifacts" / "manifests" / "stage3_run_62_manifest.json"
...
OUTPUT_DIR = ROOT / "outputs" / "stage3_run_63_combined432_model_update_N24_N40_evidence_freeze_and_N12_N16_recovery_candidate_generation"
REPORT_DIR = ROOT / "docs" / "stage3" / "runs" / "run_63_combined432_model_update_N24_N40_evidence_freeze_and_N12_N16_recovery_candidate_generation"
REPORT_PATH = REPORT_DIR / "RUN_63_COMBINED432_MODEL_UPDATE_N24_N40_EVIDENCE_FREEZE_AND_N12_N16_RECOVERY_CANDIDATE_GENERATION_REPORT.md"
MANIFEST_PATH = ROOT / "artifacts" / "manifests" / "stage3_run_63_manifest.json"
CLAIM_BOUNDARY_MD = OUTPUT_DIR / "run63_claim_boundary.md"
CLAIM_BOUNDARY_JSON = OUTPUT_DIR / "run63_claim_boundary.json"

EXPECTED_NATIVE = {12: 36, 16: 36, 24: 176, 40: 184}
EXPECTED_PLUS = {12: 36, 16: 36, 24: 176, 32: 332, 40: 184}
POOL_TARGETS = {12: 3000, 16: 3000, 24: 1500, 40: 2500}
OPTION_A_COUNTS = {12: 12, 16: 12, 24: 8, 40: 16}
OPTION_B_COUNTS = {12: 12, 16: 12, 24: 4, 40: 4}
OPTION_C_COUNTS = {24: 8, 40: 24}
GLOBAL_SEED = 63042
...
def add_features_and_targets(df: pd.DataFrame, native: bool) -> pd.DataFrame:
    ...
    if native:
        score_cols = [
            "target_u2_score_combined432_rank",
            "target_peeq_score_combined432_rank",
            "target_surfaceT_score_combined432_rank",
            "target_mises_score_combined432_rank",
        ]
        out["target_reward_combined432_u2_primary"] = as_float(out["target_reward_combined432_u2_primary"])
        prefix = "combined432"
    else:
        score_cols = [
            "target_u2_score_combined432_plus_N32_rank",
            "target_peeq_score_combined432_plus_N32_rank",
            "target_surfaceT_score_combined432_plus_N32_rank",
            "target_mises_score_combined432_plus_N32_rank",
        ]
        out["target_reward_combined432_plus_N32_mapped_u2_primary"] = as_float(out["target_reward_combined432_plus_N32_mapped_u2_primary"])
        prefix = "combined432_plus_N32"
    ...
    out[f"target_reward_{prefix}_N12_recovery"] = (
        0.52 * out[score_cols[0]] + 0.22 * out[score_cols[1]] + 0.16 * out[score_cols[2]] + 0.10 * out[score_cols[3]]
    )
    out[f"target_reward_{prefix}_N16_recovery"] = (
        0.50 * out[score_cols[0]] + 0.24 * out[score_cols[1]] + 0.16 * out[score_cols[2]] + 0.10 * out[score_cols[3]]
    )
...
def validate_inputs(native: pd.DataFrame, plus: pd.DataFrame, pred_summary: dict[str, Any], maturity: dict[str, Any]) -> dict[str, Any]:
    errors = []
    if len(native) != 432 or n_counts(native) != EXPECTED_NATIVE:
        errors.append(f"native combined432 count mismatch: rows={len(native)} counts={n_counts(native)}")
    if 32 in set(native["n"].astype(int)):
        errors.append("native combined432 contains N32")
    if len(plus) != 764 or n_counts(plus) != EXPECTED_PLUS:
        errors.append(f"combined432_plus_N32 count mismatch: rows={len(plus)} counts={n_counts(plus)}")
    for col in [
        ...
        "target_reward_combined432_N12_recovery",
        "target_reward_combined432_N16_recovery",
        "target_reward_combined432_variable_N_recovery",
        "target_reward_combined432_N40_followup",
    ]:
    ...
    if maturity:
        expected_maturity = {
            "n24_teacher_rows": 176,
            "n40_teacher_rows": 184,
            "n12_teacher_rows": 36,
            "n16_teacher_rows": 36,
        }
        ...
        errors.append("Run62 maturity audit summary missing")
    if pred_summary:
    ...
    return {
        "timestamp": now_iso(),
        "verdict": "PASS_RUN63_COMBINED432_AND_PLUS_N32_INPUTS_READY" if not errors else "FAIL_RUN63_INPUT_VALIDATION",
        "errors": errors,
        "native_combined432_rows": int(len(native)),
        "native_combined432_counts": n_counts(native),
        "combined432_plus_N32_rows": int(len(plus)),
        "combined432_plus_N32_counts": n_counts(plus),
        "maturity_context": maturity,
        "run61_prediction_audit_context": {"u2_primary_or_reward_spearman": reward_s, "mean_top5_overlap": top5, "top1_hits": pred_summary.get("top1_hits") if pred_summary else None},
    }
...
def validation_protocols(df: pd.DataFrame, regime: str) -> list[tuple[str, np.ndarray, np.ndarray]]:
    ...
    if "dataset_source" in df.columns and (df["dataset_source"].astype(str) == "run61_custom_N40_focused_batch40").any():
        test = df.index[df["dataset_source"].astype(str) == "run61_custom_N40_focused_batch40"].to_numpy()
        train = np.setdiff1d(rows, test)
        protocols.append(("train_pre_Run61_test_Run61", train, test))
...
def run_surrogate_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], Any, list[str]]:
    ...
    targets = {
        ...
        "plus_N32_unweighted": [
            "target_reward_combined432_plus_N32_mapped_u2_primary",
            "target_reward_combined432_plus_N32_strict_u2_surfaceT",
            "target_reward_combined432_plus_N32_constrained_u2_reward_balanced",
            "target_reward_combined432_plus_N32_N12_recovery",
            "target_reward_combined432_plus_N32_N16_recovery",
            "target_reward_combined432_plus_N32_variable_N_recovery",
            "target_reward_combined432_plus_N32_N40_followup",
        ],
        "plus_N32_balanced": [
            "target_reward_combined432_plus_N32_mapped_u2_primary",
            "target_reward_combined432_plus_N32_strict_u2_surfaceT",
    ...
    frames = {"native_combined432": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}
    rows = []
    ...
    native_best = grouped[grouped["regime"] == "native_combined432"].head(1).iloc[0].to_dict()
    ...
    summary = {
        ...
        "best_N12_recovery": best_for("target_reward_combined432_N12_recovery"),
        "best_N16_recovery": best_for("target_reward_combined432_N16_recovery"),
        "best_variable_N_recovery": best_for("target_reward_combined432_variable_N_recovery"),
        "best_N40_followup": best_for("target_reward_combined432_N40_followup"),
        "best_N40_u2_reward_retention": best_for("target_reward_combined432_N40_u2_reward_retention"),
        "best_N40_penalty_repair": best_for("target_reward_combined432_N40_penalty_repair"),
        "best_N24_maintenance": best_for("target_reward_combined432_N24_maintenance"),
        "best_penalty_repair": best_for("target_reward_combined432_penalty_repair"),
        "n32_augmented_better_than_native": bool(plus_best["macro_spearman"] > native_best["macro_spearman"]),
        "run58_reference": {"best_N40_u2_reward_retention_spearman": 0.6940, "best_N40_penalty_repair_spearman": 0.5786, "best_N24_maintenance_spearman": 0.7212},
    }
...
def run_gnn_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    rows = []
    for regime, df in {"native_combined432": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}.items():
        target = "target_reward_combined432_u2_primary" if regime == "native_combined432" else "target_reward_combined432_plus_N32_mapped_u2_primary"
        sw = sample_weights(df) if regime.endswith("balanced") else None
        for protocol, train, test in validation_protocols(df, regime):
            try:
...
def generate_candidates(native: pd.DataFrame, model: Any, features: list[str]) -> pd.DataFrame:
    ...
    for n in [12, 16, 24, 40]:
        ...
        best_by[(n, "reward")] = parse_order(g.sort_values("target_reward_combined432_u2_primary", ascending=False).iloc[0]["order_json"])
        best_by[(n, "constrained")] = parse_order(g.sort_values("target_reward_combined432_constrained_u2_reward_balanced", ascending=False).iloc[0]["order_json"])
        best_by[(n, "strict")] = parse_order(g.sort_values("target_reward_combined432_strict_penalty_guard", ascending=False).iloc[0]["order_json"])
        best_by[(n, "two_stage")] = parse_order(g.sort_values("target_reward_combined432_two_stage_guarded", ascending=False).iloc[0]["order_json"])
    rows, seen = [], set(existing)
    for n, target_count in POOL_TARGETS.items():
        if n in (12, 16):
            source_plan = [
                f"N{n}_recovery_anchor_top", f"N{n}_recovery_anchor_local_search",
                f"N{n}_u2_safe_exploitation", f"N{n}_penalty_aware_repair",
                f"N{n}_uncertainty_calibration", f"N{n}_diversity_coverage",
                f"N{n}_sentinel_control",
            ]
        elif n == 40:
            source_plan = [
                "N40_followup_u2_reward_retention", "N40_followup_penalty_repair",
                "N40_followup_two_stage_repair", "N40_followup_median_guard",
                "N40_followup_uncertainty_calibration", "N40_followup_diversity_coverage",
                "N40_followup_sentinel_control",
            ]
        elif n == 24:
            source_plan = [
                "N24_frozen_reference_maintenance", "N24_frozen_penalty_diagnostic",
                "N24_frozen_uncertainty_calibration", "N24_frozen_diversity_coverage", "N24_frozen_sentinel_control",
            ]
        attempts = 0
        while sum(1 for r in rows if r["n"] == n) < target_count and attempts < target_count * 20:
            ...
            n12_recovery = 0.52 * u2_pred + 0.22 * peeq_guard + 0.16 * surf_guard + 0.10 * mises_guard
            n16_recovery = 0.50 * u2_pred + 0.24 * peeq_guard + 0.16 * surf_guard + 0.10 * mises_guard
            ...
            cid = f"R63_N{n}_C{sum(1 for r in rows if r['n']==n)+1:05d}"
            rows.append({
                "candidate_id": cid,
...
def select_batch(pool: pd.DataFrame, counts: dict[int, int], label: str) -> pd.DataFrame:
    ...
    primary_plan = {
        24: [
            ("N24_frozen_reference_maintenance", 3),
            ("N24_frozen_penalty_diagnostic", 2),
            ("N24_frozen_uncertainty_calibration", 1),
            ("N24_frozen_diversity_coverage", 1),
            ("N24_frozen_sentinel_control", 1),
        ],
        40: [
            ("N40_followup_u2_reward_retention", 5),
            ("N40_followup_penalty_repair", 5),
            ("N40_followup_two_stage_repair", 4),
            ("N40_followup_median_guard", 3),
            ("N40_followup_uncertainty_calibration", 2),
            ("N40_followup_diversity_coverage", 2),
            ("N40_followup_sentinel_control", 1),
        ],
        12: [
            ("N12_recovery_anchor_top", 4),
            ("N12_recovery_anchor_local_search", 3),
            ("N12_u2_safe_exploitation", 2),
            ("N12_penalty_aware_repair", 1),
            ("N12_uncertainty_calibration", 1),
            ("N12_diversity_coverage", 1),
        ],
        16: [
            ("N16_recovery_anchor_top", 4),
            ("N16_recovery_anchor_local_search", 3),
            ("N16_u2_safe_exploitation", 2),
            ("N16_penalty_aware_repair", 1),
            ("N16_uncertainty_calibration", 1),
            ("N16_diversity_coverage", 1),
        ],
    }
    fallback_order = [
        "N12_recovery_anchor_top", "N12_recovery_anchor_local_search", "N12_u2_safe_exploitation", "N12_penalty_aware_repair",
        "N16_recovery_anchor_top", "N16_recovery_anchor_local_search", "N16_u2_safe_exploitation", "N16_penalty_aware_repair",
        "N24_frozen_reference_maintenance", "N24_frozen_penalty_diagnostic",
        "N40_followup_u2_reward_retention", "N40_followup_penalty_repair", "N40_followup_two_stage_repair",
    ]
    for n, count in counts.items():
        ...
        for i, row in enumerate(take[:count], 1):
            ...
            row["handoff_strategy_name"] = f"S3R63REC_N{n}_B{i:02d}_{short}"
...
def compare_batches(options: dict[str, pd.DataFrame], native: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    previous = {"combined432_teacher_orders": set(native["order_hash"].astype(str))}
    for name, path in {"run61": RUN61_HANDOFF, "run56": RUN56_HANDOFF, "run51": RUN51_HANDOFF, "run46": RUN46_HANDOFF, "run41": RUN41_HANDOFF, "run36": RUN36_HANDOFF, "run27": RUN27_HANDOFF, "superseded_run31": RUN31_OLD}.items():
        if path.exists():
    ...
    headline = "Run63 batch options were checked for exact order overlap, small-N recovery score, optional N40 follow-up score, novelty, and source diversity against combined432, Run61, Run56, Run51, Run46, Run41, Run36, Run27, and superseded Run31."
...
def write_evidence_freeze(native: pd.DataFrame, validation: dict[str, Any], pred_summary: dict[str, Any], maturity: dict[str, Any]) -> dict[str, Any]:
    timeline = [
        ...
        {"stage": "Run61_custom_N40_focused_batch40", "native_rows": 432, "N12": 36, "N16": 36, "N24": 176, "N40": 184, "interpretation": "N24 PEEQ and N40 U2/reward-family records advanced; N24/N40 evidence is dense enough to freeze while N12/N16 remain under-sampled"},
    ]
    freeze = pd.DataFrame(timeline)
    freeze_path = OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.csv"
    freeze.to_csv(freeze_path, index=False)

    summary = {
        "verdict": "RUN63_FINAL_N24_N40_EVIDENCE_FREEZE_READY_FULL_VARIABLE_N_LIMITED_BY_N12_N16",
        "headline": "N24/N40 active-learning evidence is mature enough to freeze at N24=176 and N40=184 native teacher rows; full variable-N RL remains limited by N12/N16 at 36 rows each.",
        "native_combined432_counts": n_counts(native),
        "run62_maturity_context": maturity,
        "run61_prediction_audit_context": {
        ...
        "freeze_decision": {
            ...
            "continue_N40_followup_only_as_secondary_option": True,
            "continue_full_variable_N_claim_boundary": "limited_by_N12_N16_anchor_rows",
        },
        "timeline": timeline,
        "validation_verdict": validation["verdict"],
    }
    write_json(OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.json", summary)
    (OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.md").write_text(
        "# N24/N40 Final Active-Learning RL Evidence Freeze\n\n"
        ...
        "The next candidate-generation priority is N12/N16 recovery anchors, with only a secondary optional N40 follow-up path.\n\n"
        "## Timeline\n"
...
def write_claim_boundary() -> None:
    safe = [
        "Run63 updates offline models using native combined432 and combined432_plus_N32.",
        "Run63 performs a final N24/N40 evidence-freeze audit while preserving the full variable-N limitation from N12/N16 row counts.",
        "Run63 pivots primary candidate generation toward N12/N16 recovery anchors.",
        "Run63 keeps N40 follow-up as a secondary optional batch only.",
        "Run63 generates candidate-order batches for future teacher validation.",
        "Run63 does not include teacher validation for new candidates.",
    ]
    unsafe = [
    ...
    CLAIM_BOUNDARY_MD.write_text("# Run63 Claim Boundary\n\n## Safe claims\n" + "\n".join(f"- {x}" for x in safe) + "\n\n## Unsafe claims\n" + "\n".join(f"- {x}" for x in unsafe) + "\n", encoding="utf-8")
    write_json(CLAIM_BOUNDARY_JSON, {"verdict": "RUN63_MODEL_UPDATE_EVIDENCE_FREEZE_AND_CANDIDATE_GENERATION_ONLY_NO_TEACHER_VALIDATION", "safe_claims": safe, "unsafe_claims": unsafe})


def write_report(summary: dict[str, Any]) -> None:
    REPORT_PATH.write_text(f"""# Stage 3 Run 63 - Combined432 Model Update, N24/N40 Evidence Freeze, and N12/N16 Recovery Candidate Generation
...
- Run61 prediction audit: `{RUN61_PRED_SUMMARY}`
- Run62 maturity audit: `{RUN62_MATURITY}`
...
Best variable-N recovery: `{summary['surrogate'].get('best_variable_N_recovery')}`.
...
Candidate pool counts: `{summary['candidate_pool_counts']}`. N12/N16 meet the >=3000 candidate minimums, with N24 frozen references and optional N40 follow-up candidates included for comparison.

## 10. Option A - Variable-N Recovery Anchor Batch48
Path: `{OUTPUT_DIR / 'run63_variable_N_recovery_anchor_batch48_candidate_orders.csv'}`. Counts: `{summary['option_counts']['option_A_variable_N_recovery_anchor_batch48']}`.

## 11. Option B - Small-N Recovery Batch32
Path: `{OUTPUT_DIR / 'run63_smallN_recovery_batch32_candidate_orders.csv'}`. Counts: `{summary['option_counts']['option_B_smallN_recovery_batch32']}`.

## 12. Option C - Optional N40 Follow-Up Batch32
Path: `{OUTPUT_DIR / 'run63_optional_N40_followup_batch32_candidate_orders.csv'}`. Counts: `{summary['option_counts']['option_C_optional_N40_followup_batch32']}`.
...
## 15. Output Files
- Candidate pool: `{OUTPUT_DIR / 'run63_candidate_pool_scored.csv'}`
- Surrogate summary: `{OUTPUT_DIR / 'run63_surrogate_validation_summary.json'}`
- GNN summary: `{OUTPUT_DIR / 'run63_gnn_reward_validation_summary.json'}`
- Pointer summary: `{OUTPUT_DIR / 'run63_graph_pointer_policy_validation_summary.json'}`
- Evidence freeze: `{OUTPUT_DIR / 'n24_n40_final_active_learning_rl_evidence_freeze.md'}`
- Batch options comparison: `{OUTPUT_DIR / 'run63_batch_options_comparison_summary.json'}`
...
def main() -> None:
    ensure_dirs()
    native = add_features_and_targets(read_csv(COMBINED432_READY), native=True)
    plus = add_features_and_targets(read_csv(COMBINED432_PLUS_N32_READY), native=False)
    pred_summary = json.loads(RUN61_PRED_SUMMARY.read_text(encoding="utf-8")) if RUN61_PRED_SUMMARY.exists() else {}
    maturity = json.loads(RUN62_MATURITY.read_text(encoding="utf-8")) if RUN62_MATURITY.exists() else {}
    validation = validate_inputs(native, plus, pred_summary, maturity)
    write_json(OUTPUT_DIR / "run63_input_validation_summary.json", validation)
    if not validation["verdict"].startswith("PASS"):
        raise SystemExit(validation["errors"])

    native.to_csv(OUTPUT_DIR / "combined432_scan_order_features.csv", index=False)
    plus.to_csv(OUTPUT_DIR / "combined432_plus_N32_scan_order_features.csv", index=False)
    target_rows = [
        {"target": "target_reward_combined432_u2_primary", "definition": "0.65 U2 + 0.20 PEEQ + 0.10 SurfaceT + 0.05 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_constrained_u2_reward_balanced", "definition": "0.50 U2 + 0.25 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_strict_penalty_guard", "definition": "0.40 U2 + 0.30 PEEQ + 0.20 SurfaceT + 0.10 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_penalty_repair", "definition": "0.30 U2 + 0.30 PEEQ + 0.25 SurfaceT + 0.15 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_N12_recovery", "definition": "N12 recovery target: 0.52 U2 + 0.22 PEEQ + 0.16 SurfaceT + 0.10 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_N16_recovery", "definition": "N16 recovery target: 0.50 U2 + 0.24 PEEQ + 0.16 SurfaceT + 0.10 Mises", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_variable_N_recovery", "definition": "Small-N recovery target for N12/N16 and conservative constrained/penalty mix for frozen N24/N40 references", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_N40_followup", "definition": "Secondary N40 follow-up target mixing N40 U2/reward retention, N40 penalty repair, and two-stage repair", "dataset": "native_combined432"},
        {"target": "target_reward_combined432_plus_N32_mapped_u2_primary", "definition": "mapped U2-primary target with N32 semantic warnings preserved", "dataset": "combined432_plus_N32"},
        {"target": "target_reward_combined432_plus_N32_strict_u2_surfaceT", "definition": "strict U2 + SurfaceT target for safer N32 metric semantics", "dataset": "combined432_plus_N32"},
    ]
    pd.DataFrame(target_rows).to_csv(OUTPUT_DIR / "run63_target_reward_definition_audit.csv", index=False)
    write_json(OUTPUT_DIR / "run63_target_reward_definition_audit.json", {"headline": "Run63 defines N12/N16 recovery targets as the primary candidate-generation objective, freezes N24/N40 evidence for reporting, and keeps N40 follow-up as a secondary option with N32 semantic warnings preserved.", "targets": target_rows})

    detailed, best_cfg, surrogate_summary, best_model, best_features = run_surrogate_validation(native, plus)
    detailed.to_csv(OUTPUT_DIR / "run63_surrogate_validation_results_detailed.csv", index=False)
    best_cfg.to_csv(OUTPUT_DIR / "run63_best_surrogate_configurations.csv", index=False)
    write_json(OUTPUT_DIR / "run63_surrogate_validation_summary.json", surrogate_summary)

    gnn_results, gnn_summary = run_gnn_validation(native, plus)
    gnn_results.to_csv(OUTPUT_DIR / "run63_gnn_reward_validation_results.csv", index=False)
    write_json(OUTPUT_DIR / "run63_gnn_reward_validation_summary.json", gnn_summary)

    pointer_log, pointer_summary = pointer_policy(native, plus)
    pointer_log.to_csv(OUTPUT_DIR / "run63_graph_pointer_policy_training_log.csv", index=False)
    write_json(OUTPUT_DIR / "run63_graph_pointer_policy_validation_summary.json", pointer_summary)

    pool = generate_candidates(native, best_model, best_features)
    pool.to_csv(OUTPUT_DIR / "run63_candidate_pool_scored.csv", index=False)

    option_a = select_batch(pool, OPTION_A_COUNTS, "variable_N_recovery_anchor_batch48")
    option_b = select_batch(pool, OPTION_B_COUNTS, "smallN_recovery_batch32")
    option_c = select_batch(pool, OPTION_C_COUNTS, "optional_N40_followup_batch32")
    option_a.to_csv(OUTPUT_DIR / "run63_variable_N_recovery_anchor_batch48_candidate_orders.csv", index=False)
    option_b.to_csv(OUTPUT_DIR / "run63_smallN_recovery_batch32_candidate_orders.csv", index=False)
    option_c.to_csv(OUTPUT_DIR / "run63_optional_N40_followup_batch32_candidate_orders.csv", index=False)

    comparison, comparison_summary = compare_batches({"option_A_variable_N_recovery_anchor_batch48": option_a, "option_B_smallN_recovery_batch32": option_b, "option_C_optional_N40_followup_batch32": option_c}, native)
    comparison.to_csv(OUTPUT_DIR / "run63_batch_options_comparison_to_previous.csv", index=False)
    write_json(OUTPUT_DIR / "run63_batch_options_comparison_summary.json", comparison_summary)

    write_claim_boundary()
    evidence_freeze = write_evidence_freeze(native, validation, pred_summary, maturity)
    option_counts = {"option_A_variable_N_recovery_anchor_batch48": n_counts(option_a), "option_B_smallN_recovery_batch32": n_counts(option_b), "option_C_optional_N40_followup_batch32": n_counts(option_c)}
    summary = {
        "target_audit_headline": "Run63 target definitions prioritize N12/N16 recovery anchors, freeze N24/N40 evidence, and retain N40 follow-up only as a secondary option while preserving N32 metric-semantic warnings.",
    ...
    output_files = [
        OUTPUT_DIR / "run63_input_validation_summary.json",
        OUTPUT_DIR / "combined432_scan_order_features.csv",
        OUTPUT_DIR / "combined432_plus_N32_scan_order_features.csv",
        OUTPUT_DIR / "run63_target_reward_definition_audit.csv",
        OUTPUT_DIR / "run63_target_reward_definition_audit.json",
        OUTPUT_DIR / "run63_surrogate_validation_results_detailed.csv",
        OUTPUT_DIR / "run63_best_surrogate_configurations.csv",
        OUTPUT_DIR / "run63_surrogate_validation_summary.json",
        OUTPUT_DIR / "run63_gnn_reward_validation_results.csv",
        OUTPUT_DIR / "run63_gnn_reward_validation_summary.json",
        OUTPUT_DIR / "run63_graph_pointer_policy_training_log.csv",
        OUTPUT_DIR / "run63_graph_pointer_policy_validation_summary.json",
        OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.csv",
        OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.json",
        OUTPUT_DIR / "n24_n40_final_active_learning_rl_evidence_freeze.md",
        OUTPUT_DIR / "run63_candidate_pool_scored.csv",
        OUTPUT_DIR / "run63_variable_N_recovery_anchor_batch48_candidate_orders.csv",
        OUTPUT_DIR / "run63_smallN_recovery_batch32_candidate_orders.csv",
        OUTPUT_DIR / "run63_optional_N40_followup_batch32_candidate_orders.csv",
        OUTPUT_DIR / "run63_batch_options_comparison_to_previous.csv",
        OUTPUT_DIR / "run63_batch_options_comparison_summary.json",
        CLAIM_BOUNDARY_MD,
        CLAIM_BOUNDARY_JSON,
        REPORT_PATH,
        MANIFEST_PATH,
    ...
    manifest = {
        ...
        "input_files": [str(COMBINED432_READY), str(COMBINED432_PLUS_N32_READY), str(RUN61_ENRICHED), str(RUN61_PRED_SUMMARY), str(RUN62_MATURITY), str(RUN62_REPORT), str(RUN62_MANIFEST)],
        ...
        "best_variable_N_recovery_summary": surrogate_summary.get("best_variable_N_recovery"),
        "best_N40_followup_summary": surrogate_summary.get("best_N40_followup"),
        ...
        "candidate_pool_counts": n_counts(pool),
        "recommended_option": "option_A_variable_N_recovery_anchor_batch48",
        "batch_option_paths": {
            "option_A_variable_N_recovery_anchor_batch48": str(OUTPUT_DIR / "run63_variable_N_recovery_anchor_batch48_candidate_orders.csv"),
            "option_B_smallN_recovery_batch32": str(OUTPUT_DIR / "run63_smallN_recovery_batch32_candidate_orders.csv"),
            "option_C_optional_N40_followup_batch32": str(OUTPUT_DIR / "run63_optional_N40_followup_batch32_candidate_orders.csv"),
        },
