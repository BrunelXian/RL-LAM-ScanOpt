...
RUN_ID = "stageM_v02K2_odb_teacher_metric_extraction"
BATCH_NAME = "stage3_ppo_v02K2_targeted_N24_N40_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
STAGEL_GENERATION_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40"
    / "stageL_CAE_INP_handoff"
    / "manifest"
    / "stageL_v02K2_CAE_generation_manifest.csv"
)
STAGEL_CASE_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40"
    / "stageL_CAE_INP_handoff"
    / "manifest"
    / "stageL_v02K2_case_manifest.csv"
)
SELECTED_BATCH_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40"
    / "stageK2_n40_completion"
    / "selected_batch32_K2"
    / "v02K2_ppo_targeted_N24_N40_candidate_batch32.csv"
)
STAGEL_HANDOFF_MANIFEST_JSON = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40"
    / "stageL_CAE_INP_handoff"
    / "stageL_v02K2_CAE_INP_handoff_manifest.json"
)
STAGEL_COMMAND_FILE = (
    PROJECT_ROOT
    / "outputs"
    ...
    / "stageL_CAE_INP_handoff"
    / "commands"
    / "stageL_v02K2_batch32_abqjobpilot_commands_READY_TO_RUN.txt"
)
STAGEL_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40"
    / "PPO_V02K2_STAGEL_CAE_INP_HANDOFF_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40" / "stageM_ODB_teacher_metric_extraction"
REPORT_MD = PROJECT_ROOT / "docs" / "stage3_ppo_rl_lam_fea_addendum_v02_targeted_N24_N40" / "PPO_V02K2_STAGEM_ODB_TEACHER_METRIC_EXTRACTION_REPORT.md"
MANIFEST_PATH = OUTPUT_DIR / "stageM_v02K2_odb_teacher_metric_extraction_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "stageM_v02K2_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "stageM_v02K2_solver_completion_audit_summary.json"
AUDIT_MD = OUTPUT_DIR / "stageM_v02K2_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "stageM_v02K2_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "stageM_v02K2_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "stageM_v02K2_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "stageM_v02K2_teacher_metrics.json"
SUMMARY_JSON = OUTPUT_DIR / "stageM_v02K2_extraction_summary.json"
...
STAGEM_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_stageL": True,
    "final_step_required_for_metrics": "step_final_cooling",
    "targeted_batch": "PPO v02K2 N24/N40 batch32",
}
...
def load_manifest_rows() -> list[dict[str, str]]:
    with STAGEL_GENERATION_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
    ...
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} Stage L v02K2 batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside Stage L v02K2 batch32 batch name")
    ...
    for row in rows:
        ...
        merged["stageK2_role"] = selected.get("stageK2_role", "")
        merged["ppo_generation_mode"] = selected.get("ppo_generation_mode", "")
        merged["predicted_reward"] = selected.get("predicted_reward", "")
        merged["conservative_reward"] = selected.get("conservative_reward", "")
        normalized_rows.append(merged)
    if any(row["handoff_strategy_name"] not in selected_by_strategy for row in normalized_rows):
        raise RuntimeError("Stage L manifest contains strategies missing from selected v02K2 batch32 CSV")
...
def audit_case(row: dict[str, str]) -> dict[str, object]:
    ...
    return {
        ...
        "stageK2_role": row.get("stageK2_role", ""),
...
def extract_case(audit_row: dict[str, object]) -> dict[str, object]:
    base = {
        ...
        "stageK2_role": audit_row["stageK2_role"],
        "ppo_generation_mode": audit_row["ppo_generation_mode"],
        "predicted_reward": audit_row["predicted_reward"],
        "conservative_reward": audit_row["conservative_reward"],
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_STAGEM_V02K2_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_STAGEM_V02K2_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_STAGEM_V02K2_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "stageM_notes": STAGEM_NOTES,
        "stageL_report": str(STAGEL_REPORT_MD),
        "stageL_command_file": str(STAGEL_COMMAND_FILE),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_STAGEM_V02K2_ODB_TEACHER_METRIC_EXTRACTION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_STAGEM_V02K2_ODB_TEACHER_METRIC_EXTRACTION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Stage M v02K2 Solver Completion Audit",
        "",
...
def write_report(audit_summary: dict[str, object], extraction_summary: dict[str, object] | None, audit_rows: list[dict[str, object]], extraction_rows: list[dict[str, object]]) -> None:
    ...
    lines.extend(["", "## StageK2 Roles", ""])
    for role in sorted({str(row.get("stageK2_role", "")) for row in audit_rows if row.get("stageK2_role")}):
        role_count = sum(1 for row in audit_rows if row.get("stageK2_role") == role)
        lines.append(f"- `{role}`: `{role_count}` cases")
    incomplete = [row for row in audit_rows if row["completion_status"] == "FAIL_INCOMPLETE_OR_ABORTED"]
                ...
                extraction_rows.append(
                    {
                        ...
                        "conservative_reward": row["conservative_reward"],
                        "job_name": row["job_name"],
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "stageL_generation_manifest_csv": str(STAGEL_GENERATION_MANIFEST_CSV),
                    "stageL_case_manifest_csv": str(STAGEL_CASE_MANIFEST_CSV),
                    "selected_batch_csv": str(SELECTED_BATCH_CSV),
                    "stageL_handoff_manifest_json": str(STAGEL_HANDOFF_MANIFEST_JSON),
                    "stageL_command_file": str(STAGEL_COMMAND_FILE),
                    "stageL_report_md": str(STAGEL_REPORT_MD),
                },
                "stageM_notes": STAGEM_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
