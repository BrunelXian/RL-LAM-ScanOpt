"""Preflight Stage 3 run25 shortlist64 CAE/INP generation inputs."""
...
RUN24_DIR = PROJECT_ROOT / "outputs" / "stage3_run_24_run23_shortlist64_active_learning_handoff_package"
INPUT_CSV = RUN24_DIR / "stage3_run24_shortlist64_candidate_orders.csv"
SCAN_ORDER_DIR = RUN24_DIR / "scan_orders"
...
SUMMARY_PATH = OUTPUT_DIR / "run25_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run25_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run25_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run25_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN25_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN25_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    if len(rows) != 64:
        errors.append("row count {} != 64".format(len(rows)))
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "shortlist64":
            errors.append("row {} is not shortlist64: {}".format(idx, row.get("batch_option")))
        ...
        if "batch24" in lowered or "batch32" in lowered:
            errors.append("row {} references forbidden batch24/batch32 text".format(idx))
    ...
    verdict = "PASS_RUN25_SHORTLIST64_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN25_SHORTLIST64_INPUT_INVALID"
    summary = {
        "verdict": verdict,
