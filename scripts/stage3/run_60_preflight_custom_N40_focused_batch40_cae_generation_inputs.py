...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_59_run58_N40_focused_calibrated_penalty_repair_batch40_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run59_N40_focused_calibrated_penalty_repair_batch40_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run60_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run60_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run60_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run60_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN60_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN60_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "custom_N40_focused_calibrated_penalty_repair_batch40":
            errors.append("row {} is not custom_N40_focused_calibrated_penalty_repair_batch40: {}".format(idx, row.get("batch_option")))
        ...
        if "batch64" in lowered or "recovery" in lowered or "batch48" in lowered:
            errors.append("row {} references forbidden Run58 Option B batch64 / Option C recovery batch48 text".format(idx))
    ...
    verdict = "PASS_RUN60_CUSTOM_N40_FOCUSED_BATCH40_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN60_CUSTOM_N40_FOCUSED_BATCH40_INPUT_INVALID"
    summary = {
        "verdict": verdict,
