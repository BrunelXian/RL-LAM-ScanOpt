"""Fixed-N MaskablePPO environment for PPO v03 lex-primary N24/N40."""
...
try:
    ...
    raise RuntimeError("gymnasium is required for PPO v03 training") from exc

from v03_lex_primary_reward_interface import V03LexPrimaryReward
...
class V03EnvConfig:
    ...
    reward_clip_max: float | None = 1.5
...
class V03FixedNLamScanOrderPPOEnv(gym.Env):
    metadata = {"render_modes": []}

    def __init__(self, config: V03EnvConfig) -> None:
        super().__init__()
        if int(config.n) not in (24, 40):
            raise ValueError(f"v03 supports fixed N24/N40 only, got {config.n}")
        self.config = config
        self.n = int(config.n)
        self.reward_model = V03LexPrimaryReward.load(config.surrogate_model_path)
    ...
    def step(self, action: int):
        ...
        if done:
            if not validate_order(self.n, self.order):
                ...
                reward = self.reward_model.predict_reward(self.n, self.order)
