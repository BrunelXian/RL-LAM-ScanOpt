"""Create and validate run75 final_smallN_diagnostic_batch32 abqjobpilot command file.
...
COMMAND_FILE = OUTPUT_DIR / "stage3_run75_final_smallN_diagnostic_batch32_abqjobpilot_commands_READY_TO_RUN.txt"
CSV_PATH = OUTPUT_DIR / "stage3_run75_final_smallN_diagnostic_batch32_abqjobpilot_command_validation.csv"
SUMMARY_PATH = OUTPUT_DIR / "stage3_run75_final_smallN_diagnostic_batch32_abqjobpilot_command_validation_summary.json"
...
def validate_line(line: str, row: dict[str, str]) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "forbidden_batch_reference": "batch24" in line.lower() or "option_c" in line.lower(),
        "has_forbidden_n_command": int(row["n"]) == 32 or case_folder.startswith("N32"),
        "verdict": "FAIL",
    }
    checks = [
...
def main() -> int:
    ...
    errors = []
    if len(commands) != 32:
        errors.append("command count {} != 32".format(len(commands)))
    ...
    verdict = "PASS_RUN75_ABQJOBPILOT_COMMAND_FILE_VALID" if not errors else "FAIL_RUN75_ABQJOBPILOT_COMMAND_FILE_INVALID"
    summary = {
        "verdict": verdict,
    ...
    return 0 if verdict == "PASS_RUN75_ABQJOBPILOT_COMMAND_FILE_VALID" else 1
