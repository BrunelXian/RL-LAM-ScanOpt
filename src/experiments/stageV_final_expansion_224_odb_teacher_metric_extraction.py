...
RUN_ID = "stageV_final_expansion_224_odb_teacher_metric_extraction"
BATCH_NAME = "stage3_ppo_final_expansion_224_to_320_batch224_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
STAGEU_CASE_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_final_expansion_224_to_320"
    / "stageU_CAE_INP_handoff"
    / "manifest"
    / "stageU_final_expansion_case_manifest.csv"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_ppo_final_expansion_224_to_320" / "stageV_ODB_teacher_metric_extraction"
REPORT_MD = PROJECT_ROOT / "docs" / "stage3_ppo_final_expansion_224_to_320" / "PPO_FINAL_EXPANSION_STAGEV_ODB_TEACHER_METRIC_EXTRACTION_REPORT.md"
MANIFEST_PATH = OUTPUT_DIR / "stageV_ppo_final_expansion_224_extraction_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "stageV_completion_gate.csv"
AUDIT_JSON = OUTPUT_DIR / "stageV_completion_gate_summary.json"
AUDIT_MD = OUTPUT_DIR / "stageV_completion_gate.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "stageV_ppo_final_expansion_224_ODB_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "stageV_ppo_final_expansion_224_ODB_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "stageV_ppo_final_expansion_224_ODB_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "stageV_ppo_final_expansion_224_ODB_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "stageV_ppo_final_expansion_224_extraction_summary.json"
...
EXPECTED_BY_N = {12: 32, 16: 32, 24: 80, 40: 80}
EXPECTED_BY_BATCH = {
    "final_expansion_batch01": 32,
    "final_expansion_batch02": 32,
    "final_expansion_batch03": 32,
    "final_expansion_batch04": 32,
    "final_expansion_batch05": 32,
    "final_expansion_batch06": 32,
    "final_expansion_batch07": 32,
}
TOTAL_EXPECTED = 224
STAGEV_NOTES = {
    "namespace": "stage3_ppo_final_expansion_224_to_320",
    "includes_N12": True,
    "includes_N16": True,
    "includes_N24": True,
    "includes_N32_cases": False,
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_stageU": True,
    "final_step_required_for_metrics": "step_final_cooling",
    "targeted_batch": "PPO final expansion 224 to 320 batch224",
    "read_only_ODB_extraction": True,
}
...
def load_manifest_rows() -> list[dict[str, str]]:
    with STAGEU_CASE_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} Stage U final expansion manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside Stage U final expansion batch name")
    normalized_rows: list[dict[str, str]] = []
    for row in rows:
        strategy = row.get("strategy_name", "")
        merged = dict(row)
        merged["handoff_strategy_name"] = strategy
        merged["scan_order_json"] = row.get("order_json", "")
...
def audit_case(row: dict[str, str]) -> dict[str, object]:
    ...
    inp_path = Path(row.get("inp_path") or row.get("expected_inp") or case_dir / f"{job_name}.inp")
    ...
    return {
        ...
        "final_expansion_batch": row.get("final_expansion_batch", ""),
        "n": int(row["n"]),
        "handoff_strategy_name": strategy,
        "selected_by_bucket": row.get("selected_by_bucket", ""),
        "global_candidate_index": row.get("global_candidate_index", ""),
        ...
        "ppo_version_source": row.get("ppo_version_source", ""),
        "ppo_generation_mode": row.get("ppo_generation_mode", ""),
        "predicted_quality_score": row.get("predicted_quality_score", ""),
        "min_hamming_to_combined552_sameN": row.get("min_hamming_to_combined552_sameN", ""),
        "min_hamming_to_conventional_baseline": row.get("min_hamming_to_conventional_baseline", ""),
        "nearest_conventional_baseline": row.get("nearest_conventional_baseline", ""),
...
def extract_case(audit_row: dict[str, object]) -> dict[str, object]:
    base = {
        ...
        "final_expansion_batch": audit_row["final_expansion_batch"],
        "n": audit_row["n"],
        "handoff_strategy_name": audit_row["handoff_strategy_name"],
        "selected_by_bucket": audit_row["selected_by_bucket"],
        "global_candidate_index": audit_row["global_candidate_index"],
        "order_hash": audit_row["order_hash"],
        "order_compact": audit_row["order_compact"],
        "scan_order_json": audit_row["scan_order_json"],
        "candidate_source": audit_row["candidate_source"],
        "ppo_checkpoint": audit_row["ppo_checkpoint"],
        "ppo_version_source": audit_row["ppo_version_source"],
        "ppo_generation_mode": audit_row["ppo_generation_mode"],
        "predicted_quality_score": audit_row["predicted_quality_score"],
        "min_hamming_to_combined552_sameN": audit_row["min_hamming_to_combined552_sameN"],
        "min_hamming_to_conventional_baseline": audit_row["min_hamming_to_conventional_baseline"],
        "nearest_conventional_baseline": audit_row["nearest_conventional_baseline"],
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    for batch, expected in EXPECTED_BY_BATCH.items():
        group = [row for row in audit_rows if str(row["final_expansion_batch"]) == batch]
        by_batch[batch] = {
            "expected": expected,
            "audited": len(group),
            "complete": sum(1 for row in group if row["completion_status"] in {"PASS_SOLVER_COMPLETE", "WARNING_SUCCESS_WITH_WARNINGS"}),
    ...
    expected_ok = all(by_n[f"N{n}"]["audited"] == expected for n, expected in EXPECTED_BY_N.items()) and all(
        by_batch[batch]["audited"] == expected for batch, expected in EXPECTED_BY_BATCH.items()
    )
    if failed or not expected_ok:
        verdict = "FAIL_STAGEV_FINAL_EXPANSION_224_COMPLETION_GATE_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_STAGEV_FINAL_EXPANSION_224_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_STAGEV_FINAL_EXPANSION_224_COMPLETION_GATE_224_OF_224"
    return {
        ...
        "stageV_notes": STAGEV_NOTES,
        "stageU_case_manifest_csv": str(STAGEU_CASE_MANIFEST_CSV),
        "by_N": by_n,
        "by_batch": by_batch,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    by_batch: dict[str, dict[str, int]] = {}
    for batch, expected in EXPECTED_BY_BATCH.items():
        group = [row for row in rows if str(row.get("final_expansion_batch", "")) == batch]
        by_batch[batch] = {
            "expected": expected,
            "extracted": sum(1 for row in group if row["teacher_validation_status"] == "PASS_TEACHER_FIELDS_EXTRACTED"),
            "failed": sum(1 for row in group if row["teacher_validation_status"] != "PASS_TEACHER_FIELDS_EXTRACTED"),
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_STAGEV_FINAL_EXPANSION_224_ODB_TEACHER_METRIC_EXTRACTION_224_OF_224" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_STAGEV_FINAL_EXPANSION_224_ODB_TEACHER_METRIC_EXTRACTION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Stage V Final Expansion 224 Completion Gate",
        "",
    ...
    lines.extend(["", "| final_expansion_batch | expected | complete | warning | failed_or_incomplete |", "|---|---:|---:|---:|---:|"])
    for batch in EXPECTED_BY_BATCH:
        group = audit_summary["by_batch"][batch]
        lines.append(f"| {batch} | {group['expected']} | {group['complete']} | {group['warning']} | {group['failed_or_incomplete']} |")
...
def write_report(audit_summary: dict[str, object], extraction_summary: dict[str, object] | None, audit_rows: list[dict[str, object]], extraction_rows: list[dict[str, object]]) -> None:
    ...
    lines.extend(["", "## Batch Summary", "", "| final_expansion_batch | expected | complete | warning | failed_or_incomplete |", "|---|---:|---:|---:|---:|"])
    for batch in EXPECTED_BY_BATCH:
        group = audit_summary["by_batch"][batch]
        lines.append(f"| {batch} | {group['expected']} | {group['complete']} | {group['warning']} | {group['failed_or_incomplete']} |")
    incomplete = [row for row in audit_rows if row["completion_status"] == "FAIL_INCOMPLETE_OR_ABORTED"]
    ...
    if extraction_summary is not None:
        ...
        lines.extend(["", "| final_expansion_batch | expected | extracted | failed |", "|---|---:|---:|---:|"])
        for batch in EXPECTED_BY_BATCH:
            group = extraction_summary["by_batch"][batch]
            lines.append(f"| {batch} | {group['expected']} | {group['extracted']} | {group['failed']} |")
...
def main() -> int:
    for directory in [OUTPUT_DIR, REPORT_MD.parent]:
                ...
                extraction_rows.append(
                    {
                        ...
                        "global_candidate_index": row["global_candidate_index"],
                        ...
                        "predicted_quality_score": row["predicted_quality_score"],
                        "min_hamming_to_combined552_sameN": row["min_hamming_to_combined552_sameN"],
                        "min_hamming_to_conventional_baseline": row["min_hamming_to_conventional_baseline"],
                        "nearest_conventional_baseline": row["nearest_conventional_baseline"],
                        "job_name": row["job_name"],
