...
RUN_ID = "run_71_smallN_recovery_focused_batch40_odb_teacher_validation"
BATCH_NAME = "stage3_run69_smallN_recovery_focused_batch40_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN70_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_70_smallN_recovery_focused_batch40_cae_inp_generation"
    / "stage3_run70_smallN_recovery_focused_batch40_cae_generation_manifest.csv"
)
RUN70_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_70_smallN_recovery_focused_batch40_cae_inp_generation"
    / "RUN_70_SMALLN_RECOVERY_FOCUSED_BATCH40_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_71_smallN_recovery_focused_batch40_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_71_smallN_recovery_focused_batch40_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_71_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_solver_completion_audit.json"
AUDIT_MD = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_teacher_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "run71_smallN_recovery_focused_batch40_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_71_SMALLN_RECOVERY_FOCUSED_BATCH40_ODB_TEACHER_VALIDATION_REPORT.md"
...
EXPECTED_BY_N = {12: 16, 16: 16, 24: 4, 40: 4}
TOTAL_EXPECTED = 40
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run70": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN70_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run70 small-N recovery-focused batch40 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run70 small-N recovery-focused batch40 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN71_SMALLN_RECOVERY_FOCUSED_BATCH40_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN71_SMALLN_RECOVERY_FOCUSED_BATCH40_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN71_SMALLN_RECOVERY_FOCUSED_BATCH40_SOLVER_COMPLETION_40_OF_40"
    return {
        ...
        "run70_report": str(RUN70_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN71_SMALLN_RECOVERY_FOCUSED_BATCH40_ODB_TEACHER_VALIDATION_40_OF_40" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN71_SMALLN_RECOVERY_FOCUSED_BATCH40_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Run71 Small-N Recovery-Focused Batch40 Solver Completion Audit",
        "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run70_manifest_csv": str(RUN70_MANIFEST_CSV),
                    "run70_report_md": str(RUN70_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
