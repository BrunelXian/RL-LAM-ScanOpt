...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_44_run43_constrained_N24_N40_batch32_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run44_constrained_N24_N40_batch32_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run45_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run45_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run45_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run45_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN45_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN45_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    if len(rows) != 32:
        errors.append("row count {} != 32".format(len(rows)))
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "constrained_N24_N40_batch32":
            errors.append("row {} is not constrained_N24_N40_batch32: {}".format(idx, row.get("batch_option")))
        ...
        if "batch60" in lowered or "recovery" in lowered:
            errors.append("row {} references forbidden batch60/recovery text".format(idx))
        if n not in EXPECTED_NS:
            errors.append("row {} unexpected N {}".format(idx, n))
        if n in (12, 16, 32):
            errors.append("row {} contains forbidden N{} case".format(idx, n))
        ...
        normalized.append(
            {
                ...
                "original_run43_candidate_id": row.get("original_run43_candidate_id", ""),
                "original_run43_strategy_name": row.get("original_run43_strategy_name", ""),
                ...
                "constrained_reward_balanced": row.get("constrained_reward_balanced", ""),
    ...
    verdict = "PASS_RUN45_CONSTRAINED_N24_N40_BATCH32_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN45_CONSTRAINED_N24_N40_BATCH32_INPUT_INVALID"
    summary = {
        "verdict": verdict,
