"""Create and validate run25 shortlist64 abqjobpilot command file.
...
COMMAND_FILE = OUTPUT_DIR / "stage3_run25_shortlist64_abqjobpilot_commands_READY_TO_RUN.txt"
CSV_PATH = OUTPUT_DIR / "stage3_run25_shortlist64_abqjobpilot_command_validation.csv"
SUMMARY_PATH = OUTPUT_DIR / "stage3_run25_shortlist64_abqjobpilot_command_validation_summary.json"
...
def validate_line(line: str, row: dict[str, str]) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "forbidden_batch_reference": "batch24" in line.lower() or "batch32" in line.lower(),
        "verdict": "FAIL",
    }
    checks = [
...
def main() -> int:
    ...
    errors = []
    if len(commands) != 64:
        errors.append("command count {} != 64".format(len(commands)))
    ...
    verdict = "PASS_RUN25_ABQJOBPILOT_COMMAND_FILE_VALID" if not errors else "FAIL_RUN25_ABQJOBPILOT_COMMAND_FILE_INVALID"
    summary = {
        "verdict": verdict,
    ...
    return 0 if verdict == "PASS_RUN25_ABQJOBPILOT_COMMAND_FILE_VALID" else 1
