"""Validate the strict 2-cell local primitive against fixed segment-6."""
...
BASELINE_CSV = TABLE_DIR / "reward_breakdown_baselines_local_primitive.csv"
BASELINE_SUMMARY_TXT = TABLE_DIR / "reward_breakdown_baselines_local_primitive_summary.txt"
BASELINE_SUMMARY_MD = TABLE_DIR / "reward_breakdown_baselines_local_primitive_summary.md"
BREAKDOWN_PLOT = FIGURE_DIR / "baseline_reward_breakdown_local_primitive.png"
HEATMAP_PLOT = FIGURE_DIR / "baseline_heatmap_comparison_local_primitive.png"
ORDER_PLOT = FIGURE_DIR / "baseline_scan_order_comparison_local_primitive.png"

COMPARISON_CSV = TABLE_DIR / "local_primitive_vs_segment6_comparison.csv"
COMPARISON_TXT = TABLE_DIR / "local_primitive_vs_segment6_comparison.txt"
COMPARISON_MD = TABLE_DIR / "local_primitive_vs_segment6_comparison.md"
VERDICT_TXT = TABLE_DIR / "local_primitive_action_verdict.txt"
VERDICT_MD = TABLE_DIR / "local_primitive_action_verdict.md"
...
def make_local_primitive_env(mask: np.ndarray, reward_weights: dict[str, float]) -> ScanPlanningLocalPrimitiveEnv:
    """Create the strict local primitive environment."""
...
def get_action_center(env: Any, action: int) -> tuple[int, int] | None:
    ...
    if isinstance(env, ScanPlanningLocalPrimitiveEnv):
...
def choose_local_primitive_action(
    cell: tuple[int, int],
    lookahead_cells: list[tuple[int, int]],
    env: ScanPlanningLocalPrimitiveEnv,
    covered: set[tuple[int, int]],
) -> int:
    """Choose the best 2-cell primitive anchored at one cell."""
    row, col = cell
    ...
    if best_action is None:
        raise RuntimeError("No valid local primitive action found for planner compression.")
    return best_action


def compress_local_primitive_actions(
    cell_plan: list[tuple[int, int]],
    env: ScanPlanningLocalPrimitiveEnv,
) -> list[int]:
    """Compress a cell-order plan into 2-cell local primitive actions."""
    actions: list[int] = []
    ...
    for index, cell in enumerate(cell_plan):
        ...
        action_index = choose_local_primitive_action(cell, lookahead_cells, env, covered)
        actions.append(action_index)
...
def build_primitive_catalog_rows(env: ScanPlanningLocalPrimitiveEnv) -> list[dict[str, Any]]:
    """Summarise local primitive catalog locality."""
    sizes = [len(cells) for cells in env.action_cells if cells]
    return [
        {
            "environment": "local_primitive",
...
def main() -> None:
    """Run baseline validation and diagnostics for the local primitive environment."""
    ...
    primitive_env = make_local_primitive_env(target_mask, reward_weights)
    primitive_env.reset(seed=42)
    primitive_catalog_rows = build_primitive_catalog_rows(primitive_env)
    ...
    for planner_name, cell_plan in cell_plans.items():
        action_plan = compress_local_primitive_actions(cell_plan, primitive_env)
        result = run_action_plan(make_local_primitive_env, target_mask, reward_weights, action_plan)
        primitive_results[planner_name] = result
    ...
    save_comparison_grid(
        ...
        title="Local Primitive Baseline Heatmap Comparison",
        cmap="magma",
    ...
    save_comparison_grid(
        ...
        ORDER_PLOT,
        field_key="order_map",
        title="Local Primitive Baseline Scan Order Comparison",
        cmap="inferno",
    ...
    summary_lines = [
        "Local primitive baseline summary",
        "- Ranking by total reward: " + ", ".join(
            f"{row['planner']} ({float(row['total_reward']):.3f})" for row in ranking
        ),
    ]
    write_text_pair(BASELINE_SUMMARY_TXT, BASELINE_SUMMARY_MD, "Local Primitive Baseline Summary", summary_lines)

    segment_signal = reward_signal_share(make_segment6_env, target_mask, reward_weights)
    primitive_signal = reward_signal_share(make_local_primitive_env, target_mask, reward_weights)
    ...
    primitive_env_for_compression = make_local_primitive_env(target_mask, reward_weights)
    primitive_env_for_compression.reset(seed=42)
    primitive_random = run_action_plan(
        make_local_primitive_env,
        target_mask,
        reward_weights,
        compress_local_primitive_actions(cell_plans["random"], primitive_env_for_compression),
    )
    primitive_distance = run_action_plan(
        make_local_primitive_env,
        target_mask,
        reward_weights,
        compress_local_primitive_actions(cell_plans["distance_aware_cool_first"], primitive_env_for_compression),
    )
    primitive_cool = run_action_plan(
        make_local_primitive_env,
        target_mask,
        reward_weights,
        compress_local_primitive_actions(cell_plans["cool_first"], primitive_env_for_compression),
    )
    primitive_env_for_compression.close()
    ...
    comparison_rows = [
        ...
        *primitive_catalog_rows,
        ...
        },
        {
            "environment": "local_primitive",
            "metric": "coverage_completion_share",
            "value": primitive_signal["coverage_completion_share"],
        },
        {
            "environment": "local_primitive",
            "metric": "thermal_share",
            "value": primitive_signal["thermal_share"],
        },
        {
            "environment": "local_primitive",
            "metric": "jump_share",
            "value": primitive_signal["jump_share"],
        },
    ]
    for environment_name, result in (
        ...
        ("local_primitive_random", primitive_random),
        ("local_primitive_distance_aware_cool_first", primitive_distance),
        ("local_primitive_cool_first", primitive_cool),
    ):
    ...
    primitive_distance_metrics = early_metrics(primitive_distance)
    primitive_mean = next(row["value"] for row in primitive_catalog_rows if row["metric"] == "affected_cells_mean")
    primitive_catalog_size = next(row["value"] for row in primitive_catalog_rows if row["metric"] == "action_catalog_size")
    distance_top = ranking[0]["planner"]
    raster_bottom = ranking[-1]["planner"]

    comparison_lines = [
        f"- Segment6 locality mean={np.mean(segment_sizes):.2f}, local-primitive locality mean={float(primitive_mean):.2f}",
        f"- Segment6 thermal share={segment_signal['thermal_share']:.3f}, local-primitive thermal share={primitive_signal['thermal_share']:.3f}",
        f"- Segment6 action catalog size={segment_action_count}, local-primitive action catalog size={int(primitive_catalog_size)}",
        f"- Segment6 distance-aware early adjacency={segment_distance_metrics['early_adjacency_ratio']:.3f}, local-primitive distance-aware early adjacency={primitive_distance_metrics['early_adjacency_ratio']:.3f}",
        f"- Segment6 distance-aware cumulative early reheat={segment_distance_metrics['cumulative_early_reheat']:.3f}, local-primitive distance-aware cumulative early reheat={primitive_distance_metrics['cumulative_early_reheat']:.3f}",
        f"- Local-primitive ranking top={distance_top}, bottom={raster_bottom}",
    ]
    write_text_pair(COMPARISON_TXT, COMPARISON_MD, "Local Primitive vs Segment6 Comparison", comparison_lines)

    more_local = float(primitive_mean) <= 2.5 and float(primitive_mean) < float(np.mean(segment_sizes))
    thermal_not_weaker = primitive_signal["thermal_share"] >= segment_signal["thermal_share"]
    sensible_ranking = distance_top == "distance_aware_cool_first" and raster_bottom == "raster"
    distance_structured = primitive_distance_metrics["early_adjacency_ratio"] <= 0.10
    better_candidate = more_local and thermal_not_weaker and sensible_ranking and distance_structured

    verdict_lines = [
        f"1. Is this new local primitive more local than fixed segment-6? {'YES' if more_local else 'NO'}",
        f"2. Is thermal signal share stronger or at least not weaker than fixed segment-6? {'YES' if thermal_not_weaker else 'NO'}",
        f"3. Does it preserve sensible baseline ranking? {'YES' if sensible_ranking else 'NO'}",
        f"4. Does distance_aware_cool_first remain low-clustering and structured? {'YES' if distance_structured else 'NO'}",
        f"5. Is this genuinely a better PPO candidate than fixed segment-6? {'YES' if better_candidate else 'NO'}",
        "",
        (
            f"Most important quantitative improvement: action catalog size {segment_action_count} -> {int(primitive_catalog_size)} "
            f"with locality mean {np.mean(segment_sizes):.2f} -> {float(primitive_mean):.2f}."
        ),
        (
            f"Most important remaining weakness: thermal share {segment_signal['thermal_share']:.3f} -> {primitive_signal['thermal_share']:.3f} "
            f"and distance-aware early adjacency {segment_distance_metrics['early_adjacency_ratio']:.3f} -> {primitive_distance_metrics['early_adjacency_ratio']:.3f}."
        ),
        "",
        (
            "GO: local primitive is a better PPO candidate than fixed segment-6"
            if better_candidate
            else "NO-GO: local primitive does not improve the representation enough; patch-based action should be deprioritized"
        ),
    ]
    write_text_pair(VERDICT_TXT, VERDICT_MD, "Local Primitive Action Verdict", verdict_lines)

    print("Local primitive diagnostics complete.")
