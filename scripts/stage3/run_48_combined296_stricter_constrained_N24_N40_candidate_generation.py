...
RUN_ID = "run_48_combined296_stricter_constrained_N24_N40_candidate_generation"
RUN_NAME = "combined296 stricter constrained N24/N40 candidate generation"
SCRIPT_PATH = ROOT / "scripts" / "stage3" / "run_48_combined296_stricter_constrained_N24_N40_candidate_generation.py"
...
RUN46_ENRICHED = ROOT / "outputs" / "stage3_run_47_constrained_N24_N40_batch32_teacher_metrics_ingestion_and_combined296_ranking" / "run46_constrained_N24_N40_batch32_teacher_dataset_enriched.csv"
RUN46_COMPARISON = ROOT / "outputs" / "stage3_run_47_constrained_N24_N40_batch32_teacher_metrics_ingestion_and_combined296_ranking" / "run46_vs_combined264_best_comparison.csv"
RUN46_EFFECTIVENESS = ROOT / "outputs" / "stage3_run_47_constrained_N24_N40_batch32_teacher_metrics_ingestion_and_combined296_ranking" / "run46_constrained_batch32_effectiveness_audit.csv"
RUN46_PRED_AUDIT = ROOT / "outputs" / "stage3_run_47_constrained_N24_N40_batch32_teacher_metrics_ingestion_and_combined296_ranking" / "run46_prediction_audit_for_run43_batch32.csv"
RUN46_PRED_SUMMARY = ROOT / "outputs" / "stage3_run_47_constrained_N24_N40_batch32_teacher_metrics_ingestion_and_combined296_ranking" / "run46_prediction_audit_for_run43_batch32_summary.json"
...
RUN47_MANIFEST = ROOT / "artifacts" / "manifests" / "stage3_run_47_manifest.json"
...
OUTPUT_DIR = ROOT / "outputs" / "stage3_run_48_combined296_stricter_constrained_N24_N40_candidate_generation"
REPORT_DIR = ROOT / "docs" / "stage3" / "runs" / "run_48_combined296_stricter_constrained_N24_N40_candidate_generation"
REPORT_PATH = REPORT_DIR / "RUN_48_COMBINED296_STRICTER_CONSTRAINED_N24_N40_CANDIDATE_GENERATION_REPORT.md"
MANIFEST_PATH = ROOT / "artifacts" / "manifests" / "stage3_run_48_manifest.json"
CLAIM_BOUNDARY_MD = OUTPUT_DIR / "run48_claim_boundary.md"
CLAIM_BOUNDARY_JSON = OUTPUT_DIR / "run48_claim_boundary.json"

EXPECTED_NATIVE = {12: 36, 16: 36, 24: 112, 40: 112}
EXPECTED_PLUS = {12: 36, 16: 36, 24: 112, 32: 332, 40: 112}
POOL_TARGETS = {12: 500, 16: 500, 24: 5000, 40: 5000}
OPTION_A_COUNTS = {24: 16, 40: 16}
OPTION_B_COUNTS = {24: 30, 40: 30}
...
def add_features_and_targets(df: pd.DataFrame, native: bool) -> pd.DataFrame:
    ...
    if native:
        ...
        out["target_reward_combined296_u2_primary"] = as_float(out["target_reward_combined296_u2_primary"])
        prefix = "combined296"
    else:
        score_cols = [
            "target_u2_score_combined296_plus_N32_rank",
            "target_peeq_score_combined296_plus_N32_rank",
            "target_surfaceT_score_combined296_plus_N32_rank",
            "target_mises_score_combined296_plus_N32_rank",
        ]
        out["target_reward_combined296_plus_N32_mapped_u2_primary"] = as_float(out["target_reward_combined296_plus_N32_mapped_u2_primary"])
        prefix = "combined296_plus_N32"
...
def validate_inputs(native: pd.DataFrame, plus: pd.DataFrame, pred_summary: dict[str, Any]) -> dict[str, Any]:
    errors = []
    if len(native) != 296 or n_counts(native) != EXPECTED_NATIVE:
        errors.append(f"native combined296 count mismatch: rows={len(native)} counts={n_counts(native)}")
    if 32 in set(native["n"].astype(int)):
        errors.append("native combined296 contains N32")
    if len(plus) != 628 or n_counts(plus) != EXPECTED_PLUS:
        errors.append(f"combined296_plus_N32 count mismatch: rows={len(plus)} counts={n_counts(plus)}")
    ...
    return {
        "timestamp": now_iso(),
        "verdict": "PASS_RUN48_COMBINED296_AND_PLUS_N32_INPUTS_READY" if not errors else "FAIL_RUN48_INPUT_VALIDATION",
        "errors": errors,
        "native_combined296_rows": int(len(native)),
        "native_combined296_counts": n_counts(native),
        "combined296_plus_N32_rows": int(len(plus)),
        "combined296_plus_N32_counts": n_counts(plus),
        "run46_prediction_audit_context": {"overall_reward_spearman": reward_s, "mean_top5_overlap": top5},
    }
...
def run_surrogate_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], Any, list[str]]:
    ...
    targets = {
        "native_combined296": [
            "target_reward_combined296_u2_primary",
            "target_reward_combined296_constrained_u2_reward_balanced",
            "target_reward_combined296_strict_penalty_guard",
            "target_reward_combined296_two_stage_guarded",
            "target_reward_combined296_no_penalty_worse_than_median",
            "target_reward_combined296_u2_guarded",
        ],
        "plus_N32_unweighted": [
            "target_reward_combined296_plus_N32_mapped_u2_primary",
            "target_reward_combined296_plus_N32_strict_u2_surfaceT",
            "target_reward_combined296_plus_N32_constrained_u2_reward_balanced",
            "target_reward_combined296_plus_N32_strict_penalty_guard",
        ],
        "plus_N32_balanced": [
            "target_reward_combined296_plus_N32_mapped_u2_primary",
            "target_reward_combined296_plus_N32_strict_u2_surfaceT",
    ...
    frames = {"native_combined296": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}
    rows = []
    ...
    native_best = grouped[grouped["regime"] == "native_combined296"].head(1).iloc[0].to_dict()
    ...
    summary = {
        ...
        "run38_reference": {"native_spearman": 0.8681, "native_top5": 2.0, "plus_N32_balanced_spearman": 0.8503, "plus_N32_balanced_top5": 1.6},
    }
...
def run_gnn_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    rows = []
    for regime, df in {"native_combined296": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}.items():
        target = "target_reward_combined296_u2_primary" if regime == "native_combined296" else "target_reward_combined296_plus_N32_mapped_u2_primary"
        sw = sample_weights(df) if regime.endswith("balanced") else None
        for protocol, train, test in validation_protocols(df, regime):
            try:
...
def generate_candidates(native: pd.DataFrame, model: Any, features: list[str]) -> pd.DataFrame:
    ...
    for n in [12, 16, 24, 40]:
        ...
        best_by[(n, "reward")] = parse_order(g.sort_values("target_reward_combined296_u2_primary", ascending=False).iloc[0]["order_json"])
        best_by[(n, "constrained")] = parse_order(g.sort_values("target_reward_combined296_constrained_u2_reward_balanced", ascending=False).iloc[0]["order_json"])
        best_by[(n, "strict")] = parse_order(g.sort_values("target_reward_combined296_strict_penalty_guard", ascending=False).iloc[0]["order_json"])
        best_by[(n, "two_stage")] = parse_order(g.sort_values("target_reward_combined296_two_stage_guarded", ascending=False).iloc[0]["order_json"])
    rows, seen = [], set(existing)
    source_plan = [
        "strict_penalty_guard_top", "strict_penalty_guard_local_search", "two_stage_guarded_top",
        "two_stage_guarded_local_search", "no_penalty_worse_than_median", "constrained_surrogate_top",
        "constrained_surrogate_local_search", "U2_top_region_penalty_repair", "PEEQ_repair_candidates",
        "SurfaceT_repair_candidates", "Mises_repair_candidates", "hybrid_agreement",
    ...
    for n, target_count in POOL_TARGETS.items():
        ...
        while sum(1 for r in rows if r["n"] == n) < target_count and attempts < target_count * 80:
            ...
            if source in {"U2_top_region_penalty_repair", "PEEQ_repair_candidates"}:
                base = best_by[(n, "u2")]
            elif source in {"strict_penalty_guard_top", "strict_penalty_guard_local_search", "Mises_repair_candidates"}:
                base = best_by[(n, "strict")]
            elif source in {"two_stage_guarded_top", "two_stage_guarded_local_search", "no_penalty_worse_than_median"}:
                base = best_by[(n, "two_stage")]
            elif source in {"SurfaceT_repair_candidates", "constrained_surrogate_local_search"}:
            ...
            constrained = 0.45 * strict + 0.35 * two_stage + 0.20 * median_guard - 0.12 * max(0.0, 0.14 - near_d)
            cid = f"R48_N{n}_C{sum(1 for r in rows if r['n']==n)+1:05d}"
            rows.append({
                "candidate_id": cid,
...
def select_batch(pool: pd.DataFrame, counts: dict[int, int], label: str) -> pd.DataFrame:
    selected = []
    bucket_order = ["strict_penalty_guard_top", "strict_penalty_guard_local_search", "two_stage_guarded_top", "two_stage_guarded_local_search", "no_penalty_worse_than_median", "PEEQ_repair_candidates", "SurfaceT_repair_candidates", "Mises_repair_candidates", "U2_top_region_penalty_repair", "constrained_surrogate_top", "hybrid_agreement", "hybrid_disagreement", "uncertainty_calibration", "diversity_coverage", "sentinel_control"]
    for n, count in counts.items():
        ...
        for bucket in bucket_order:
            ...
            for _, row in bg.head(per_bucket).iterrows():
                if len(take) < count:
        ...
        for i, row in enumerate(take[:count], 1):
            short = str(row["candidate_source"]).replace("_local_search", "").replace("_candidates", "").replace("constrained_", "constr").replace("strict_penalty_guard", "strict_guard")[:30]
            row["batch_option"] = label
            row["handoff_strategy_name"] = f"S3R48SPG_N{n}_B{i:02d}_{short}"
...
def compare_batches(options: dict[str, pd.DataFrame], native: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    previous = {"combined296_teacher_orders": set(native["order_hash"].astype(str))}
    for name, path in {"run46": RUN46_HANDOFF, "run41": RUN41_HANDOFF, "run36": RUN36_HANDOFF, "run27": RUN27_HANDOFF, "superseded_run31": RUN31_OLD}.items():
        if path.exists():
    ...
    headline = "All Run48 options were checked for exact order overlap and strict-guard/novelty composition against combined296, Run46, Run41, Run36, Run27, and superseded Run31."
    return comp, {"headline": headline, "rows": comp.to_dict(orient="records")}


def write_claim_boundary() -> None:
    safe = [
        "Run48 updates offline models using native combined296 and combined296_plus_N32.",
        "Run48 evaluates stricter penalty-guarded targets after Run46 produced U2/reward gains without raw PEEQ/SurfaceT/Mises records.",
        "Run48 generates candidate-order batches for future teacher validation.",
        "Run48 does not include teacher validation for new candidates.",
    ]
    unsafe = [
    ...
    CLAIM_BOUNDARY_MD.write_text("# Run48 Claim Boundary\n\n## Safe claims\n" + "\n".join(f"- {x}" for x in safe) + "\n\n## Unsafe claims\n" + "\n".join(f"- {x}" for x in unsafe) + "\n", encoding="utf-8")
    write_json(CLAIM_BOUNDARY_JSON, {"verdict": "RUN48_MODEL_UPDATE_AND_CANDIDATE_GENERATION_ONLY_NO_TEACHER_VALIDATION", "safe_claims": safe, "unsafe_claims": unsafe})


def write_report(summary: dict[str, Any]) -> None:
    REPORT_PATH.write_text(f"""# Stage 3 Run 48 - Combined296 Stricter Constrained N24/N40 Candidate Generation
...
## 2. Inputs
- Native combined296: `{COMBINED296_READY}`
- combined296_plus_N32: `{COMBINED296_PLUS_N32_READY}`
- Run46 prediction audit: `{RUN46_PRED_SUMMARY}`
...
Candidate pool counts: `{summary['candidate_pool_counts']}`. N24 and N40 each meet the >=5000 candidate minimum.

## 10. Option A Stricter Constrained N24/N40 Batch32
Path: `{OUTPUT_DIR / 'run48_stricter_constrained_N24_N40_batch32_candidate_orders.csv'}`. Counts: `{summary['option_counts']['A']}`.

## 11. Option B Stricter Constrained N24/N40 Batch60
Path: `{OUTPUT_DIR / 'run48_stricter_constrained_N24_N40_batch60_candidate_orders.csv'}`. Counts: `{summary['option_counts']['B']}`.

## 12. Option C Native Recovery Batch40 With Anchors
Path: `{OUTPUT_DIR / 'run48_native_recovery_batch40_with_anchors_candidate_orders.csv'}`. Counts: `{summary['option_counts']['C']}`.
...
## 15. Output Files
- Candidate pool: `{OUTPUT_DIR / 'run48_candidate_pool_scored.csv'}`
- Surrogate summary: `{OUTPUT_DIR / 'run48_surrogate_validation_summary.json'}`
- GNN summary: `{OUTPUT_DIR / 'run48_gnn_reward_validation_summary.json'}`
- Pointer summary: `{OUTPUT_DIR / 'run48_graph_pointer_policy_validation_summary.json'}`
- Batch comparison: `{OUTPUT_DIR / 'run48_batch_options_comparison_summary.json'}`
...
def main() -> None:
    ensure_dirs()
    native = add_features_and_targets(read_csv(COMBINED296_READY), native=True)
    plus = add_features_and_targets(read_csv(COMBINED296_PLUS_N32_READY), native=False)
    pred_summary = json.loads(RUN46_PRED_SUMMARY.read_text(encoding="utf-8")) if RUN46_PRED_SUMMARY.exists() else {}
    validation = validate_inputs(native, plus, pred_summary)
    write_json(OUTPUT_DIR / "run48_input_validation_summary.json", validation)
    if not validation["verdict"].startswith("PASS"):
        raise SystemExit(validation["errors"])

    native.to_csv(OUTPUT_DIR / "combined296_scan_order_features.csv", index=False)
    plus.to_csv(OUTPUT_DIR / "combined296_plus_N32_scan_order_features.csv", index=False)
    target_rows = [
        {"target": "target_reward_combined296_u2_primary", "definition": "0.65 U2 + 0.20 PEEQ + 0.10 SurfaceT + 0.05 Mises", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_constrained_u2_reward_balanced", "definition": "0.50 U2 + 0.25 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_strict_penalty_guard", "definition": "0.40 U2 + 0.30 PEEQ + 0.20 SurfaceT + 0.10 Mises", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_two_stage_guarded", "definition": "top-25-percent U2 gate followed by weighted PEEQ/SurfaceT/Mises penalty ranking", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_no_penalty_worse_than_median", "definition": "top-35-percent U2 gate with median guards for PEEQ, SurfaceT, and Mises", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_u2_guarded", "definition": "U2-primary score with penalties for below-median and bottom-quartile PEEQ/SurfaceT/Mises scores within N", "dataset": "native_combined296"},
        {"target": "target_reward_combined296_plus_N32_mapped_u2_primary", "definition": "mapped U2-primary target with N32 semantic warnings preserved", "dataset": "combined296_plus_N32"},
        {"target": "target_reward_combined296_plus_N32_strict_u2_surfaceT", "definition": "strict U2 + SurfaceT target for safer N32 metric semantics", "dataset": "combined296_plus_N32"},
    ]
    pd.DataFrame(target_rows).to_csv(OUTPUT_DIR / "run48_target_reward_definition_audit.csv", index=False)
    write_json(OUTPUT_DIR / "run48_target_reward_definition_audit.json", {"headline": "Stricter constrained targets were created to keep U2 near the top region while increasing PEEQ, SurfaceT, and Mises guard pressure.", "targets": target_rows})

    detailed, best_cfg, surrogate_summary, best_model, best_features = run_surrogate_validation(native, plus)
    detailed.to_csv(OUTPUT_DIR / "run48_surrogate_validation_results_detailed.csv", index=False)
    best_cfg.to_csv(OUTPUT_DIR / "run48_best_surrogate_configurations.csv", index=False)
    write_json(OUTPUT_DIR / "run48_surrogate_validation_summary.json", surrogate_summary)

    gnn_results, gnn_summary = run_gnn_validation(native, plus)
    gnn_results.to_csv(OUTPUT_DIR / "run48_gnn_reward_validation_results.csv", index=False)
    write_json(OUTPUT_DIR / "run48_gnn_reward_validation_summary.json", gnn_summary)

    pointer_log, pointer_summary = pointer_policy(native, plus)
    pointer_log.to_csv(OUTPUT_DIR / "run48_graph_pointer_policy_training_log.csv", index=False)
    write_json(OUTPUT_DIR / "run48_graph_pointer_policy_validation_summary.json", pointer_summary)

    pool = generate_candidates(native, best_model, best_features)
    pool.to_csv(OUTPUT_DIR / "run48_candidate_pool_scored.csv", index=False)

    option_a = select_batch(pool, OPTION_A_COUNTS, "constrained_N24_N40_batch32")
    option_b = select_batch(pool, OPTION_B_COUNTS, "constrained_N24_N40_batch60")
    option_c = select_batch(pool, OPTION_C_COUNTS, "native_recovery_batch40_with_anchors")
    option_a.to_csv(OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch32_candidate_orders.csv", index=False)
    option_b.to_csv(OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch60_candidate_orders.csv", index=False)
    option_c.to_csv(OUTPUT_DIR / "run48_native_recovery_batch40_with_anchors_candidate_orders.csv", index=False)

    comparison, comparison_summary = compare_batches({"A_batch32": option_a, "B_batch60": option_b, "C_batch40": option_c}, native)
    comparison.to_csv(OUTPUT_DIR / "run48_batch_options_comparison_to_previous.csv", index=False)
    write_json(OUTPUT_DIR / "run48_batch_options_comparison_summary.json", comparison_summary)

    write_claim_boundary()
    option_counts = {"A": n_counts(option_a), "B": n_counts(option_b), "C": n_counts(option_c)}
    summary = {
        "target_audit_headline": "Stricter constrained targets were created to shift away from pure U2-near exploitation toward U2-primary reward balance with explicit PEEQ, SurfaceT, and Mises guards.",
    ...
    output_files = [
        OUTPUT_DIR / "run48_input_validation_summary.json",
        OUTPUT_DIR / "combined296_scan_order_features.csv",
        OUTPUT_DIR / "combined296_plus_N32_scan_order_features.csv",
        OUTPUT_DIR / "run48_target_reward_definition_audit.csv",
        OUTPUT_DIR / "run48_target_reward_definition_audit.json",
        OUTPUT_DIR / "run48_surrogate_validation_results_detailed.csv",
        OUTPUT_DIR / "run48_best_surrogate_configurations.csv",
        OUTPUT_DIR / "run48_surrogate_validation_summary.json",
        OUTPUT_DIR / "run48_gnn_reward_validation_results.csv",
        OUTPUT_DIR / "run48_gnn_reward_validation_summary.json",
        OUTPUT_DIR / "run48_graph_pointer_policy_training_log.csv",
        OUTPUT_DIR / "run48_graph_pointer_policy_validation_summary.json",
        OUTPUT_DIR / "run48_candidate_pool_scored.csv",
        OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch32_candidate_orders.csv",
        OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch60_candidate_orders.csv",
        OUTPUT_DIR / "run48_native_recovery_batch40_with_anchors_candidate_orders.csv",
        OUTPUT_DIR / "run48_batch_options_comparison_to_previous.csv",
        OUTPUT_DIR / "run48_batch_options_comparison_summary.json",
        CLAIM_BOUNDARY_MD,
        CLAIM_BOUNDARY_JSON,
        REPORT_PATH,
        MANIFEST_PATH,
    ...
    manifest = {
        ...
        "input_files": [str(COMBINED296_READY), str(COMBINED296_PLUS_N32_READY), str(RUN46_ENRICHED), str(RUN46_PRED_SUMMARY), str(RUN47_REPORT), str(RUN47_MANIFEST)],
        ...
        "candidate_pool_counts": n_counts(pool),
        "batch_option_paths": {
            "option_A_batch32": str(OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch32_candidate_orders.csv"),
            "option_B_batch60": str(OUTPUT_DIR / "run48_stricter_constrained_N24_N40_batch60_candidate_orders.csv"),
            "option_C_batch40": str(OUTPUT_DIR / "run48_native_recovery_batch40_with_anchors_candidate_orders.csv"),
        },
