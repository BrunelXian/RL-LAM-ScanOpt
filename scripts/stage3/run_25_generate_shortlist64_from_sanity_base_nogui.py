"""Generate Stage 3 run25 shortlist64 CAE/INP artifacts from sanity bases.

Run with:
abaqus cae noGUI="E:\Projects\RL-LAM-ScanOpt\scripts\stage3\run_25_generate_shortlist64_from_sanity_base_nogui.py"
...
RUN_ID = "run_25_shortlist64_active_learning_cae_inp_generation"
...
PLAN_CSV = OUTPUT_DIR / "run25_generation_plan.csv"
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run25_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run25_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run25_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run25_shortlist64_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run25_shortlist64_cae_generation_manifest.json"

EXPECTED_COUNTS = {12: 8, 16: 8, 24: 24, 40: 24}
BASES = {
...
class Run25GenerationError(RuntimeError):
    pass


def validate_scan_order(n: int, order: object) -> None:
    if not isinstance(order, list):
        raise Run25GenerationError("scan_order is not a list")
    if len(order) != n:
        raise Run25GenerationError("scan_order length {} != N {}".format(len(order), n))
    if not all(isinstance(v, int) for v in order):
        raise Run25GenerationError("scan_order contains non-integer entries")
    if sorted(order) != list(range(n)):
        raise Run25GenerationError("scan_order is not a permutation of 0..{}".format(n - 1))


def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise Run25GenerationError("missing run25 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "shortlist64":
                raise Run25GenerationError("non-shortlist64 row selected: {}".format(row.get("handoff_strategy_name")))
            if "batch24" in json.dumps(row).lower() or "batch32" in json.dumps(row).lower():
                raise Run25GenerationError("forbidden batch24/batch32 text selected")
            rows.append(dict(row, n=n, scan_order=scan_order))
    if len(rows) != 64:
        raise Run25GenerationError("generation plan row count {} != 64".format(len(rows)))
    counts = {n: len([row for row in rows if int(row["n"]) == n]) for n in EXPECTED_COUNTS}
    if counts != EXPECTED_COUNTS:
        raise Run25GenerationError("per-N generation counts {} != {}".format(counts, EXPECTED_COUNTS))
    return rows


def validate_base_audit(rows: list[dict[str, object]]) -> None:
    if not BASE_AUDIT_SUMMARY.exists():
        raise Run25GenerationError("missing base mesh audit summary: {}".format(BASE_AUDIT_SUMMARY))
    ...
    for n in sorted({int(row["n"]) for row in rows}):
        ...
        if float(row.get("total_node_count", 0)) <= 0 or float(row.get("total_element_count", 0)) <= 0:
            failed.append(n)
    if failed:
        raise Run25GenerationError("base mesh audit not passing for N values: {}".format(failed))
...
def validate_paths(rows: list[dict[str, object]]) -> None:
    for row in rows:
        ...
        if any(token in path for token in BAD_SCHEMA_TOKENS for path in paths):
            raise Run25GenerationError("bad path schema token in {}".format(row["handoff_strategy_name"]))
        if Path(str(row["case_dir"])).parent != CASE_ROOT:
            raise Run25GenerationError("case_dir not under run25 case root: {}".format(row["case_dir"]))
        for key in ("cae_path", "inp_path", "jnl_path", "scan_order_json", "generation_log_path"):
            if Path(str(row[key])).parent != Path(str(row["case_dir"])):
                raise Run25GenerationError("{} is not inside case_dir".format(key))
        existing = [path for path in (Path(str(row["cae_path"])), Path(str(row["inp_path"])), Path(str(row["jnl_path"]))) if path.exists()]
        if existing:
            raise Run25GenerationError("generated CAE/INP/JNL already exists for {}: {}".format(row["handoff_strategy_name"], existing))
...
def validate_no_solver_outputs(case_dir: Path) -> None:
    ...
    if found:
        raise Run25GenerationError("solver output files unexpectedly exist: {}".format(found))
...
def validate_heat_sets_exist_in_model(model, n: int) -> dict[int, object]:  # noqa: ANN001
    ...
    for track in range(n):
        ...
        if track not in found:
            raise Run25GenerationError("missing heat set {}".format(name))
    ...
    if len(set(all_heat_names)) != n:
        raise Run25GenerationError("expected exactly {} heat sets, found {}".format(n, sorted(set(all_heat_names))))
    return found
...
def infer_templates(model) -> dict[str, object]:  # noqa: ANN001
    ...
    if missing:
        raise Run25GenerationError("missing template objects: {}".format(missing))
    load = model.loads[MAPPING["template_load"]]
    if load.__class__.__name__ != MAPPING["load_type"]:
        raise Run25GenerationError("template load type {} != {}".format(load.__class__.__name__, MAPPING["load_type"]))
...
def create_step_from_template(model, template_step, name: str, previous: str, time_period: float, initial_inc=None, max_inc=None) -> None:  # noqa: ANN001,E501
    ...
    if step_type == "CoupledTempDisplacementStep":
        ...
        raise Run25GenerationError("unsupported step template type {}".format(step_type))
...
def set_existing_step_time(model, step_name: str, duration: float) -> None:  # noqa: ANN001
    try:
        ...
        raise Run25GenerationError("failed setting {} duration: {}".format(step_name, exc))
    ...
    for seq, track in enumerate(scan_order):
        ...
        if seq == 0:
            if scan not in model.steps or cool not in model.steps:
                raise Run25GenerationError("template scan/cool step missing for seq0")
...
def create_heat_load(model, seq: int, track: int, heat_regions: dict[int, object]):  # noqa: ANN001
    ...
    if seq == 0:
        ...
        try:
            ...
            if track != 0:
                raise Run25GenerationError("failed to retarget template load to track {}: {}".format(track, exc))
...
def append_final_cooling(model, records: list[dict[str, object]]) -> dict[str, object]:  # noqa: ANN001
    ...
    if final in model.steps:
        raise Run25GenerationError("final cooling step already exists")
    ...
    if checks["final_cooling_timePeriod"] != 1200.0:
        raise Run25GenerationError("final cooling timePeriod verification failed")
    if checks["final_cooling_initialInc"] != 0.01:
        raise Run25GenerationError("final cooling initialInc verification failed")
    if checks["final_cooling_maxInc"] != 60.0:
        raise Run25GenerationError("final cooling maxInc verification failed")
    return checks
...
def validate_model_before_write(model, n: int, scan_order: list[int], records: list[dict[str, object]], deactivations: list[dict[str, str]], final_record: dict[str, object]) -> None:  # noqa: ANN001,E501
    ...
    if len(records) != n or len(deactivations) != n:
        raise Run25GenerationError("sequence/deactivation count != N")
    for seq in range(n):
        if MAPPING["scan_step_pattern"].format(seq=seq) not in model.steps:
            raise Run25GenerationError("missing scan step {}".format(seq))
        if MAPPING["cool_step_pattern"].format(seq=seq) not in model.steps:
            raise Run25GenerationError("missing cool step {}".format(seq))
        if MAPPING["load_pattern"].format(seq=seq) not in model.loads:
            raise Run25GenerationError("missing heat load {}".format(seq))
    if final_record["final_cooling_previous"] != records[-1]["cool_step"]:
        raise Run25GenerationError("final cooling is not after last cool step")
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run25_case_metadata.json"
    metadata_keys = [
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "Run25 shortlist64 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN25_SHORTLIST64_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN25_SHORTLIST64_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN25_SHORTLIST64_GENERATION")
        print(str(exc))
