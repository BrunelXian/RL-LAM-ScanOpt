...
RUN_ID = "run_66_variable_N_recovery_anchor_batch48_odb_teacher_validation"
BATCH_NAME = "stage3_run64_variable_N_recovery_anchor_batch48_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN65_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_65_variable_N_recovery_anchor_batch48_cae_inp_generation"
    / "stage3_run65_variable_N_recovery_anchor_batch48_cae_generation_manifest.csv"
)
RUN65_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_65_variable_N_recovery_anchor_batch48_cae_inp_generation"
    / "RUN_65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_66_variable_N_recovery_anchor_batch48_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_66_variable_N_recovery_anchor_batch48_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_66_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_solver_completion_audit.json"
AUDIT_MD = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_teacher_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "run66_variable_N_recovery_anchor_batch48_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_ODB_TEACHER_VALIDATION_REPORT.md"
...
EXPECTED_BY_N = {12: 12, 16: 12, 24: 8, 40: 16}
TOTAL_EXPECTED = 48
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run65": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN65_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run65 variable-N recovery anchor batch48 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run65 variable-N recovery anchor batch48 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_SOLVER_COMPLETION_48_OF_48"
    return {
        ...
        "run65_report": str(RUN65_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_ODB_TEACHER_VALIDATION_48_OF_48" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN66_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Run66 Variable-N Recovery Anchor Batch48 Solver Completion Audit",
        "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run65_manifest_csv": str(RUN65_MANIFEST_CSV),
                    "run65_report_md": str(RUN65_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
