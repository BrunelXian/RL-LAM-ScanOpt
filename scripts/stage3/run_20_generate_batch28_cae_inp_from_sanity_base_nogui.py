"""Generate Stage 3 run20 batch28 CAE/INP artifacts from sanity bases.

Run with:
abaqus cae noGUI="E:\Projects\RL-LAM-ScanOpt\scripts\stage3\run_20_generate_batch28_cae_inp_from_sanity_base_nogui.py"

This script writes model-generation artifacts only. It never submits jobs,
...
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run20_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run20_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run20_generation_summary.json"
...
class Run20GenerationError(RuntimeError):
    pass


def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise Run20GenerationError("missing run20 generation plan: {}".format(PLAN_CSV))
    ...
    if len(rows) != 28:
        raise Run20GenerationError("generation plan row count {} != 28".format(len(rows)))
    return rows


def validate_scan_order(n: int, order: object) -> None:
    if not isinstance(order, list):
        raise Run20GenerationError("scan_order is not a list")
    if len(order) != n:
        raise Run20GenerationError("scan_order length {} != N {}".format(len(order), n))
    if not all(isinstance(v, int) for v in order):
        raise Run20GenerationError("scan_order contains non-integer entries")
    if sorted(order) != list(range(n)):
        raise Run20GenerationError("scan_order is not a permutation of 0..{}".format(n - 1))


def validate_base_audit(rows: list[dict[str, object]]) -> None:
    if not BASE_AUDIT_SUMMARY.exists():
        raise Run20GenerationError("missing base mesh audit summary: {}".format(BASE_AUDIT_SUMMARY))
    summary = json.loads(BASE_AUDIT_SUMMARY.read_text(encoding="utf-8"))
    by_n = {int(row["n"]): row for row in summary.get("rows", [])}
    needed = sorted({int(row["n"]) for row in rows})
    failed = []
    for n in needed:
        row = by_n.get(n)
        ...
        if float(row.get("total_node_count", 0)) <= 0 or float(row.get("total_element_count", 0)) <= 0:
            failed.append(n)
    if failed:
        raise Run20GenerationError("base mesh audit not passing for N values: {}".format(failed))
...
def validate_paths(rows: list[dict[str, object]]) -> None:
    for row in rows:
        ...
        if any(token in path for token in BAD_SCHEMA_TOKENS for path in paths):
            raise Run20GenerationError("bad path schema token in {}".format(row["handoff_strategy_name"]))
        if Path(str(row["case_dir"])).parent != CASE_ROOT:
            raise Run20GenerationError("case_dir not under run20 case root: {}".format(row["case_dir"]))
        for key in ("cae_path", "inp_path", "jnl_path", "scan_order_json", "generation_log_path"):
            if Path(str(row[key])).parent != Path(str(row["case_dir"])):
                raise Run20GenerationError("{} is not inside case_dir".format(key))
        existing_model_files = [
            path
            for path in (Path(str(row["cae_path"])), Path(str(row["inp_path"])), Path(str(row["jnl_path"])))
            if path.exists()
        ]
        if existing_model_files:
            raise Run20GenerationError(
                "generated CAE/INP/JNL already exists for {}: {}".format(
                    row["handoff_strategy_name"], existing_model_files
                )
            )
...
def validate_no_solver_outputs(case_dir: Path) -> None:
    ...
    if found:
        raise Run20GenerationError("solver output files unexpectedly exist: {}".format(found))
...
def validate_heat_sets_exist_in_model(model, n: int) -> dict[int, object]:  # noqa: ANN001
    ...
    for track in range(n):
        ...
        if track not in found:
            raise Run20GenerationError("missing heat set {}".format(name))
    all_heat_names = [
        name for name in list(root.sets.keys()) if re.fullmatch(r"set_body_heat_\d+", name)
    ]
    for instance in root.instances.values():
        all_heat_names.extend(
            name for name in list(instance.sets.keys()) if re.fullmatch(r"set_body_heat_\d+", name)
        )
    if len(set(all_heat_names)) != n:
        raise Run20GenerationError(
            "expected exactly {} heat sets, found {}".format(n, sorted(set(all_heat_names)))
        )
    return found
...
def infer_templates(model) -> dict[str, object]:  # noqa: ANN001
    missing = []
    if MAPPING["template_scan_step"] not in model.steps:
        missing.append(MAPPING["template_scan_step"])
    if MAPPING["template_cool_step"] not in model.steps:
        missing.append(MAPPING["template_cool_step"])
    if MAPPING["template_load"] not in model.loads:
        missing.append(MAPPING["template_load"])
    if missing:
        raise Run20GenerationError("missing template objects: {}".format(missing))
    load = model.loads[MAPPING["template_load"]]
    if load.__class__.__name__ != MAPPING["load_type"]:
        raise Run20GenerationError(
            "template load type {} != {}".format(load.__class__.__name__, MAPPING["load_type"])
        )
    return {
        "scan_step": model.steps[MAPPING["template_scan_step"]],
        "cool_step": model.steps[MAPPING["template_cool_step"]],
        "load": load,
    }
...
def create_step_from_template(model, template_step, name: str, previous: str, time_period: float, initial_inc=None, max_inc=None) -> None:  # noqa: ANN001,E501
    ...
    if step_type == "CoupledTempDisplacementStep":
        ...
        raise Run20GenerationError("unsupported step template type {}".format(step_type))
...
def set_existing_step_time(model, step_name: str, duration: float) -> None:  # noqa: ANN001
    try:
        ...
        raise Run20GenerationError("failed setting {} duration: {}".format(step_name, exc))
    ...
    for seq, track in enumerate(scan_order):
        ...
        if seq == 0:
            if scan not in model.steps or cool not in model.steps:
                raise Run20GenerationError("template scan/cool step missing for seq0")
            ...
            create_step_from_template(
                ...
                scan,
                previous,
                float(MAPPING["scan_duration_seconds"]),
            )
            create_step_from_template(
                model,
                templates["cool_step"],
...
def create_heat_load(model, seq: int, track: int, heat_regions: dict[int, object]):  # noqa: ANN001
    ...
    if seq == 0:
        ...
        try:
            ...
            if track != 0:
                raise Run20GenerationError(
                    "failed to retarget template load to track {}: {}".format(track, exc)
                )
...
def append_final_cooling(model, records: list[dict[str, object]]) -> dict[str, object]:  # noqa: ANN001
    ...
    if final in model.steps:
        raise Run20GenerationError("final cooling step already exists")
    ...
    if checks["final_cooling_timePeriod"] != 1200.0:
        raise Run20GenerationError("final cooling timePeriod verification failed")
    if checks["final_cooling_initialInc"] != 0.01:
        raise Run20GenerationError("final cooling initialInc verification failed")
    if checks["final_cooling_maxInc"] != 60.0:
        raise Run20GenerationError("final cooling maxInc verification failed")
    return checks
...
def validate_model_before_write(model, n: int, scan_order: list[int], records: list[dict[str, object]], deactivations: list[dict[str, str]], final_record: dict[str, object]) -> None:  # noqa: ANN001,E501
    validate_scan_order(n, scan_order)
    if len(records) != n:
        raise Run20GenerationError("sequence record count != N")
    if len(deactivations) != n:
        raise Run20GenerationError("deactivation record count != N")
    for seq in range(n):
        if MAPPING["scan_step_pattern"].format(seq=seq) not in model.steps:
            raise Run20GenerationError("missing scan step {}".format(seq))
        if MAPPING["cool_step_pattern"].format(seq=seq) not in model.steps:
            raise Run20GenerationError("missing cool step {}".format(seq))
        if MAPPING["load_pattern"].format(seq=seq) not in model.loads:
            raise Run20GenerationError("missing heat load {}".format(seq))
    if final_record["final_cooling_previous"] != records[-1]["cool_step"]:
        raise Run20GenerationError("final cooling is not after last cool step")
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run20_case_metadata.json"
    metadata = {
        "run_id": RUN_ID,
        ...
        "predicted_reward_combined80_u2_primary": row[
            "predicted_reward_combined80_u2_primary"
        ],
        "predicted_rank_within_n": row["predicted_rank_within_n"],
        "predicted_uncertainty_std": row["predicted_uncertainty_std"],
        "novelty_distance_to_combined80": row["novelty_distance_to_combined80"],
    ...
    if not Path(str(row["inp_path"])).exists() or Path(str(row["inp_path"])).stat().st_size <= 0:
        raise Run20GenerationError("INP was not written: {}".format(row["inp_path"]))
    Path(str(row["jnl_path"])).write_text(
        "Run20 generation journal placeholder for {}.\n"
        "CAE/INP generated only; no solver, datacheck, abqjobpilot, enqueue, ODB, or teacher validation run.\n".format(
            row["handoff_strategy_name"]
        ),
        encoding="utf-8",
    )


def write_generation_log(row: dict[str, object], records: list[dict[str, object]], deactivations: list[dict[str, str]], final_record: dict[str, object]) -> None:  # noqa: E501
    payload = {
        "status": "GENERATED",
        "run_id": RUN_ID,
        "batch_name": BATCH_NAME,
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN20_BATCH28_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN20_BATCH28_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN20_BATCH20_GENERATION")
        print(str(exc))
