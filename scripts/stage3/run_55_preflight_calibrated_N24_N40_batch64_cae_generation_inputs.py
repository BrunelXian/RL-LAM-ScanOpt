...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_54_run53_calibrated_N24_N40_batch64_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run54_calibrated_N24_N40_batch64_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run55_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run55_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run55_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run55_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN55_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN55_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "calibrated_N24_N40_batch64":
            errors.append("row {} is not calibrated_N24_N40_batch64: {}".format(idx, row.get("batch_option")))
        ...
        if "batch32" in lowered or "recovery" in lowered:
            errors.append("row {} references forbidden reference batch32/recovery text".format(idx))
    ...
    verdict = "PASS_RUN55_CALIBRATED_N24_N40_BATCH64_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN55_CALIBRATED_N24_N40_BATCH64_INPUT_INVALID"
    summary = {
        "verdict": verdict,
