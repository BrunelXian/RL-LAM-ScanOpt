...
RUN_ID = "run_36_N32_informed_native_batch32_odb_teacher_validation"
BATCH_NAME = "stage3_run34_N32_informed_native_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN35_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_35_N32_informed_native_batch32_cae_inp_generation"
    / "stage3_run35_N32_informed_native_batch32_cae_generation_manifest.csv"
)
RUN35_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_35_N32_informed_native_batch32_cae_inp_generation"
    / "RUN_35_N32_INFORMED_NATIVE_BATCH32_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_36_N32_informed_native_batch32_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_36_N32_informed_native_batch32_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_36_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run36_N32_informed_native_batch32_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run36_N32_informed_native_batch32_solver_completion_audit.json"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run36_N32_informed_native_batch32_odb_extraction_summary.csv"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run36_N32_informed_native_batch32_teacher_metrics.csv"
SUMMARY_JSON = OUTPUT_DIR / "run36_N32_informed_native_batch32_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_36_N32_INFORMED_NATIVE_BATCH32_ODB_TEACHER_VALIDATION_REPORT.md"
...
EXPECTED_BY_N = {12: 4, 16: 4, 24: 12, 40: 12}
TOTAL_EXPECTED = 32
SPECIAL_GENERATION_NOTES = {
    "includes_N32_cases": False,
    "supersedes_old_Run31": True,
    "n40_cool_initialInc": 0.001,
}
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN35_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run35 N32-informed native batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run35 N32-informed native batch32 name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN36_N32_INFORMED_NATIVE_BATCH32_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN36_N32_INFORMED_NATIVE_BATCH32_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN36_N32_INFORMED_NATIVE_BATCH32_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "run35_report": str(RUN35_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN36_N32_INFORMED_NATIVE_BATCH32_ODB_TEACHER_VALIDATION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN36_N32_INFORMED_NATIVE_BATCH32_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run35_manifest_csv": str(RUN35_MANIFEST_CSV),
                    "run35_report_md": str(RUN35_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
