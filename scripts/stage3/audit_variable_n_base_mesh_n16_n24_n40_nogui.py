"""Abaqus noGUI mesh audit for corrected N16/N24/N40 sanity bases.

This script opens the base CAE files read-only in practice: it inspects the
models and writes audit CSV/JSON reports, but never saves the base CAEs.
"""
...
CSV_PATH = OUTPUT_DIR / "base_mesh_audit_N16_N24_N40.csv"
SUMMARY_PATH = OUTPUT_DIR / "base_mesh_audit_N16_N24_N40_summary.json"
    ...
    try:
        ...
        for track in range(n):
            ...
            if _heat_set_exists(model, name):
                ...
                missing_heat_sets.append(name)
        row["missing_heat_sets"] = ";".join(missing_heat_sets)
        ...
        row["verdict"] = "PASS_BASE_MESH_READY" if required_ok else "FAIL_BASE_MESH_MISSING"
        return row
    except Exception:
...
def main() -> int:
    ...
    summary = {
        ...
        "overall_verdict": (
            "PASS_BASE_MESH_READY"
            if all(row["verdict"] == "PASS_BASE_MESH_READY" for row in rows)
            else "FAIL_BASE_MESH_MISSING"
        ),
    }
    SUMMARY_PATH.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(summary["overall_verdict"])
    print("summary={}".format(SUMMARY_PATH))
    return 0 if summary["overall_verdict"] == "PASS_BASE_MESH_READY" else 1
