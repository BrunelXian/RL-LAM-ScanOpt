...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_64_run63_variable_N_recovery_anchor_batch48_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run64_variable_N_recovery_anchor_batch48_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run65_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run65_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run65_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run65_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN65_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN65_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    if len(rows) != 48:
        errors.append("row count {} != 48".format(len(rows)))
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "variable_N_recovery_anchor_batch48":
            errors.append("row {} is not variable_N_recovery_anchor_batch48: {}".format(idx, row.get("batch_option")))
        ...
        if "small-n recovery batch32" in lowered or "optional n40 follow-up batch32" in lowered:
            errors.append("row {} references forbidden Run63 Option B/C text".format(idx))
    ...
    verdict = "PASS_RUN65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_INPUT_INVALID"
    summary = {
        "verdict": verdict,
