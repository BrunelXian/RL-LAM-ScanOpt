"""Generate Stage 3 run14 batch20 CAE/INP artifacts from sanity bases.
...
PLAN_CSV = OUTPUT_DIR / "run14_generation_plan.csv"
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run14_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run14_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run14_generation_summary.json"
...
class Run14GenerationError(RuntimeError):
    pass


def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise Run14GenerationError("missing run14 generation plan: {}".format(PLAN_CSV))
    ...
    if len(rows) != 20:
        raise Run14GenerationError("generation plan row count {} != 20".format(len(rows)))
    return rows


def validate_scan_order(n: int, order: object) -> None:
    if not isinstance(order, list):
        raise Run14GenerationError("scan_order is not a list")
    if len(order) != n:
        raise Run14GenerationError("scan_order length {} != N {}".format(len(order), n))
    if not all(isinstance(v, int) for v in order):
        raise Run14GenerationError("scan_order contains non-integer entries")
    if sorted(order) != list(range(n)):
        raise Run14GenerationError("scan_order is not a permutation of 0..{}".format(n - 1))


def validate_base_audit(rows: list[dict[str, object]]) -> None:
    if not BASE_AUDIT_SUMMARY.exists():
        raise Run14GenerationError("missing base mesh audit summary: {}".format(BASE_AUDIT_SUMMARY))
    ...
    for n in needed:
        ...
        if float(row.get("total_node_count", 0)) <= 0 or float(row.get("total_element_count", 0)) <= 0:
            failed.append(n)
    if failed:
        raise Run14GenerationError("base mesh audit not passing for N values: {}".format(failed))
...
def validate_paths(rows: list[dict[str, object]]) -> None:
    for row in rows:
        ...
        if any(token in path for token in BAD_SCHEMA_TOKENS for path in paths):
            raise Run14GenerationError("bad path schema token in {}".format(row["handoff_strategy_name"]))
        if Path(str(row["case_dir"])).parent != CASE_ROOT:
            raise Run14GenerationError("case_dir not under run14 case root: {}".format(row["case_dir"]))
        for key in ("cae_path", "inp_path", "jnl_path", "scan_order_json", "generation_log_path"):
            if Path(str(row[key])).parent != Path(str(row["case_dir"])):
                raise Run14GenerationError("{} is not inside case_dir".format(key))
...
def validate_no_solver_outputs(case_dir: Path) -> None:
    ...
    if found:
        raise Run14GenerationError("solver output files unexpectedly exist: {}".format(found))
...
def validate_heat_sets_exist_in_model(model, n: int) -> dict[int, object]:  # noqa: ANN001
    ...
    for track in range(n):
        ...
        if track not in found:
            raise Run14GenerationError("missing heat set {}".format(name))
    all_heat_names = [
...
def infer_templates(model) -> dict[str, object]:  # noqa: ANN001
    ...
    if missing:
        raise Run14GenerationError("missing template objects: {}".format(missing))
...
def create_step_from_template(model, template_step, name: str, previous: str, time_period: float, initial_inc=None, max_inc=None) -> None:  # noqa: ANN001,E501
    ...
    if step_type == "CoupledTempDisplacementStep":
        ...
        raise Run14GenerationError("unsupported step template type {}".format(step_type))
...
def set_existing_step_time(model, step_name: str, duration: float) -> None:  # noqa: ANN001
    try:
        ...
        raise Run14GenerationError("failed setting {} duration: {}".format(step_name, exc))
    ...
    for seq, track in enumerate(scan_order):
        ...
        if seq == 0:
            if scan not in model.steps or cool not in model.steps:
                raise Run14GenerationError("template scan/cool step missing for seq0")
...
def append_final_cooling(model, records: list[dict[str, object]]) -> dict[str, object]:  # noqa: ANN001
    ...
    if final in model.steps:
        raise Run14GenerationError("final cooling step already exists")
    ...
    if checks["final_cooling_timePeriod"] != 1200.0:
        raise Run14GenerationError("final cooling timePeriod verification failed")
    if checks["final_cooling_initialInc"] != 0.01:
        raise Run14GenerationError("final cooling initialInc verification failed")
    if checks["final_cooling_maxInc"] != 60.0:
        raise Run14GenerationError("final cooling maxInc verification failed")
    return checks
...
def validate_model_before_write(model, n: int, scan_order: list[int], records: list[dict[str, object]], deactivations: list[dict[str, str]], final_record: dict[str, object]) -> None:  # noqa: ANN001,E501
    validate_scan_order(n, scan_order)
    if len(records) != n:
        raise Run14GenerationError("sequence record count != N")
    if len(deactivations) != n:
        raise Run14GenerationError("deactivation record count != N")
    for seq in range(n):
        if MAPPING["scan_step_pattern"].format(seq=seq) not in model.steps:
            raise Run14GenerationError("missing scan step {}".format(seq))
        if MAPPING["cool_step_pattern"].format(seq=seq) not in model.steps:
            raise Run14GenerationError("missing cool step {}".format(seq))
        if MAPPING["load_pattern"].format(seq=seq) not in model.loads:
            raise Run14GenerationError("missing heat load {}".format(seq))
    if final_record["final_cooling_previous"] != records[-1]["cool_step"]:
        raise Run14GenerationError("final cooling is not after last cool step")
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run14_case_metadata.json"
    metadata = {
        "run_id": RUN_ID,
        ...
        "predicted_reward_mean_all": row["predicted_reward_mean_all"],
    ...
    if not Path(str(row["inp_path"])).exists() or Path(str(row["inp_path"])).stat().st_size <= 0:
        raise Run14GenerationError("INP was not written: {}".format(row["inp_path"]))
    Path(str(row["jnl_path"])).write_text(
        "Run14 generation journal placeholder for {}.\n"
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN14_BATCH20_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN14_BATCH20_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN14_BATCH20_GENERATION")
        print(str(exc))
