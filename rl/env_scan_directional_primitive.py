"""Gymnasium environment for strict directional 2-cell scan primitives."""
...
class ScanPlanningDirectionalPrimitiveEnv(ScanPlanningEnv):
    """Directional 2-cell primitive variant with explicit anchor semantics.
    ...
    Unlike the earlier local primitive environment, this catalog preserves
    the explicit anchor-direction representation rather than deduplicating
    identical static masks. This keeps the action semantics maximally stable
    and easy to inspect for the final patch-family validation.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._rebuild_directional_actions()
    ...
    def _build_action_cells(
        ...
        """Return target-masked cells covered by one strict 2-cell primitive."""
        row, col = anchor
        candidates = [(row, col)]
    ...
    def _rebuild_directional_actions(self) -> None:
        """Recompute the explicit directional primitive catalog."""
        ...
        self.executed_directional_actions: list[int] = []
    ...
    def _remaining_cells_for_action(self, action: int) -> list[tuple[int, int]]:
        """Return currently executable cells for one directional primitive."""
        return [
            cell
    ...
    def reset(
        ...
        *,
        ...
        """Reset the environment and rebuild the directional primitive catalog."""
        observation, info = super().reset(seed=seed, options=options)
        self._rebuild_directional_actions()
        info.update(
            {
                "action_mode": "directional_primitive",
                "primitive_size": 2,
                "directional_action_count": len(self.action_cells),
    ...
    def is_valid_action(self, action: int) -> bool:
        """Return True only when the primitive can still affect target cells."""
    ...
    def action_masks(self) -> np.ndarray:
        """Return a flat mask over the directional primitive catalog."""
    ...
    def _base_info(
        ...
        *,
        ...
        """Return Stage A logging fields plus directional primitive metadata."""
        ...
        return {
            ...
            "primitive_size": 2,
            "directional_action_count": len(self.action_cells),
            "executed_directional_actions": len(self.executed_directional_actions),
        }

    def step(self, action: int) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        """Apply one directional primitive and return the Stage A reward."""
        self.steps_taken += 1
        ...
        if not is_valid:
            ...
            self.executed_directional_actions.append(action_index)
            for row, col in local_cells:
