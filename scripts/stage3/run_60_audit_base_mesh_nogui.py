...
CSV_PATH = OUTPUT_DIR / "run60_base_mesh_audit.csv"
SUMMARY_PATH = OUTPUT_DIR / "run60_base_mesh_audit_summary.json"
BASES = {
...
def main() -> int:
    ...
    overall = "PASS_RUN60_BASE_MESH_AUDIT_READY" if all(row["verdict"] == "PASS_BASE_MESH_READY" for row in rows) else "FAIL_RUN60_BASE_MESH_AUDIT_INVALID"
    summary = {"verdict": overall, "rows": rows, "csv_path": str(CSV_PATH), "summary_path": str(SUMMARY_PATH)}
    SUMMARY_PATH.write_text(json.dumps(summary, indent=2), encoding="utf-8")
    print(overall)
    print("summary={}".format(SUMMARY_PATH))
    return 0 if overall == "PASS_RUN60_BASE_MESH_AUDIT_READY" else 1
