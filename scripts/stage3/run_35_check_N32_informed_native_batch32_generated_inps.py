"""Check Stage 3 run35 N32_informed_native_batch32 generated INP files."""
...
CSV_PATH = OUTPUT_DIR / "stage3_run35_N32_informed_native_batch32_generated_inp_check.csv"
SUMMARY_PATH = OUTPUT_DIR / "stage3_run35_N32_informed_native_batch32_generated_inp_check_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / "stage3_run34_N32_informed_native_batch32_v01"
...
def main() -> int:
    ...
    n32_inp_count = len([path for path in CASE_ROOT.rglob("*.inp") if "\\N32" in str(path)]) if CASE_ROOT.exists() else 0
    solver_count = sum(len(list(CASE_ROOT.rglob("*{}".format(ext)))) for ext in SOLVER_EXTS) if CASE_ROOT.exists() else 0
    verdict = "PASS_RUN35_N32_INFORMED_NATIVE_BATCH32_32_INPS_READY_FOR_USER_REVIEW" if len(results) == 32 and all(row["verdict"] == "PASS" for row in results) and inp_count == 32 and cae_count == 32 and solver_count == 0 and n32_inp_count == 0 else "FAIL_RUN35_N32_INFORMED_NATIVE_BATCH32_INP_CHECK_INVALID"
    summary = {
        "verdict": verdict,
    ...
    return 0 if verdict == "PASS_RUN35_N32_INFORMED_NATIVE_BATCH32_32_INPS_READY_FOR_USER_REVIEW" else 1
