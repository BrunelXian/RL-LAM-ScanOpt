"""Check Stage 3 run14 batch20 generated INP files."""
...
CSV_PATH = OUTPUT_DIR / "run14_generated_inp_check.csv"
SUMMARY_PATH = OUTPUT_DIR / "run14_generated_inp_check_summary.json"
...
def main() -> int:
    ...
    verdict = (
        ...
        if len(results) == 20 and all(row["verdict"] == "PASS" for row in results)
        else "FAIL_RUN14_BATCH20_INP_CHECK_INVALID"
    )
    summary = {
        "verdict": verdict,
    ...
    return 0 if verdict == "PASS_RUN14_BATCH20_20_INPS_READY_FOR_USER_REVIEW" else 1
