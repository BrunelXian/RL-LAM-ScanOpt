...
CSV_PATH = OUTPUT_DIR / "stage3_run50_stricter_constrained_N24_N40_batch32_generated_inp_check.csv"
SUMMARY_PATH = OUTPUT_DIR / "stage3_run50_stricter_constrained_N24_N40_batch32_generated_inp_check_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / "stage3_run49_stricter_constrained_N24_N40_batch32_v01"
...
def main() -> int:
    ...
    if CASE_ROOT.exists():
        forbidden_inp_count = len([
            ...
            if any(part.startswith(prefix) for part in path.parts for prefix in ("N12", "N16", "N32"))
        ])
    solver_count = sum(len(list(CASE_ROOT.rglob("*{}".format(ext)))) for ext in SOLVER_EXTS) if CASE_ROOT.exists() else 0
    verdict = "PASS_RUN50_STRICTER_CONSTRAINED_N24_N40_BATCH32_32_INPS_READY_FOR_USER_REVIEW" if len(results) == 32 and all(row["verdict"] == "PASS" for row in results) and inp_count == 32 and cae_count == 32 and solver_count == 0 and forbidden_inp_count == 0 else "FAIL_RUN50_STRICTER_CONSTRAINED_N24_N40_BATCH32_INP_CHECK_INVALID"
    summary = {
        "verdict": verdict,
        ...
        "forbidden_N12_N16_N32_inp_count": forbidden_inp_count,
    ...
    return 0 if verdict == "PASS_RUN50_STRICTER_CONSTRAINED_N24_N40_BATCH32_32_INPS_READY_FOR_USER_REVIEW" else 1
