...
RUN_ID = "run_51_stricter_constrained_N24_N40_batch32_odb_teacher_validation"
BATCH_NAME = "stage3_run49_stricter_constrained_N24_N40_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN50_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_50_stricter_constrained_N24_N40_batch32_cae_inp_generation"
    / "stage3_run50_stricter_constrained_N24_N40_batch32_cae_generation_manifest.csv"
)
RUN50_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_50_stricter_constrained_N24_N40_batch32_cae_inp_generation"
    / "RUN_50_STRICTER_CONSTRAINED_N24_N40_BATCH32_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_51_stricter_constrained_N24_N40_batch32_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_51_stricter_constrained_N24_N40_batch32_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_51_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_solver_completion_audit.json"
AUDIT_MD = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_teacher_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "run51_stricter_constrained_N24_N40_batch32_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_51_STRICTER_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION_REPORT.md"
...
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run50": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN50_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run50 stricter constrained N24/N40 batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run50 stricter constrained N24/N40 batch32 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN51_STRICTER_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN51_STRICTER_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN51_STRICTER_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "run50_report": str(RUN50_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN51_STRICTER_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN51_STRICTER_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Run51 Stricter Constrained N24/N40 Batch32 Solver Completion Audit",
        "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run50_manifest_csv": str(RUN50_MANIFEST_CSV),
                    "run50_report_md": str(RUN50_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
