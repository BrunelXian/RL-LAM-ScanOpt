...
RUN_ID = "run_43_combined264_constrained_N24_N40_reward_balanced_candidate_generation"
RUN_NAME = "combined264 constrained N24/N40 reward-balanced candidate generation"
SCRIPT_PATH = ROOT / "scripts" / "stage3" / "run_43_combined264_constrained_N24_N40_reward_balanced_candidate_generation.py"
...
COMBINED264_PLUS_N32_READY = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "combined264_plus_N32_RL_ready_dataset.csv"
RUN41_ENRICHED = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "run41_native_N24_N40_focused_batch60_teacher_dataset_enriched.csv"
RUN41_COMPARISON = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "run41_vs_combined204_best_comparison.csv"
RUN41_EFFECTIVENESS = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "run41_N24_N40_focused_batch60_effectiveness_audit.csv"
RUN41_PRED_AUDIT = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "run41_prediction_audit_for_run38_batch60.csv"
RUN41_PRED_SUMMARY = ROOT / "outputs" / "stage3_run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "run41_prediction_audit_for_run38_batch60_summary.json"
RUN42_REPORT = ROOT / "docs" / "stage3" / "runs" / "run_42_native_N24_N40_focused_batch60_teacher_metrics_ingestion_and_combined264_ranking" / "RUN_42_NATIVE_N24_N40_FOCUSED_BATCH60_TEACHER_METRICS_INGESTION_AND_COMBINED264_RANKING_REPORT.md"
RUN42_MANIFEST = ROOT / "artifacts" / "manifests" / "stage3_run_42_manifest.json"
...
OUTPUT_DIR = ROOT / "outputs" / "stage3_run_43_combined264_constrained_N24_N40_reward_balanced_candidate_generation"
REPORT_DIR = ROOT / "docs" / "stage3" / "runs" / "run_43_combined264_constrained_N24_N40_reward_balanced_candidate_generation"
REPORT_PATH = REPORT_DIR / "RUN_43_COMBINED264_CONSTRAINED_N24_N40_REWARD_BALANCED_CANDIDATE_GENERATION_REPORT.md"
MANIFEST_PATH = ROOT / "artifacts" / "manifests" / "stage3_run_43_manifest.json"
CLAIM_BOUNDARY_MD = OUTPUT_DIR / "run43_claim_boundary.md"
CLAIM_BOUNDARY_JSON = OUTPUT_DIR / "run43_claim_boundary.json"

EXPECTED_NATIVE = {12: 36, 16: 36, 24: 96, 40: 96}
EXPECTED_PLUS = {12: 36, 16: 36, 24: 96, 32: 332, 40: 96}
POOL_TARGETS = {12: 500, 16: 500, 24: 4000, 40: 4000}
...
def add_features_and_targets(df: pd.DataFrame, native: bool) -> pd.DataFrame:
    ...
    if native:
        score_cols = [
            "target_u2_score_combined264_rank",
            "target_peeq_score_combined264_rank",
            "target_surfaceT_score_combined264_rank",
            "target_mises_score_combined264_rank",
        ]
        out["target_reward_combined264_u2_primary"] = as_float(out["target_reward_combined264_u2_primary"])
        prefix = "combined264"
    else:
        score_cols = [
            "target_u2_score_combined264_plus_N32_rank",
            "target_peeq_score_combined264_plus_N32_rank",
            "target_surfaceT_score_combined264_plus_N32_rank",
            "target_mises_score_combined264_plus_N32_rank",
        ]
        out["target_reward_combined264_plus_N32_mapped_u2_primary"] = as_float(out["target_reward_combined264_plus_N32_mapped_u2_primary"])
        prefix = "combined264_plus_N32"
...
def validate_inputs(native: pd.DataFrame, plus: pd.DataFrame, pred_summary: dict[str, Any]) -> dict[str, Any]:
    errors = []
    if len(native) != 264 or n_counts(native) != EXPECTED_NATIVE:
        errors.append(f"native combined264 count mismatch: rows={len(native)} counts={n_counts(native)}")
    if 32 in set(native["n"].astype(int)):
        errors.append("native combined264 contains N32")
    if len(plus) != 596 or n_counts(plus) != EXPECTED_PLUS:
        errors.append(f"combined264_plus_N32 count mismatch: rows={len(plus)} counts={n_counts(plus)}")
    ...
    return {
        "timestamp": now_iso(),
        "verdict": "PASS_RUN43_COMBINED264_AND_PLUS_N32_INPUTS_READY" if not errors else "FAIL_RUN43_INPUT_VALIDATION",
        "errors": errors,
        "native_combined264_rows": int(len(native)),
        "native_combined264_counts": n_counts(native),
        "combined264_plus_N32_rows": int(len(plus)),
        "combined264_plus_N32_counts": n_counts(plus),
        "run41_prediction_audit_context": {"overall_reward_spearman": reward_s, "mean_top5_overlap": top5},
    }
...
def make_model(name: str, seed: int) -> Any:
    ...
    if name == "RandomForestRegressor":
        return sk["RandomForestRegressor"](n_estimators=120, random_state=seed, min_samples_leaf=2, n_jobs=-1)
    if name == "ExtraTreesRegressor":
        return sk["ExtraTreesRegressor"](n_estimators=160, random_state=seed, min_samples_leaf=1, n_jobs=-1)
    if name == "GradientBoostingRegressor":
        return sk["GradientBoostingRegressor"](random_state=seed)
    if name == "OrderGraphMLPRegressor":
        return sk["make_pipeline"](sk["StandardScaler"](), sk["MLPRegressor"](hidden_layer_sizes=(32, 16), alpha=0.001, max_iter=800, random_state=seed))
    raise KeyError(name)
...
def validation_protocols(df: pd.DataFrame, regime: str) -> list[tuple[str, np.ndarray, np.ndarray]]:
    ...
    if "dataset_source" in df.columns and (df["dataset_source"].astype(str) == "run41_native_N24_N40_focused_batch60").any():
        test = df.index[df["dataset_source"].astype(str) == "run41_native_N24_N40_focused_batch60"].to_numpy()
        train = np.setdiff1d(rows, test)
        protocols.append(("train_pre_Run41_test_Run41", train, test))
...
def run_surrogate_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], Any, list[str]]:
    ...
    targets = {
        "native_combined264": [
            "target_reward_combined264_u2_primary",
            "target_reward_combined264_constrained_u2_reward_balanced",
            "target_reward_combined264_u2_guarded",
        ],
        "plus_N32_unweighted": [
            "target_reward_combined264_plus_N32_mapped_u2_primary",
            "target_reward_combined264_plus_N32_strict_u2_surfaceT",
        ],
        "plus_N32_balanced": [
            "target_reward_combined264_plus_N32_mapped_u2_primary",
            "target_reward_combined264_plus_N32_strict_u2_surfaceT",
    ...
    frames = {"native_combined264": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}
    rows = []
    ...
    native_best = grouped[grouped["regime"] == "native_combined264"].head(1).iloc[0].to_dict()
...
def run_gnn_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    rows = []
    for regime, df in {"native_combined264": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}.items():
        target = "target_reward_combined264_u2_primary" if regime == "native_combined264" else "target_reward_combined264_plus_N32_mapped_u2_primary"
        sw = sample_weights(df) if regime.endswith("balanced") else None
        for protocol, train, test in validation_protocols(df, regime):
            try:
...
def generate_candidates(native: pd.DataFrame, model: Any, features: list[str]) -> pd.DataFrame:
    ...
    for n in [12, 16, 24, 40]:
        ...
        best_by[(n, "reward")] = parse_order(g.sort_values("target_reward_combined264_u2_primary", ascending=False).iloc[0]["order_json"])
        best_by[(n, "constrained")] = parse_order(g.sort_values("target_reward_combined264_constrained_u2_reward_balanced", ascending=False).iloc[0]["order_json"])
    rows, seen = [], set(existing)
    source_plan = [
        "constrained_surrogate_top", "constrained_surrogate_local_search", "U2_guarded_local_search",
        "reward_balanced_local_search", "PEEQ_guarded_candidates", "SurfaceT_guarded_candidates",
        "graph_pointer_temperature_sample", "GNN_reward_local_search", "hybrid_agreement",
    ...
    for n, target_count in POOL_TARGETS.items():
        ...
        while sum(1 for r in rows if r["n"] == n) < target_count and attempts < target_count * 80:
            ...
            if source in {"U2_guarded_local_search", "PEEQ_guarded_candidates"}:
                base = best_by[(n, "u2")]
            elif source in {"reward_balanced_local_search", "SurfaceT_guarded_candidates", "constrained_surrogate_local_search"}:
            ...
            constrained = hybrid - 0.10 * max(0.0, 0.15 - near_d)
            cid = f"R43_N{n}_C{sum(1 for r in rows if r['n']==n)+1:05d}"
            rows.append({
                "candidate_id": cid,
...
def select_batch(pool: pd.DataFrame, counts: dict[int, int], label: str) -> pd.DataFrame:
    selected = []
    bucket_order = ["reward_balanced_local_search", "constrained_surrogate_top", "PEEQ_guarded_candidates", "SurfaceT_guarded_candidates", "U2_guarded_local_search", "hybrid_agreement", "hybrid_disagreement", "uncertainty_calibration", "diversity_coverage", "graph_pointer_temperature_sample", "GNN_reward_local_search", "sentinel_control"]
    for n, count in counts.items():
        ...
        for i, row in enumerate(take[:count], 1):
            short = str(row["candidate_source"]).replace("_local_search", "").replace("_candidates", "").replace("constrained_", "constr")[:28]
            row["batch_option"] = label
            row["handoff_strategy_name"] = f"S3R43CB_N{n}_B{i:02d}_{short}"
...
def compare_batches(options: dict[str, pd.DataFrame], native: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    previous = {"combined264_teacher_orders": set(native["order_hash"].astype(str))}
    for name, path in {"run41": RUN41_HANDOFF, "run36": RUN36_HANDOFF, "run27": RUN27_HANDOFF, "superseded_run31": RUN31_OLD}.items():
        if path.exists():
    ...
    for opt, df in options.items():
        ...
        row = {
            ...
            "mean_penalty_guard_score": float((df["predicted_peeq_guarded_score"] + df["predicted_surfaceT_guarded_score"]).mean() / 2.0),
    ...
    headline = "All Run43 options were checked for exact order overlap and constrained-score/novelty composition against combined264, Run41, Run36, Run27, and superseded Run31."
    return comp, {"headline": headline, "rows": comp.to_dict(orient="records")}


def write_claim_boundary() -> None:
    safe = [
        "Run43 updates offline models using native combined264 and combined264_plus_N32.",
        "Run43 evaluates whether constrained U2 + reward-balanced targets can improve N24/N40 candidate selection after Run41.",
        "Run43 generates candidate-order batches for future teacher validation.",
        "Run43 does not include teacher validation for new candidates.",
    ]
    unsafe = [
    ...
    CLAIM_BOUNDARY_MD.write_text("# Run43 Claim Boundary\n\n## Safe claims\n" + "\n".join(f"- {x}" for x in safe) + "\n\n## Unsafe claims\n" + "\n".join(f"- {x}" for x in unsafe) + "\n", encoding="utf-8")
    write_json(CLAIM_BOUNDARY_JSON, {"verdict": "RUN43_MODEL_UPDATE_AND_CANDIDATE_GENERATION_ONLY_NO_TEACHER_VALIDATION", "safe_claims": safe, "unsafe_claims": unsafe})


def write_report(summary: dict[str, Any]) -> None:
    REPORT_PATH.write_text(f"""# Stage 3 Run 43 - Combined264 Constrained N24/N40 Reward-Balanced Candidate Generation
...
## 2. Inputs
- Native combined264: `{COMBINED264_READY}`
- combined264_plus_N32: `{COMBINED264_PLUS_N32_READY}`
- Run41 prediction audit: `{RUN41_PRED_SUMMARY}`
...
Candidate pool counts: `{summary['candidate_pool_counts']}`. N24 and N40 each meet the >=4000 candidate minimum.

## 10. Option A Constrained N24/N40 Batch32
Path: `{OUTPUT_DIR / 'run43_constrained_N24_N40_batch32_candidate_orders.csv'}`. Counts: `{summary['option_counts']['A']}`.

## 11. Option B Constrained N24/N40 Batch60
Path: `{OUTPUT_DIR / 'run43_constrained_N24_N40_batch60_candidate_orders.csv'}`. Counts: `{summary['option_counts']['B']}`.

## 12. Option C Native Recovery Batch40 With Anchors
Path: `{OUTPUT_DIR / 'run43_native_recovery_batch40_with_anchors_candidate_orders.csv'}`. Counts: `{summary['option_counts']['C']}`.
...
## 15. Output Files
- Candidate pool: `{OUTPUT_DIR / 'run43_candidate_pool_scored.csv'}`
- Surrogate summary: `{OUTPUT_DIR / 'run43_surrogate_validation_summary.json'}`
- GNN summary: `{OUTPUT_DIR / 'run43_gnn_reward_validation_summary.json'}`
- Pointer summary: `{OUTPUT_DIR / 'run43_graph_pointer_policy_validation_summary.json'}`
- Batch comparison: `{OUTPUT_DIR / 'run43_batch_options_comparison_summary.json'}`
...
def main() -> None:
    ensure_dirs()
    native = add_features_and_targets(read_csv(COMBINED264_READY), native=True)
    plus = add_features_and_targets(read_csv(COMBINED264_PLUS_N32_READY), native=False)
    pred_summary = json.loads(RUN41_PRED_SUMMARY.read_text(encoding="utf-8")) if RUN41_PRED_SUMMARY.exists() else {}
    validation = validate_inputs(native, plus, pred_summary)
    write_json(OUTPUT_DIR / "run43_input_validation_summary.json", validation)
    if not validation["verdict"].startswith("PASS"):
        raise SystemExit(validation["errors"])

    native.to_csv(OUTPUT_DIR / "combined264_scan_order_features.csv", index=False)
    plus.to_csv(OUTPUT_DIR / "combined264_plus_N32_scan_order_features.csv", index=False)
    target_rows = [
        {"target": "target_reward_combined264_u2_primary", "definition": "0.65 U2 + 0.20 PEEQ + 0.10 SurfaceT + 0.05 Mises", "dataset": "native_combined264"},
        {"target": "target_reward_combined264_constrained_u2_reward_balanced", "definition": "0.50 U2 + 0.25 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined264"},
        {"target": "target_reward_combined264_u2_guarded", "definition": "U2-primary score with penalties for below-median and bottom-quartile PEEQ/SurfaceT/Mises scores within N", "dataset": "native_combined264"},
        {"target": "target_reward_combined264_plus_N32_mapped_u2_primary", "definition": "mapped U2-primary target with N32 semantic warnings preserved", "dataset": "combined264_plus_N32"},
        {"target": "target_reward_combined264_plus_N32_strict_u2_surfaceT", "definition": "strict U2 + SurfaceT target for safer N32 metric semantics", "dataset": "combined264_plus_N32"},
    ]
    pd.DataFrame(target_rows).to_csv(OUTPUT_DIR / "run43_target_reward_definition_audit.csv", index=False)
    write_json(OUTPUT_DIR / "run43_target_reward_definition_audit.json", {"headline": "Constrained targets were created to keep U2 primary while increasing PEEQ, SurfaceT, and Mises guard weight.", "targets": target_rows})

    detailed, best_cfg, surrogate_summary, best_model, best_features = run_surrogate_validation(native, plus)
    detailed.to_csv(OUTPUT_DIR / "run43_surrogate_validation_results_detailed.csv", index=False)
    best_cfg.to_csv(OUTPUT_DIR / "run43_best_surrogate_configurations.csv", index=False)
    write_json(OUTPUT_DIR / "run43_surrogate_validation_summary.json", surrogate_summary)

    gnn_results, gnn_summary = run_gnn_validation(native, plus)
    gnn_results.to_csv(OUTPUT_DIR / "run43_gnn_reward_validation_results.csv", index=False)
    write_json(OUTPUT_DIR / "run43_gnn_reward_validation_summary.json", gnn_summary)

    pointer_log, pointer_summary = pointer_policy(native, plus)
    pointer_log.to_csv(OUTPUT_DIR / "run43_graph_pointer_policy_training_log.csv", index=False)
    write_json(OUTPUT_DIR / "run43_graph_pointer_policy_validation_summary.json", pointer_summary)

    pool = generate_candidates(native, best_model, best_features)
    pool.to_csv(OUTPUT_DIR / "run43_candidate_pool_scored.csv", index=False)
    ...
    option_a.to_csv(OUTPUT_DIR / "run43_constrained_N24_N40_batch32_candidate_orders.csv", index=False)
    option_b.to_csv(OUTPUT_DIR / "run43_constrained_N24_N40_batch60_candidate_orders.csv", index=False)
    option_c.to_csv(OUTPUT_DIR / "run43_native_recovery_batch40_with_anchors_candidate_orders.csv", index=False)

    comparison, comparison_summary = compare_batches({"A_batch32": option_a, "B_batch60": option_b, "C_batch40": option_c}, native)
    comparison.to_csv(OUTPUT_DIR / "run43_batch_options_comparison_to_previous.csv", index=False)
    write_json(OUTPUT_DIR / "run43_batch_options_comparison_summary.json", comparison_summary)

    write_claim_boundary()
    option_counts = {"A": n_counts(option_a), "B": n_counts(option_b), "C": n_counts(option_c)}
    summary = {
        "target_audit_headline": "Constrained targets were created to shift away from pure U2-near exploitation toward U2-primary reward balance with explicit penalty guards.",
    ...
    output_files = [
        OUTPUT_DIR / "run43_input_validation_summary.json",
        OUTPUT_DIR / "combined264_scan_order_features.csv",
        OUTPUT_DIR / "combined264_plus_N32_scan_order_features.csv",
        OUTPUT_DIR / "run43_target_reward_definition_audit.csv",
        OUTPUT_DIR / "run43_target_reward_definition_audit.json",
        OUTPUT_DIR / "run43_surrogate_validation_results_detailed.csv",
        OUTPUT_DIR / "run43_best_surrogate_configurations.csv",
        OUTPUT_DIR / "run43_surrogate_validation_summary.json",
        OUTPUT_DIR / "run43_gnn_reward_validation_results.csv",
        OUTPUT_DIR / "run43_gnn_reward_validation_summary.json",
        OUTPUT_DIR / "run43_graph_pointer_policy_training_log.csv",
        OUTPUT_DIR / "run43_graph_pointer_policy_validation_summary.json",
        OUTPUT_DIR / "run43_candidate_pool_scored.csv",
        OUTPUT_DIR / "run43_constrained_N24_N40_batch32_candidate_orders.csv",
        OUTPUT_DIR / "run43_constrained_N24_N40_batch60_candidate_orders.csv",
        OUTPUT_DIR / "run43_native_recovery_batch40_with_anchors_candidate_orders.csv",
        OUTPUT_DIR / "run43_batch_options_comparison_to_previous.csv",
        OUTPUT_DIR / "run43_batch_options_comparison_summary.json",
        CLAIM_BOUNDARY_MD,
        CLAIM_BOUNDARY_JSON,
        REPORT_PATH,
        MANIFEST_PATH,
    ...
    manifest = {
        ...
        "input_files": [str(COMBINED264_READY), str(COMBINED264_PLUS_N32_READY), str(RUN41_ENRICHED), str(RUN41_PRED_SUMMARY), str(RUN42_REPORT), str(RUN42_MANIFEST)],
        ...
        "candidate_pool_counts": n_counts(pool),
        "batch_option_paths": {
            "option_A_batch32": str(OUTPUT_DIR / "run43_constrained_N24_N40_batch32_candidate_orders.csv"),
            "option_B_batch60": str(OUTPUT_DIR / "run43_constrained_N24_N40_batch60_candidate_orders.csv"),
            "option_C_batch40": str(OUTPUT_DIR / "run43_native_recovery_batch40_with_anchors_candidate_orders.csv"),
        },
