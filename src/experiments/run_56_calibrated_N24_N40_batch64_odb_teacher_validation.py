...
RUN_ID = "run_56_calibrated_N24_N40_batch64_odb_teacher_validation"
BATCH_NAME = "stage3_run54_calibrated_N24_N40_batch64_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN55_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_55_calibrated_N24_N40_batch64_cae_inp_generation"
    / "stage3_run55_calibrated_N24_N40_batch64_cae_generation_manifest.csv"
)
RUN55_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_55_calibrated_N24_N40_batch64_cae_inp_generation"
    / "RUN_55_CALIBRATED_N24_N40_BATCH64_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_56_calibrated_N24_N40_batch64_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_56_calibrated_N24_N40_batch64_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_56_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_solver_completion_audit.json"
AUDIT_MD = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_teacher_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "run56_calibrated_N24_N40_batch64_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_56_CALIBRATED_N24_N40_BATCH64_ODB_TEACHER_VALIDATION_REPORT.md"
...
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run55": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN55_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run55 calibrated N24/N40 batch64 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run55 calibrated N24/N40 batch64 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN56_CALIBRATED_N24_N40_BATCH64_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN56_CALIBRATED_N24_N40_BATCH64_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN56_CALIBRATED_N24_N40_BATCH64_SOLVER_COMPLETION_64_OF_64"
    return {
        ...
        "run55_report": str(RUN55_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN56_CALIBRATED_N24_N40_BATCH64_ODB_TEACHER_VALIDATION_64_OF_64" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN56_CALIBRATED_N24_N40_BATCH64_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Run56 Calibrated N24/N40 Batch64 Solver Completion Audit",
        "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run55_manifest_csv": str(RUN55_MANIFEST_CSV),
                    "run55_report_md": str(RUN55_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
