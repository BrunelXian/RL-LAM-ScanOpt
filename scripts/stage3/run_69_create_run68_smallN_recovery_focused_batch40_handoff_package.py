...
RUN_ID = "run_69_run68_smallN_recovery_focused_batch40_handoff_package"
RUN_NAME = "run68 small-N recovery-focused batch40 handoff package"
SCRIPT_PATH = ROOT / "scripts" / "stage3" / "run_69_create_run68_smallN_recovery_focused_batch40_handoff_package.py"

RUN68_DIR = ROOT / "outputs" / "stage3_run_68_combined480_model_update_smallN_recovery_candidate_generation"
RUN68_OPTION_A = RUN68_DIR / "run68_smallN_recovery_focused_batch40_candidate_orders.csv"
RUN68_POOL = RUN68_DIR / "run68_candidate_pool_scored.csv"
RUN68_COMPARISON = RUN68_DIR / "run68_batch_options_comparison_to_previous.csv"
RUN68_COMPARISON_SUMMARY = RUN68_DIR / "run68_batch_options_comparison_summary.json"
RUN68_EVIDENCE_UPDATE = RUN68_DIR / "full_variable_N_evidence_update_after_run67.md"
...
RUN68_MANIFEST = ROOT / "artifacts" / "manifests" / "stage3_run_68_manifest.json"
...
OUTPUT_DIR = ROOT / "outputs" / "stage3_run_69_run68_smallN_recovery_focused_batch40_handoff_package"
SCAN_DIR = OUTPUT_DIR / "scan_orders"
REPORT_DIR = ROOT / "docs" / "stage3" / "runs" / "run_69_run68_smallN_recovery_focused_batch40_handoff_package"
REPORT_PATH = REPORT_DIR / "RUN_69_RUN68_SMALLN_RECOVERY_FOCUSED_BATCH40_HANDOFF_PACKAGE_REPORT.md"
MANIFEST_PATH = ROOT / "artifacts" / "manifests" / "stage3_run_69_manifest.json"
CLAIM_BOUNDARY_MD = OUTPUT_DIR / "run69_claim_boundary.md"
CLAIM_BOUNDARY_JSON = OUTPUT_DIR / "run69_claim_boundary.json"

BATCH_NAME = "stage3_run69_smallN_recovery_focused_batch40_v01"
BATCH_OPTION = "smallN_recovery_focused_batch40"
EXPECTED_COUNTS = {12: 16, 16: 16, 24: 4, 40: 4}

VALIDATION_JSON = OUTPUT_DIR / "run69_input_validation_summary.json"
HANDOFF_CSV = OUTPUT_DIR / "stage3_run69_smallN_recovery_focused_batch40_candidate_orders.csv"
CAE_TEMPLATE = OUTPUT_DIR / "stage3_run69_smallN_recovery_focused_batch40_future_cae_handoff_manifest_TEMPLATE.csv"
ABQ_TEMPLATE = OUTPUT_DIR / "stage3_run69_smallN_recovery_focused_batch40_abqjobpilot_commands_TEMPLATE.txt"
REVIEW_CSV = OUTPUT_DIR / "smallN_recovery_focused_batch40_review_summary.csv"
REVIEW_JSON = OUTPUT_DIR / "smallN_recovery_focused_batch40_review_summary.json"
REVIEW_MD = OUTPUT_DIR / "smallN_recovery_focused_batch40_review_summary.md"
...
def previous_hashes() -> dict[str, set[str]]:
    return {
        "combined480": load_hashes(COMBINED480_READY),
...
def validate_input(df: pd.DataFrame) -> dict[str, Any]:
    errors: list[str] = []
    if not RUN68_OPTION_A.exists():
        errors.append(f"missing selected input CSV: {RUN68_OPTION_A}")
    if len(df) != 40:
        errors.append(f"row count expected 40, got {len(df)}")
    ...
    if "batch_option" in df.columns and not (df["batch_option"].astype(str) == BATCH_OPTION).all():
        errors.append("selected batch is not uniformly Run68 Option A smallN_recovery_focused_batch40")
    ...
    summary = {
        "timestamp": now_iso(),
        "verdict": "PASS_RUN69_SMALLN_RECOVERY_FOCUSED_BATCH40_INPUT_READY" if not errors else "FAIL_RUN69_INPUT_VALIDATION",
        "errors": errors,
...
def rename_handoff(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in handoff.iterrows():
        n = int(row["n"])
        per_n_counter[n] += 1
        names.append(f"S3R69SNR_N{n}_B{per_n_counter[n]:02d}_{short_bucket(row)}")
    handoff["handoff_strategy_name"] = names
    handoff["run_id"] = RUN_ID
    handoff["batch_option"] = BATCH_OPTION
    handoff["batch_name"] = BATCH_NAME
    handoff["original_run68_candidate_id"] = handoff.get("candidate_id", "")
    handoff["original_run68_strategy_name"] = handoff.get("strategy_name", "")
    ...
    handoff["smallN_recovery_prediction"] = handoff.get("variable_N_recovery_score")
    handoff["variable_N_balanced_prediction"] = handoff.get("variable_N_recovery_score")
    ...
    handoff["notes"] = "Run69 handoff only. Small-N recovery-focused batch40 candidate is not teacher-validated."

    columns = [
        "run_id", "batch_option", "batch_name", "n", "handoff_strategy_name",
        "original_run68_candidate_id", "original_run68_strategy_name",
        ...
        "smallN_recovery_prediction", "variable_N_balanced_prediction",
        ...
        "nearest_existing_teacher_strategy", "native_validation_N", "smallN_recovery",
        "variable_N_recovery", "anchor_case", "order_json", "order_compact", "order_hash",
...
def row_metadata(row: pd.Series) -> dict[str, Any]:
    return {
        ...
        "smallN_recovery_prediction": row.get("smallN_recovery_prediction"),
        "variable_N_balanced_prediction": row.get("variable_N_balanced_prediction"),
...
def write_scan_jsons(handoff: pd.DataFrame) -> None:
    ...
    for _, row in handoff.iterrows():
        payload = {
            ...
            "original_run68_candidate_id": row["original_run68_candidate_id"],
            ...
            "notes": "Run69 handoff only. Small-N recovery-focused batch40 candidate is not teacher-validated.",
        }
...
def write_future_templates(handoff: pd.DataFrame) -> None:
    ...
    commands = [
        ...
        "# Do not execute this file during Run69.",
    ]
    for _, row in handoff.iterrows():
        n = int(row["n"])
...
def write_review(handoff: pd.DataFrame, validation: dict[str, Any]) -> dict[str, Any]:
    summary = {
        "timestamp": now_iso(),
        "headline": "Run69 packages the recommended small-N recovery follow-up after Run67/Run68: N12/N16 recovery is primary while N24/N40 are retained only as mature anchors.",
        ...
        "mean_smallN_recovery_prediction_per_N": numeric_summary(handoff, "smallN_recovery_prediction"),
        "mean_variable_N_balanced_prediction_per_N": numeric_summary(handoff, "variable_N_balanced_prediction"),
    ...
    REVIEW_MD.write_text(
        "# Small-N Recovery-Focused Batch40 Review Summary\n\n"
        f"{summary['headline']}\n\n"
        f"- Total: {summary['total_count']}\n"
        f"- Per-N counts: {summary['per_N_counts']}\n"
        "- Includes N12/N16/N24/N40; excludes N32.\n"
        "- This batch continues repairing N12/N16 under-sampling while retaining only minimal mature N24/N40 anchors.\n"
        "- This is not teacher-validated until future Abaqus validation.\n"
        "- Run69 did not create CAE/INP files.\n\n"
...
def write_claim_boundary() -> None:
    safe = [
        "Run69 packages selected Run68 Option A small-N recovery-focused batch40 candidates for human review and future CAE generation.",
        "The selected batch contains N12=16, N16=16, N24=4, and N40=4.",
        "No N32 candidates are included.",
        "The batch is designed to continue repairing N12/N16 under-sampling while retaining minimal mature N24/N40 anchors.",
    ...
    write_json(CLAIM_BOUNDARY_JSON, {"verdict": "RUN69_HANDOFF_ONLY_SMALLN_RECOVERY_FOCUSED_BATCH40_NO_TEACHER_VALIDATION", "safe_claims": safe, "unsafe_claims": unsafe})


def write_report(validation: dict[str, Any], review: dict[str, Any]) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)
    REPORT_PATH.write_text(f"""# Stage 3 Run 69 - Run68 Small-N Recovery-Focused Batch40 Handoff Package
...
## 2. Inputs
- Selected Run68 Option A: `{RUN68_OPTION_A}`
- Run68 candidate pool: `{RUN68_POOL}`
- Run68 comparison summary: `{RUN68_COMPARISON_SUMMARY}`
- Run68 evidence update: `{RUN68_EVIDENCE_UPDATE}`
- Native combined480 RL-ready dataset: `{COMBINED480_READY}`
...
def write_manifest(validation: dict[str, Any], review: dict[str, Any]) -> None:
    ...
    manifest = {
        ...
        "input_files": [
            str(RUN68_OPTION_A),
            str(RUN68_POOL),
            str(RUN68_COMPARISON),
            str(RUN68_COMPARISON_SUMMARY),
            str(RUN68_EVIDENCE_UPDATE),
            str(RUN68_REPORT),
            str(RUN68_MANIFEST),
            str(COMBINED480_READY),
        ],
...
def main() -> None:
    ...
    selected = read_csv(RUN68_OPTION_A)
