...
RUN_ID = "run_27_shortlist64_odb_teacher_validation"
BATCH_NAME = "stage3_run24_shortlist64_active_learning_calibration_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN25_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_25_shortlist64_active_learning_cae_inp_generation"
    / "stage3_run25_shortlist64_cae_generation_manifest.csv"
)
PATCH_SUMMARY_JSON = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_25_shortlist64_active_learning_cae_inp_generation"
    / "run25_N40_B02_B03_B04_B05_cool_initialInc_patch_summary.json"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_27_shortlist64_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_27_shortlist64_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_27_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run27_shortlist64_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run27_shortlist64_solver_completion_audit.json"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run27_shortlist64_odb_extraction_summary.csv"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run27_shortlist64_teacher_metrics.csv"
SUMMARY_JSON = OUTPUT_DIR / "run27_shortlist64_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_27_SHORTLIST64_ODB_TEACHER_VALIDATION_REPORT.md"
...
EXPECTED_BY_N = {12: 8, 16: 8, 24: 24, 40: 24}
PATCHED_STRATEGIES = {
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN25_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != 64:
        raise RuntimeError(f"Expected 64 run25 shortlist64 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside shortlist64 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN27_SHORTLIST64_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN27_SHORTLIST64_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN27_SHORTLIST64_SOLVER_COMPLETION_64_OF_64"
    return {
        ...
        "patch_summary": str(PATCH_SUMMARY_JSON),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN27_SHORTLIST64_ODB_TEACHER_VALIDATION_64_OF_64" if not failed and len(rows) == 64 else "FAIL_RUN27_SHORTLIST64_ODB_TEACHER_VALIDATION",
        "total_expected": 64,
...
def write_report(audit_summary: dict[str, object], extraction_summary: dict[str, object] | None, audit_rows: list[dict[str, object]], extraction_rows: list[dict[str, object]]) -> None:
    ...
    lines.extend(["", "## Patched N40 Cases", ""])
...
def main() -> int:
    ...
    if audit_summary["total_failed_or_incomplete"] == 0 and audit_summary["total_complete"] == 64:
        for index, row in enumerate(audit_rows, start=1):
            print(f"[{index:02d}/64] extracting {row['handoff_strategy_name']}")
            try:
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run25_manifest_csv": str(RUN25_MANIFEST_CSV),
                    "patch_summary_json": str(PATCH_SUMMARY_JSON),
                },
                "outputs_written": [
                    str(AUDIT_CSV),
                    str(AUDIT_JSON),
