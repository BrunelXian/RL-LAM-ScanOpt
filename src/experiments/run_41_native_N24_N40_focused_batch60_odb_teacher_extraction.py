...
RUN_ID = "run_41_native_N24_N40_focused_batch60_odb_teacher_extraction"
BATCH_NAME = "stage3_run39_native_N24_N40_focused_batch60_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN40_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_40_native_N24_N40_focused_batch60_cae_inp_generation"
    / "stage3_run40_native_N24_N40_focused_batch60_cae_generation_manifest.csv"
)
RUN40_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_40_native_N24_N40_focused_batch60_cae_inp_generation"
    / "RUN_40_NATIVE_N24_N40_FOCUSED_BATCH60_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_41_native_N24_N40_focused_batch60_odb_teacher_extraction"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_41_native_N24_N40_focused_batch60_odb_teacher_extraction"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_41_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run41_native_N24_N40_focused_batch60_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run41_native_N24_N40_focused_batch60_solver_completion_audit.json"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run41_native_N24_N40_focused_batch60_odb_extraction_summary.csv"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run41_native_N24_N40_focused_batch60_teacher_metrics.csv"
SUMMARY_JSON = OUTPUT_DIR / "run41_native_N24_N40_focused_batch60_odb_teacher_extraction_summary.json"
REPORT_MD = REPORT_DIR / "RUN_41_NATIVE_N24_N40_FOCUSED_BATCH60_ODB_TEACHER_EXTRACTION_REPORT.md"
...
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run40": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN40_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run40 native N24/N40 focused batch60 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run40 native N24/N40 focused batch60 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN41_NATIVE_N24_N40_FOCUSED_BATCH60_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN41_NATIVE_N24_N40_FOCUSED_BATCH60_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN41_NATIVE_N24_N40_FOCUSED_BATCH60_SOLVER_COMPLETION_60_OF_60"
    return {
        ...
        "run40_report": str(RUN40_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN41_NATIVE_N24_N40_FOCUSED_BATCH60_ODB_TEACHER_EXTRACTION_60_OF_60" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN41_NATIVE_N24_N40_FOCUSED_BATCH60_ODB_TEACHER_EXTRACTION",
        "total_expected": TOTAL_EXPECTED,
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run40_manifest_csv": str(RUN40_MANIFEST_CSV),
                    "run40_report_md": str(RUN40_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
