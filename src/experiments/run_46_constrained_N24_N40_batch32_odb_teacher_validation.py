...
RUN_ID = "run_46_constrained_N24_N40_batch32_odb_teacher_validation"
BATCH_NAME = "stage3_run44_constrained_N24_N40_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN45_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_45_constrained_N24_N40_batch32_cae_inp_generation"
    / "stage3_run45_constrained_N24_N40_batch32_cae_generation_manifest.csv"
)
RUN45_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_45_constrained_N24_N40_batch32_cae_inp_generation"
    / "RUN_45_CONSTRAINED_N24_N40_BATCH32_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_46_constrained_N24_N40_batch32_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_46_constrained_N24_N40_batch32_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_46_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run46_constrained_N24_N40_batch32_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run46_constrained_N24_N40_batch32_solver_completion_audit.json"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run46_constrained_N24_N40_batch32_odb_extraction_summary.csv"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run46_constrained_N24_N40_batch32_teacher_metrics.csv"
SUMMARY_JSON = OUTPUT_DIR / "run46_constrained_N24_N40_batch32_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_46_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION_REPORT.md"
...
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run45": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN45_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run45 constrained N24/N40 batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run45 constrained N24/N40 batch32 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN46_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN46_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN46_CONSTRAINED_N24_N40_BATCH32_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "run45_report": str(RUN45_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN46_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN46_CONSTRAINED_N24_N40_BATCH32_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_report(audit_summary: dict[str, object], extraction_summary: dict[str, object] | None, audit_rows: list[dict[str, object]], extraction_rows: list[dict[str, object]]) -> None:
    ...
    lines = [
        "# Stage 3 Run46 Constrained N24/N40 Batch32 ODB Teacher Validation Report",
        "",
        "## Verdict",
        "",
        f"`{verdict}`",
    ...
    lines.extend(
        [
            ...
            "## Scientific Boundary",
            "",
            "This report records computed teacher metrics for Run45 constrained N24/N40 batch32. It does not claim physical superiority beyond computed teacher metrics, RL/GNN success, or arbitrary-N generalisation.",
            "",
            "## Guardrails",
            "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run45_manifest_csv": str(RUN45_MANIFEST_CSV),
                    "run45_report_md": str(RUN45_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
