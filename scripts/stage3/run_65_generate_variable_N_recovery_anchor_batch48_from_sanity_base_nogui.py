...
RUN_ID = "run_65_variable_N_recovery_anchor_batch48_cae_inp_generation"
...
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run65_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run65_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run65_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run65_variable_N_recovery_anchor_batch48_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run65_variable_N_recovery_anchor_batch48_cae_generation_manifest.json"
...
def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run65 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "run63_variable_N_recovery_anchor_batch48":
                raise run40GenerationError("non-run63_variable_N_recovery_anchor_batch48 row selected: {}".format(row.get("handoff_strategy_name")))
    ...
    if len(rows) != 48:
        raise run40GenerationError("generation plan row count {} != 48".format(len(rows)))
    ...
    if any(int(row["n"]) == 32 for row in rows):
        raise run40GenerationError("forbidden N32 row selected")
    return rows
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run65_case_metadata.json"
    metadata_keys = [
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "run65 constrained N24/N40 batch32 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN65_VARIABLE_N_RECOVERY_ANCHOR_BATCH48_GENERATION")
        print(str(exc))
