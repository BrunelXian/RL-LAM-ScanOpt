...
RUN_ID = "run_55_calibrated_N24_N40_batch64_cae_inp_generation"
...
PLAN_CSV = OUTPUT_DIR / "run55_generation_plan.csv"
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run55_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run55_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run55_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run55_calibrated_N24_N40_batch64_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run55_calibrated_N24_N40_batch64_cae_generation_manifest.json"
...
def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run55 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "run53_calibrated_N24_N40_batch64":
                raise run40GenerationError("non-run53_calibrated_N24_N40_batch64 row selected: {}".format(row.get("handoff_strategy_name")))
    ...
    if len(rows) != 64:
        raise run40GenerationError("generation plan row count {} != 64".format(len(rows)))
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run55_case_metadata.json"
    metadata_keys = [
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "run55 constrained N24/N40 batch32 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN55_CALIBRATED_N24_N40_BATCH64_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN55_CALIBRATED_N24_N40_BATCH64_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN55_CALIBRATED_N24_N40_BATCH64_GENERATION")
        print(str(exc))
