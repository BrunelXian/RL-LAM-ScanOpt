...
SELECTED_BATCH = "batch28"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN20_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_20_batch28_cae_inp_generation"
    / "stage3_run20_batch28_cae_generation_manifest.csv"
)
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_20_batch28_odb_teacher_validation"

AUDIT_CSV = OUTPUT_DIR / "run20_batch28_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run20_batch28_solver_completion_audit.json"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run20_batch28_odb_extraction_summary.csv"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run20_batch28_teacher_metrics.csv"
SUMMARY_JSON = OUTPUT_DIR / "run20_batch28_odb_teacher_validation_summary.json"
REPORT_MD = OUTPUT_DIR / "run20_batch28_odb_teacher_validation_report.md"
...
EXPECTED_BY_N = {12: 4, 16: 4, 24: 10, 40: 10}
...
def latest_full_run_log_segment(log_text: str) -> str:
    marker = "START FULL_RUNNING"
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN20_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != 28:
        raise RuntimeError(f"Expected 28 run20 batch28 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside the selected batch28 batch name")
    return rows
...
def audit_case(row: dict[str, str]) -> dict[str, object]:
    ...
    latest_log_text = latest_full_run_log_segment(log_text)
    log_fatal_marker = contains_any(latest_log_text, FATAL_MARKERS)
    warning_marker = any(contains_warning(text) for text in [sta_text, dat_text, msg_text, log_text])
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN20_BATCH28_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN20_BATCH28_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN20_BATCH28_SOLVER_COMPLETION_28_OF_28"
    return {
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN20_BATCH28_ODB_TEACHER_VALIDATION_28_OF_28" if not failed and len(rows) == 28 else "FAIL_RUN20_BATCH28_ODB_TEACHER_VALIDATION",
        "total_expected": 28,
...
def main() -> int:
    ...
    if audit_summary["total_failed_or_incomplete"] == 0 and audit_summary["total_complete"] == 28:
        for index, row in enumerate(audit_rows, start=1):
            print(f"[{index:02d}/28] extracting {row['handoff_strategy_name']}")
            try:
        ...
        json.dumps(
            {
                ...
                "guardrails": {
                    ...
                    "cae_inp_jnl_odb_modified": False,
                    "batch24_processed": False,
                    "mixed_with_probe60_or_run08_run16": False,
                },
            },
            indent=2,
            default=json_safe,
    ...
    final_verdict = extraction_summary["verdict"] if extraction_summary is not None else audit_summary["verdict"]
