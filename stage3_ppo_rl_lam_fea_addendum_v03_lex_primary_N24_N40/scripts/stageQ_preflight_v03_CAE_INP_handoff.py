"""Stage Q preflight for PPO v03 lex-primary N24/N40 CAE/INP handoff."""
...
CSV_OUT = CHECKS_DIR / "stageQ_preflight_v03_CAE_INP_handoff.csv"
JSON_OUT = CHECKS_DIR / "stageQ_preflight_v03_CAE_INP_handoff_summary.json"
BASES = {
...
def main() -> int:
    ...
    rows_out.append({"check": "selected_v03_batch_exists", "status": "PASS" if selected_exists else "FAIL", "details": str(SELECTED_CSV)})
    if not selected_exists:
        blockers.append("selected v03 batch CSV missing")
    else:
        ...
        checks = {
            ...
            "candidate_source_PPO_v03_checkpoint_inference": all(row.get("candidate_source") == "PPO_v03_checkpoint_inference" for row in selected_rows),
            ...
            "ppo_v03_checkpoint_present": all(row.get("ppo_v03_checkpoint") for row in selected_rows),
            "ppo_seed_20260627": all(str(row.get("ppo_seed")) == "20260627" for row in selected_rows),
            "partial_training_caveat_recorded": True,
        }
    ...
    verdict = "FAIL_STAGEQ_V03_CAE_INP_PREFLIGHT_BLOCKED" if blockers else ("WARNING_STAGEQ_V03_CAE_INP_PREFLIGHT_REVIEW" if warnings else "PASS_STAGEQ_V03_CAE_INP_PREFLIGHT_READY")
    summary = {
        "verdict": verdict,
        ...
        "training_verdict": "WARNING_V03_PPO_TRAINING_PARTIAL_REVIEW",
    }
