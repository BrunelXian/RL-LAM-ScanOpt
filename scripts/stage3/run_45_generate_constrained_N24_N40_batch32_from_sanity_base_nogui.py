...
RUN_ID = "run_45_constrained_N24_N40_batch32_cae_inp_generation"
BATCH_NAME = "stage3_run44_constrained_N24_N40_batch32_v01"
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_45_constrained_N24_N40_batch32_cae_inp_generation"
PLAN_CSV = OUTPUT_DIR / "run45_generation_plan.csv"
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run45_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run45_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run45_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run45_constrained_N24_N40_batch32_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run45_constrained_N24_N40_batch32_cae_generation_manifest.json"
...
def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run45 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "run43_constrained_N24_N40_batch32":
                raise run40GenerationError("non-run43_constrained_N24_N40_batch32 row selected: {}".format(row.get("handoff_strategy_name")))
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run45_case_metadata.json"
    metadata_keys = [
        "run_id", "batch_name", "selected_batch", "n", "handoff_strategy_name",
        "original_run43_candidate_id", "original_run43_strategy_name",
        ...
        "N24_N40_focused", "constrained_reward_balanced", "native_validation_N",
    ]
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "Run45 constrained N24/N40 batch32 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN45_CONSTRAINED_N24_N40_BATCH32_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN45_CONSTRAINED_N24_N40_BATCH32_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN45_CONSTRAINED_N24_N40_BATCH32_GENERATION")
        print(str(exc))
