...
RUN44_DIR = PROJECT_ROOT / "outputs" / "stage3_run_49_run48_stricter_constrained_N24_N40_batch32_handoff_package"
INPUT_CSV = RUN44_DIR / "stage3_run49_stricter_constrained_N24_N40_batch32_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run50_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run50_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run50_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run50_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN50_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN50_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "stricter_constrained_N24_N40_batch32":
            errors.append("row {} is not stricter_constrained_N24_N40_batch32: {}".format(idx, row.get("batch_option")))
    ...
    verdict = "PASS_RUN50_STRICTER_CONSTRAINED_N24_N40_BATCH32_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN50_STRICTER_CONSTRAINED_N24_N40_BATCH32_INPUT_INVALID"
    summary = {
        "verdict": verdict,
