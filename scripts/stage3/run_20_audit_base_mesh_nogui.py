"""Abaqus noGUI base mesh audit for Stage 3 run20 batch28 generation."""
...
CSV_PATH = OUTPUT_DIR / "run20_base_mesh_audit.csv"
SUMMARY_PATH = OUTPUT_DIR / "run20_base_mesh_audit_summary.json"
...
def audit_one(n: int, cae_path: Path) -> dict[str, object]:
    ...
    try:
        ...
        ok = (
            float(row["total_node_count"]) > 0
            and float(row["total_element_count"]) > 0
...
def main() -> int:
    ...
    overall = (
        ...
        else "FAIL_RUN20_BASE_MESH_AUDIT_INVALID"
    )
    summary = {
        "verdict": overall,
        "rows": rows,
    ...
    return 0 if overall == "PASS_RUN20_BASE_MESH_AUDIT_READY" else 1
