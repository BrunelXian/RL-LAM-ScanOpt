...
RUN_ID = "run_76_final_smallN_diagnostic_batch32_odb_teacher_validation"
BATCH_NAME = "stage3_run74_final_smallN_diagnostic_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
RUN75_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_run_75_final_smallN_diagnostic_batch32_cae_inp_generation"
    / "stage3_run75_final_smallN_diagnostic_batch32_cae_generation_manifest.csv"
)
RUN75_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3"
    / "runs"
    / "run_75_final_smallN_diagnostic_batch32_cae_inp_generation"
    / "RUN_75_FINAL_SMALLN_DIAGNOSTIC_BATCH32_CAE_INP_GENERATION_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_run_76_final_smallN_diagnostic_batch32_odb_teacher_validation"
REPORT_DIR = PROJECT_ROOT / "docs" / "stage3" / "runs" / "run_76_final_smallN_diagnostic_batch32_odb_teacher_validation"
MANIFEST_PATH = PROJECT_ROOT / "artifacts" / "manifests" / "stage3_run_76_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_solver_completion_audit.json"
AUDIT_MD = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_teacher_metrics_summary.json"
SUMMARY_JSON = OUTPUT_DIR / "run76_final_smallN_diagnostic_batch32_odb_teacher_validation_summary.json"
REPORT_MD = REPORT_DIR / "RUN_76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_ODB_TEACHER_VALIDATION_REPORT.md"
...
EXPECTED_BY_N = {12: 14, 16: 14, 24: 2, 40: 2}
TOTAL_EXPECTED = 32
SPECIAL_GENERATION_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_run75": True,
...
def load_manifest_rows() -> list[dict[str, str]]:
    with RUN75_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
        rows = list(csv.DictReader(handle))
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} run75 final small-N diagnostic batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside run75 final small-N diagnostic batch32 batch name")
    return rows
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_RUN76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_RUN76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_RUN76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "run75_report": str(RUN75_REPORT_MD),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_RUN76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_ODB_TEACHER_VALIDATION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_RUN76_FINAL_SMALLN_DIAGNOSTIC_BATCH32_ODB_TEACHER_VALIDATION",
        "total_expected": TOTAL_EXPECTED,
...
def write_audit_markdown(audit_summary: dict[str, object], audit_rows: list[dict[str, object]]) -> None:
    lines = [
        "# Run76 Final Small-N Diagnostic Batch32 Solver Completion Audit",
        "",
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "run75_manifest_csv": str(RUN75_MANIFEST_CSV),
                    "run75_report_md": str(RUN75_REPORT_MD),
                },
                "generation_details": SPECIAL_GENERATION_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
