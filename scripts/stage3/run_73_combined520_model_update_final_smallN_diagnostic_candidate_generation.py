...
RUN_ID = "run_73_combined520_model_update_final_smallN_diagnostic_candidate_generation"
RUN_NAME = "combined520 model update final small-N diagnostic candidate generation"
SCRIPT_PATH = ROOT / "scripts" / "stage3" / "run_73_combined520_model_update_final_smallN_diagnostic_candidate_generation.py"

RUN72_DIR = ROOT / "outputs" / "stage3_run_72_smallN_recovery_focused_batch40_teacher_metrics_ingestion_and_combined520_ranking"
COMBINED520_READY = RUN72_DIR / "combined520_RL_ready_dataset.csv"
COMBINED520_PLUS_N32_READY = RUN72_DIR / "combined520_plus_N32_RL_ready_dataset.csv"
RUN71_ENRICHED = RUN72_DIR / "run71_smallN_recovery_focused_batch40_teacher_dataset_enriched.csv"
RUN71_COMPARISON = RUN72_DIR / "run71_vs_combined480_best_comparison.csv"
RUN71_EFFECTIVENESS = RUN72_DIR / "run71_smallN_recovery_focused_batch40_effectiveness_audit.csv"
RUN71_PRED_AUDIT = RUN72_DIR / "run71_prediction_audit_for_run68_batch40.csv"
RUN71_PRED_SUMMARY = RUN72_DIR / "run71_prediction_audit_for_run68_batch40_summary.json"
RUN72_MATURITY = RUN72_DIR / "full_variable_N_updated_maturity_and_claim_boundary_summary.json"
RUN72_MATURITY_MD = RUN72_DIR / "full_variable_N_updated_maturity_and_claim_boundary_audit.md"
...
RUN72_MANIFEST = ROOT / "artifacts" / "manifests" / "stage3_run_72_manifest.json"
...
OUTPUT_DIR = ROOT / "outputs" / "stage3_run_73_combined520_model_update_final_smallN_diagnostic_candidate_generation"
REPORT_DIR = ROOT / "docs" / "stage3" / "runs" / "run_73_combined520_model_update_final_smallN_diagnostic_candidate_generation"
REPORT_PATH = REPORT_DIR / "RUN_73_COMBINED520_MODEL_UPDATE_FINAL_SMALLN_DIAGNOSTIC_CANDIDATE_GENERATION_REPORT.md"
MANIFEST_PATH = ROOT / "artifacts" / "manifests" / "stage3_run_73_manifest.json"
CLAIM_BOUNDARY_MD = OUTPUT_DIR / "run73_claim_boundary.md"
CLAIM_BOUNDARY_JSON = OUTPUT_DIR / "run73_claim_boundary.json"

EXPECTED_NATIVE = {12: 64, 16: 64, 24: 188, 40: 204}
EXPECTED_PLUS = {12: 64, 16: 64, 24: 188, 32: 332, 40: 204}
POOL_TARGETS = {12: 4000, 16: 4000, 24: 500, 40: 500}
OPTION_A_COUNTS = {12: 14, 16: 14, 24: 2, 40: 2}
OPTION_B_COUNTS = {12: 10, 16: 10, 24: 2, 40: 2}
OPTION_C_COUNTS = {}
GLOBAL_SEED = 73042
...
def add_features_and_targets(df: pd.DataFrame, native: bool) -> pd.DataFrame:
    ...
    if native:
        score_cols = [
            "target_u2_score_combined520_rank",
            "target_peeq_score_combined520_rank",
            "target_surfaceT_score_combined520_rank",
            "target_mises_score_combined520_rank",
        ]
        out["target_reward_combined520_u2_primary"] = as_float(out["target_reward_combined520_u2_primary"])
        prefix = "combined520"
    else:
        score_cols = [
            "target_u2_score_combined520_plus_N32_rank",
            "target_peeq_score_combined520_plus_N32_rank",
            "target_surfaceT_score_combined520_plus_N32_rank",
            "target_mises_score_combined520_plus_N32_rank",
        ]
        out["target_reward_combined520_plus_N32_mapped_u2_primary"] = as_float(out["target_reward_combined520_plus_N32_mapped_u2_primary"])
        prefix = "combined520_plus_N32"
    ...
    out[f"target_reward_{prefix}_N12_final_diagnostic"] = out[f"target_reward_{prefix}_N12_recovery"]
    out[f"target_reward_{prefix}_N16_final_diagnostic"] = out[f"target_reward_{prefix}_N16_recovery"]
    out[f"target_reward_{prefix}_smallN_final_diagnostic"] = out[f"target_reward_{prefix}_smallN_recovery"]
    out[f"target_reward_{prefix}_variable_N_bounded"] = out[f"target_reward_{prefix}_variable_N_balanced"]
    return out


def validate_inputs(native: pd.DataFrame, plus: pd.DataFrame, pred_summary: dict[str, Any], maturity: dict[str, Any]) -> dict[str, Any]:
    errors = []
    if len(native) != 520 or n_counts(native) != EXPECTED_NATIVE:
        errors.append(f"native combined520 count mismatch: rows={len(native)} counts={n_counts(native)}")
    if 32 in set(native["n"].astype(int)):
        errors.append("native combined520 contains N32")
    if len(plus) != 852 or n_counts(plus) != EXPECTED_PLUS:
        errors.append(f"combined520_plus_N32 count mismatch: rows={len(plus)} counts={n_counts(plus)}")
    for col in [
        ...
        "target_reward_combined520_N12_recovery",
        "target_reward_combined520_N16_recovery",
        "target_reward_combined520_N12_final_diagnostic",
        "target_reward_combined520_N16_final_diagnostic",
        "target_reward_combined520_smallN_final_diagnostic",
        "target_reward_combined520_variable_N_bounded",
        "target_reward_combined520_smallN_recovery",
        "target_reward_combined520_variable_N_recovery",
        "target_reward_combined520_N24_anchor",
        "target_reward_combined520_N40_anchor",
    ]:
    ...
    if maturity:
        expected_maturity = {
            "n24_teacher_rows": 188,
            "n40_teacher_rows": 204,
            "n12_teacher_rows": 64,
            "n16_teacher_rows": 64,
        }
        ...
        errors.append("Run72 maturity audit summary missing")
    if pred_summary:
    ...
    return {
        "timestamp": now_iso(),
        "verdict": "PASS_RUN73_COMBINED520_AND_PLUS_N32_INPUTS_READY" if not errors else "FAIL_RUN73_INPUT_VALIDATION",
        "errors": errors,
        "native_combined520_rows": int(len(native)),
        "native_combined520_counts": n_counts(native),
        "combined520_plus_N32_rows": int(len(plus)),
        "combined520_plus_N32_counts": n_counts(plus),
        "maturity_context": maturity,
        "run71_prediction_audit_context": {"u2_primary_or_reward_spearman": reward_s, "mean_top5_overlap": top5, "top1_hits": pred_summary.get("top1_hits") if pred_summary else None},
    }
...
def validation_protocols(df: pd.DataFrame, regime: str) -> list[tuple[str, np.ndarray, np.ndarray]]:
    ...
    if "dataset_source" in df.columns and (df["dataset_source"].astype(str) == "run71_smallN_recovery_focused_batch40").any():
        test = df.index[df["dataset_source"].astype(str) == "run71_smallN_recovery_focused_batch40"].to_numpy()
        train = np.setdiff1d(rows, test)
        protocols.append(("train_pre_Run71_test_Run71", train, test))
    return protocols
...
def run_surrogate_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, dict[str, Any], Any, list[str]]:
    ...
    targets = {
        ...
        "plus_N32_unweighted": [
            "target_reward_combined520_plus_N32_mapped_u2_primary",
            "target_reward_combined520_plus_N32_strict_u2_surfaceT",
            "target_reward_combined520_plus_N32_constrained_u2_reward_balanced",
            "target_reward_combined520_plus_N32_N12_final_diagnostic",
            "target_reward_combined520_plus_N32_N16_final_diagnostic",
            "target_reward_combined520_plus_N32_smallN_final_diagnostic",
            "target_reward_combined520_plus_N32_variable_N_bounded",
            "target_reward_combined520_plus_N32_N24_anchor",
            "target_reward_combined520_plus_N32_N40_anchor",
        ],
        "plus_N32_balanced": [
            "target_reward_combined520_plus_N32_mapped_u2_primary",
            "target_reward_combined520_plus_N32_strict_u2_surfaceT",
    ...
    frames = {"native_combined520": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}
    rows = []
    ...
    native_best = grouped[grouped["regime"] == "native_combined520"].head(1).iloc[0].to_dict()
    ...
    summary = {
        ...
        "best_N12_final_diagnostic": best_for("target_reward_combined520_N12_final_diagnostic"),
        "best_N16_final_diagnostic": best_for("target_reward_combined520_N16_final_diagnostic"),
        "best_smallN_final_diagnostic": best_for("target_reward_combined520_smallN_final_diagnostic"),
        "best_variable_N_bounded": best_for("target_reward_combined520_variable_N_bounded"),
        "best_N24_anchor": best_for("target_reward_combined520_N24_anchor"),
        "best_N40_anchor": best_for("target_reward_combined520_N40_anchor"),
        "best_N40_u2_reward_retention": best_for("target_reward_combined520_N40_u2_reward_retention"),
        "best_N40_penalty_repair": best_for("target_reward_combined520_N40_penalty_repair"),
        "best_N24_maintenance": best_for("target_reward_combined520_N24_maintenance"),
        "best_penalty_repair": best_for("target_reward_combined520_penalty_repair"),
        "n32_augmented_better_than_native": bool(plus_best["macro_spearman"] > native_best["macro_spearman"]),
        "run68_reference": {"best_native_spearman": 0.7385, "best_N12_recovery_spearman": 0.7385, "best_N16_recovery_spearman": 0.7385, "run71_prediction_u2_primary_spearman": 0.3348},
    }
...
def run_gnn_validation(native: pd.DataFrame, plus: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    rows = []
    for regime, df in {"native_combined520": native, "plus_N32_unweighted": plus, "plus_N32_balanced": plus}.items():
        target = "target_reward_combined520_u2_primary" if regime == "native_combined520" else "target_reward_combined520_plus_N32_mapped_u2_primary"
        sw = sample_weights(df) if regime.endswith("balanced") else None
        for protocol, train, test in validation_protocols(df, regime):
            try:
...
def generate_candidates(native: pd.DataFrame, model: Any, features: list[str]) -> pd.DataFrame:
    ...
    for n in [12, 16, 24, 40]:
        ...
        best_by[(n, "reward")] = parse_order(g.sort_values("target_reward_combined520_u2_primary", ascending=False).iloc[0]["order_json"])
        best_by[(n, "constrained")] = parse_order(g.sort_values("target_reward_combined520_constrained_u2_reward_balanced", ascending=False).iloc[0]["order_json"])
        best_by[(n, "strict")] = parse_order(g.sort_values("target_reward_combined520_strict_penalty_guard", ascending=False).iloc[0]["order_json"])
        best_by[(n, "two_stage")] = parse_order(g.sort_values("target_reward_combined520_two_stage_guarded", ascending=False).iloc[0]["order_json"])
    rows, seen = [], set(existing)
    for n, target_count in POOL_TARGETS.items():
        if n in (12, 16):
            source_plan = [
                f"N{n}_final_diag_surrogate_top", f"N{n}_local_search_around_Run71_best",
                f"N{n}_final_diag_u2_safe", f"N{n}_final_diag_penalty_aware",
                f"N{n}_final_diag_reward_balanced", f"N{n}_final_diag_uncertainty",
                f"N{n}_final_diag_diversity", f"N{n}_final_diag_sentinel_control",
            ]
        elif n == 40:
            source_plan = [
                "N40_anchor_u2_reward_reference", "N40_uncertainty_anchor",
                "N40_sentinel_control",
            ]
        elif n == 24:
            source_plan = [
                "N24_anchor_top_density_reference", "N24_uncertainty_anchor",
        ...
        while sum(1 for r in rows if r["n"] == n) < target_count and attempts < target_count * 20:
            ...
            cid = f"R73_N{n}_C{sum(1 for r in rows if r['n']==n)+1:05d}"
            rows.append({
                "candidate_id": cid,
...
def select_batch(pool: pd.DataFrame, counts: dict[int, int], label: str) -> pd.DataFrame:
    ...
    primary_plan = {
        24: [
            ("N24_anchor_top_density_reference", 2),
            ("N24_uncertainty_anchor", 1),
            ("N24_sentinel_control", 1),
        ],
        40: [
            ("N40_anchor_u2_reward_reference", 2),
            ("N40_uncertainty_anchor", 1),
            ("N40_sentinel_control", 1),
        ],
        12: [
            ("N12_final_diag_surrogate_top", 4),
            ("N12_local_search_around_Run71_best", 3),
            ("N12_final_diag_u2_safe", 3),
            ("N12_final_diag_penalty_aware", 2),
            ("N12_final_diag_reward_balanced", 2),
            ("N12_final_diag_uncertainty", 1),
            ("N12_final_diag_diversity", 1),
        ],
        16: [
            ("N16_final_diag_surrogate_top", 4),
            ("N16_local_search_around_Run71_best", 3),
            ("N16_final_diag_u2_safe", 3),
            ("N16_final_diag_penalty_aware", 2),
            ("N16_final_diag_reward_balanced", 2),
            ("N16_final_diag_uncertainty", 1),
            ("N16_final_diag_diversity", 1),
        ],
    }
    fallback_order = [
        "N12_final_diag_surrogate_top", "N12_local_search_around_Run71_best", "N12_final_diag_u2_safe", "N12_final_diag_penalty_aware",
        "N16_final_diag_surrogate_top", "N16_local_search_around_Run71_best", "N16_final_diag_u2_safe", "N16_final_diag_penalty_aware",
        "N24_anchor_top_density_reference", "N24_uncertainty_anchor",
        "N40_anchor_u2_reward_reference", "N40_uncertainty_anchor",
    ]
    for n, count in counts.items():
        ...
        for i, row in enumerate(take[:count], 1):
            ...
            row["handoff_strategy_name"] = f"S3R73FSD_N{n}_B{i:02d}_{short}"
...
def compare_batches(options: dict[str, pd.DataFrame], native: pd.DataFrame) -> tuple[pd.DataFrame, dict[str, Any]]:
    previous = {"combined520_teacher_orders": set(native["order_hash"].astype(str))}
    for name, path in {"run71": RUN71_HANDOFF, "run66": RUN66_HANDOFF, "run61": RUN61_HANDOFF, "run56": RUN56_HANDOFF, "run51": RUN51_HANDOFF, "run46": RUN46_HANDOFF, "run41": RUN41_HANDOFF, "run36": RUN36_HANDOFF, "run27": RUN27_HANDOFF, "superseded_run31": RUN31_OLD}.items():
        if path.exists():
    ...
    headline = "Run73 batch options were checked for exact order overlap, final small-N diagnostic score, optional N40 follow-up score, novelty, and source diversity against combined520, Run71, Run66, Run61, Run56, Run51, Run46, Run41, Run36, Run27, and superseded Run31."
...
def write_evidence_update(native: pd.DataFrame, validation: dict[str, Any], pred_summary: dict[str, Any], maturity: dict[str, Any]) -> dict[str, Any]:
    timeline = [
        {"stage": "combined480_before_Run71", "native_rows": 480, "N12": 48, "N16": 48, "N24": 184, "N40": 200, "interpretation": "small-N recovery was primary; N24/N40 were mature anchors"},
        {"stage": "Run71_smallN_recovery_focused_batch40", "native_rows": 520, "N12": 64, "N16": 64, "N24": 188, "N40": 204, "interpretation": "small-N evidence strengthened again; one final small-N diagnostic loop is reasonable before evidence freeze"},
    ]
    update = pd.DataFrame(timeline)
    update_path = OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.csv"
    update.to_csv(update_path, index=False)

    summary = {
        "verdict": "RUN73_FULL_VARIABLE_N_EVIDENCE_NEAR_FREEZE_FINAL_SMALLN_DIAGNOSTIC_RECOMMENDED",
        "headline": "Run72/Run71 strengthened full variable-N evidence to N12=64 and N16=64 while N24=188 and N40=204 remain mature anchors; Run73 recommends a final small-N diagnostic loop before declaring an evidence freeze, with stop-and-freeze kept as an explicit option.",
        "native_combined520_counts": n_counts(native),
        "run72_maturity_context": maturity,
        "run71_prediction_audit_context": {
        ...
        "evidence_decision": {
            ...
            "continue_full_variable_N_claim_boundary": "near_freeze_but_final_smallN_diagnostic_recommended",
        },
        "timeline": timeline,
        "validation_verdict": validation["verdict"],
    }
    write_json(OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.json", summary)
    (OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.md").write_text(
        "# Stage 3 Evidence-Freeze Readiness After Run72\n\n"
        ...
        "The next candidate-generation priority is a final N12/N16 diagnostic, with only minimal N24/N40 anchor coverage. Option C preserves the alternative to stop and freeze evidence now.\n\n"
        "## Timeline\n"
        + "\n".join(f"- {row['stage']}: N12={row['N12']}, N16={row['N16']}, N24={row['N24']}, N40={row['N40']} - {row['interpretation']}" for row in timeline)
        + "\n",
        encoding="utf-8",
    )
    return summary
...
def write_claim_boundary() -> None:
    ...
    CLAIM_BOUNDARY_MD.write_text("# Run73 Claim Boundary\n\n## Safe claims\n" + "\n".join(f"- {x}" for x in safe) + "\n\n## Unsafe claims\n" + "\n".join(f"- {x}" for x in unsafe) + "\n", encoding="utf-8")
    write_json(CLAIM_BOUNDARY_JSON, {"verdict": "RUN73_MODEL_UPDATE_AND_FINAL_SMALLN_DIAGNOSTIC_CANDIDATE_GENERATION_ONLY_NO_TEACHER_VALIDATION", "safe_claims": safe, "unsafe_claims": unsafe})


def write_report(summary: dict[str, Any]) -> None:
    REPORT_PATH.write_text(f"""# Stage 3 Run 73 - Combined520 Model Update and Final Small-N Diagnostic Candidate Generation
...
## 2. Inputs
- Native combined520: `{COMBINED520_READY}`
- combined520_plus_N32: `{COMBINED520_PLUS_N32_READY}`
- Run71 prediction audit: `{RUN71_PRED_SUMMARY}`
- Run72 maturity audit: `{RUN72_MATURITY}`
...
Best N12 final diagnostic model: `{summary['surrogate'].get('best_N12_final_diagnostic')}`.

Best N16 final diagnostic model: `{summary['surrogate'].get('best_N16_final_diagnostic')}`.

Best small-N final diagnostic model: `{summary['surrogate'].get('best_smallN_final_diagnostic')}`.

Best variable-N bounded model: `{summary['surrogate'].get('best_variable_N_bounded')}`.
...
Candidate pool counts: `{summary['candidate_pool_counts']}`. N12/N16 meet the >=4000 candidate minimums, with N24/N40 frozen-anchor references included for comparison.

## 10. Option A - Final Small-N Diagnostic Batch32
Path: `{OUTPUT_DIR / 'run73_final_smallN_diagnostic_batch32_candidate_orders.csv'}`. Counts: `{summary['option_counts']['option_A_final_smallN_diagnostic_batch32']}`.

## 11. Option B - Final Small-N Diagnostic Batch24
Path: `{OUTPUT_DIR / 'run73_final_smallN_diagnostic_batch24_candidate_orders.csv'}`. Counts: `{summary['option_counts']['option_B_final_smallN_diagnostic_batch24']}`.

## 12. Option C - Stop and Freeze Evidence Package Only
Path: `{OUTPUT_DIR / 'run73_optionC_stop_and_freeze_evidence_package_only.json'}`. Counts: `{summary['option_counts']['option_C_stop_and_freeze_evidence_package_only']}`.
...
## 15. Output Files
- Candidate pool: `{OUTPUT_DIR / 'run73_candidate_pool_scored.csv'}`
- Surrogate summary: `{OUTPUT_DIR / 'run73_surrogate_validation_summary.json'}`
- GNN summary: `{OUTPUT_DIR / 'run73_gnn_reward_validation_summary.json'}`
- Pointer summary: `{OUTPUT_DIR / 'run73_graph_pointer_policy_validation_summary.json'}`
- Evidence update: `{OUTPUT_DIR / 'stage3_evidence_freeze_readiness_after_run72.md'}`
- Batch options comparison: `{OUTPUT_DIR / 'run73_batch_options_comparison_summary.json'}`
...
def main() -> None:
    ensure_dirs()
    native = add_features_and_targets(read_csv(COMBINED520_READY), native=True)
    plus = add_features_and_targets(read_csv(COMBINED520_PLUS_N32_READY), native=False)
    pred_summary = json.loads(RUN71_PRED_SUMMARY.read_text(encoding="utf-8")) if RUN71_PRED_SUMMARY.exists() else {}
    maturity = json.loads(RUN72_MATURITY.read_text(encoding="utf-8")) if RUN72_MATURITY.exists() else {}
    validation = validate_inputs(native, plus, pred_summary, maturity)
    write_json(OUTPUT_DIR / "run73_input_validation_summary.json", validation)
    if not validation["verdict"].startswith("PASS"):
        raise SystemExit(validation["errors"])

    native.to_csv(OUTPUT_DIR / "combined520_scan_order_features.csv", index=False)
    plus.to_csv(OUTPUT_DIR / "combined520_plus_N32_scan_order_features.csv", index=False)
    target_rows = [
        {"target": "target_reward_combined520_u2_primary", "definition": "0.65 U2 + 0.20 PEEQ + 0.10 SurfaceT + 0.05 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_constrained_u2_reward_balanced", "definition": "0.50 U2 + 0.25 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_strict_penalty_guard", "definition": "0.40 U2 + 0.30 PEEQ + 0.20 SurfaceT + 0.10 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_penalty_repair", "definition": "0.30 U2 + 0.30 PEEQ + 0.25 SurfaceT + 0.15 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_N12_final_diagnostic", "definition": "N12 final diagnostic target: 0.55 U2 + 0.20 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_N16_final_diagnostic", "definition": "N16 final diagnostic target: 0.55 U2 + 0.20 PEEQ + 0.15 SurfaceT + 0.10 Mises", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_smallN_final_diagnostic", "definition": "N12/N16-preferential final small-N diagnostic target using within-N ranks; N24/N40 are low-count anchors", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_variable_N_bounded", "definition": "Variable-N bounded target with N12/N16 final diagnostic emphasis and N24/N40 frozen-anchor treatment", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_N24_anchor", "definition": "N24 frozen-anchor diagnostic target; preserve high-performing region without broad N24 exploitation", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_N40_anchor", "definition": "N40 frozen-anchor diagnostic target; preserve U2/reward region without broad N40 exploitation", "dataset": "native_combined520"},
        {"target": "target_reward_combined520_plus_N32_mapped_u2_primary", "definition": "mapped U2-primary target with N32 semantic warnings preserved", "dataset": "combined520_plus_N32"},
        {"target": "target_reward_combined520_plus_N32_strict_u2_surfaceT", "definition": "strict U2 + SurfaceT target for safer N32 metric semantics", "dataset": "combined520_plus_N32"},
    ]
    pd.DataFrame(target_rows).to_csv(OUTPUT_DIR / "run73_target_reward_definition_audit.csv", index=False)
    write_json(OUTPUT_DIR / "run73_target_reward_definition_audit.json", {"headline": "Run73 defines a final N12/N16 diagnostic as the primary candidate-generation objective, keeps N24/N40 as low-count frozen anchors, and preserves N32 metric-semantic warnings for diagnostic plus_N32 regimes.", "targets": target_rows})

    detailed, best_cfg, surrogate_summary, best_model, best_features = run_surrogate_validation(native, plus)
    detailed.to_csv(OUTPUT_DIR / "run73_surrogate_validation_results_detailed.csv", index=False)
    best_cfg.to_csv(OUTPUT_DIR / "run73_best_surrogate_configurations.csv", index=False)
    write_json(OUTPUT_DIR / "run73_surrogate_validation_summary.json", surrogate_summary)

    gnn_results, gnn_summary = run_gnn_validation(native, plus)
    gnn_results.to_csv(OUTPUT_DIR / "run73_gnn_reward_validation_results.csv", index=False)
    write_json(OUTPUT_DIR / "run73_gnn_reward_validation_summary.json", gnn_summary)

    pointer_log, pointer_summary = pointer_policy(native, plus)
    pointer_log.to_csv(OUTPUT_DIR / "run73_graph_pointer_policy_training_log.csv", index=False)
    write_json(OUTPUT_DIR / "run73_graph_pointer_policy_validation_summary.json", pointer_summary)

    pool = generate_candidates(native, best_model, best_features)
    pool.to_csv(OUTPUT_DIR / "run73_candidate_pool_scored.csv", index=False)

    option_a = select_batch(pool, OPTION_A_COUNTS, "final_smallN_diagnostic_batch32")
    option_b = select_batch(pool, OPTION_B_COUNTS, "final_smallN_diagnostic_batch24")
    option_a.to_csv(OUTPUT_DIR / "run73_final_smallN_diagnostic_batch32_candidate_orders.csv", index=False)
    option_b.to_csv(OUTPUT_DIR / "run73_final_smallN_diagnostic_batch24_candidate_orders.csv", index=False)
    option_c_metadata = {
        "batch_option": "stop_and_freeze_evidence_package_only",
        "candidate_batch_created": False,
        "purpose": "Stop validation loops and prepare the Stage 3 final evidence package if the user decides combined520 evidence is sufficient.",
        "counts": {},
        "notes": "Option C is metadata only; no candidate orders are selected in Run73 for this option.",
    }
    write_json(OUTPUT_DIR / "run73_optionC_stop_and_freeze_evidence_package_only.json", option_c_metadata)

    comparison, comparison_summary = compare_batches({"option_A_final_smallN_diagnostic_batch32": option_a, "option_B_final_smallN_diagnostic_batch24": option_b}, native)
    comparison.to_csv(OUTPUT_DIR / "run73_batch_options_comparison_to_previous.csv", index=False)
    write_json(OUTPUT_DIR / "run73_batch_options_comparison_summary.json", comparison_summary)

    write_claim_boundary()
    evidence_update = write_evidence_update(native, validation, pred_summary, maturity)
    option_counts = {"option_A_final_smallN_diagnostic_batch32": n_counts(option_a), "option_B_final_smallN_diagnostic_batch24": n_counts(option_b), "option_C_stop_and_freeze_evidence_package_only": {}}
    summary = {
        "target_audit_headline": "Run73 target definitions prioritize a final N12/N16 diagnostic, keep N24/N40 as low-count anchors, and preserve N32 metric-semantic warnings for plus_N32 diagnostics.",
    ...
    output_files = [
        OUTPUT_DIR / "run73_input_validation_summary.json",
        OUTPUT_DIR / "combined520_scan_order_features.csv",
        OUTPUT_DIR / "combined520_plus_N32_scan_order_features.csv",
        OUTPUT_DIR / "run73_target_reward_definition_audit.csv",
        OUTPUT_DIR / "run73_target_reward_definition_audit.json",
        OUTPUT_DIR / "run73_surrogate_validation_results_detailed.csv",
        OUTPUT_DIR / "run73_best_surrogate_configurations.csv",
        OUTPUT_DIR / "run73_surrogate_validation_summary.json",
        OUTPUT_DIR / "run73_gnn_reward_validation_results.csv",
        OUTPUT_DIR / "run73_gnn_reward_validation_summary.json",
        OUTPUT_DIR / "run73_graph_pointer_policy_training_log.csv",
        OUTPUT_DIR / "run73_graph_pointer_policy_validation_summary.json",
        OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.csv",
        OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.json",
        OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.md",
        OUTPUT_DIR / "run73_candidate_pool_scored.csv",
        OUTPUT_DIR / "run73_final_smallN_diagnostic_batch32_candidate_orders.csv",
        OUTPUT_DIR / "run73_final_smallN_diagnostic_batch24_candidate_orders.csv",
        OUTPUT_DIR / "run73_optionC_stop_and_freeze_evidence_package_only.json",
        OUTPUT_DIR / "run73_batch_options_comparison_to_previous.csv",
        OUTPUT_DIR / "run73_batch_options_comparison_summary.json",
        CLAIM_BOUNDARY_MD,
        CLAIM_BOUNDARY_JSON,
        REPORT_PATH,
        MANIFEST_PATH,
    ...
    manifest = {
        ...
        "input_files": [str(COMBINED520_READY), str(COMBINED520_PLUS_N32_READY), str(RUN71_ENRICHED), str(RUN71_PRED_SUMMARY), str(RUN72_MATURITY), str(RUN72_REPORT), str(RUN72_MANIFEST)],
        "output_files": [str(p) for p in output_files],
        "native_combined520_rows": int(len(native)),
        "combined520_plus_N32_rows": int(len(plus)),
        "best_surrogate_summary": surrogate_summary,
        "best_N12_final_diagnostic_model_summary": surrogate_summary.get("best_N12_final_diagnostic"),
        "best_N16_final_diagnostic_model_summary": surrogate_summary.get("best_N16_final_diagnostic"),
        "best_smallN_final_diagnostic_model_summary": surrogate_summary.get("best_smallN_final_diagnostic"),
        "best_variable_N_bounded_model_summary": surrogate_summary.get("best_variable_N_bounded"),
        ...
        "evidence_update_paths": {
            "csv": str(OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.csv"),
            "json": str(OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.json"),
            "md": str(OUTPUT_DIR / "stage3_evidence_freeze_readiness_after_run72.md"),
        },
        "candidate_pool_count": int(len(pool)),
        "candidate_pool_counts": n_counts(pool),
        "recommended_option": "option_A_final_smallN_diagnostic_batch32",
        "batch_option_paths": {
            "option_A_final_smallN_diagnostic_batch32": str(OUTPUT_DIR / "run73_final_smallN_diagnostic_batch32_candidate_orders.csv"),
            "option_B_final_smallN_diagnostic_batch24": str(OUTPUT_DIR / "run73_final_smallN_diagnostic_batch24_candidate_orders.csv"),
            "option_C_stop_and_freeze_evidence_package_only": str(OUTPUT_DIR / "run73_optionC_stop_and_freeze_evidence_package_only.json"),
        },
