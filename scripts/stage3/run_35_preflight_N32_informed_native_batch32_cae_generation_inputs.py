"""Preflight Stage 3 run35 N32_informed_native_batch32 CAE/INP generation inputs."""
...
RUN30_DIR = PROJECT_ROOT / "outputs" / "stage3_run_34_run33_N32_informed_native_batch32_handoff_package"
INPUT_CSV = RUN30_DIR / "stage3_run34_N32_informed_native_batch32_candidate_orders.csv"
...
SUMMARY_PATH = OUTPUT_DIR / "run35_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run35_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run35_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run35_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN35_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN35_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "N32_informed_native_batch32":
            errors.append("row {} is not N32_informed_native_batch32: {}".format(idx, row.get("batch_option")))
        ...
        normalized.append(
            {
                ...
                "novelty_distance_to_combined172_plus_N32": row.get("novelty_distance_to_combined172_plus_N32", ""),
                "nearest_existing_teacher_strategy": row.get("nearest_existing_teacher_strategy", ""),
                "N32_informed": row.get("N32_informed", ""),
    ...
    verdict = "PASS_RUN35_N32_INFORMED_NATIVE_BATCH32_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN35_N32_INFORMED_NATIVE_BATCH32_INPUT_INVALID"
    summary = {
        "verdict": verdict,
