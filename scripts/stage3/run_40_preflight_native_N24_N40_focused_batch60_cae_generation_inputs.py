...
RUN39_DIR = PROJECT_ROOT / "outputs" / "stage3_run_39_run38_native_N24_N40_focused_batch60_handoff_package"
INPUT_CSV = RUN39_DIR / "stage3_run39_native_N24_N40_focused_batch60_candidate_orders.csv"
SCAN_ORDER_DIR = RUN39_DIR / "scan_orders"
...
SUMMARY_PATH = OUTPUT_DIR / "run40_preflight_summary.json"
CASES_CSV = OUTPUT_DIR / "run40_preflight_cases.csv"
PLAN_CSV = OUTPUT_DIR / "run40_generation_plan.csv"
SAFETY_AUDIT_PATH = OUTPUT_DIR / "run40_future_cae_root_safety_audit.json"
...
def safety_audit() -> tuple[dict[str, object], list[str]]:
    ...
    audit = {
        ...
        "verdict": "PASS_RUN40_FUTURE_CAE_ROOT_SAFE" if not errors else "FAIL_RUN40_FUTURE_CAE_ROOT_NOT_SAFE",
    }
...
def main() -> int:
    ...
    if len(rows) != 60:
        errors.append("row count {} != 60".format(len(rows)))
    ...
    for idx, row in enumerate(rows, 1):
        ...
        if row.get("batch_option") != "native_N24_N40_focused_batch60":
            errors.append("row {} is not native_N24_N40_focused_batch60: {}".format(idx, row.get("batch_option")))
        ...
        normalized.append(
            {
                ...
                "original_run38_candidate_id": row.get("original_run38_candidate_id", ""),
                "original_run38_strategy_name": row.get("original_run38_strategy_name", ""),
                ...
                "overnight_batch60": row.get("overnight_batch60", ""),
    ...
    verdict = "PASS_RUN40_NATIVE_N24_N40_FOCUSED_BATCH60_INPUT_READY_FOR_CAE_GENERATION" if not errors else "FAIL_RUN40_NATIVE_N24_N40_FOCUSED_BATCH60_INPUT_INVALID"
    summary = {
        "verdict": verdict,
