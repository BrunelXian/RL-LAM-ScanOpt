...
RUN_ID = "stageR_v03_odb_teacher_metric_extraction"
BATCH_NAME = "stage3_ppo_v03_lex_primary_N24_N40_batch32_v01"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
STAGEQ_GENERATION_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40"
    / "stageQ_CAE_INP_handoff"
    / "manifest"
    / "stageQ_v03_CAE_generation_manifest.csv"
)
STAGEQ_CASE_MANIFEST_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40"
    / "stageQ_CAE_INP_handoff"
    / "manifest"
    / "stageQ_v03_case_manifest.csv"
)
SELECTED_BATCH_CSV = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40"
    / "candidate_generation_v03"
    / "selected_batch32"
    / "v03_ppo_lex_primary_N24_N40_candidate_batch32.csv"
)
STAGEQ_HANDOFF_MANIFEST_JSON = (
    PROJECT_ROOT
    / "outputs"
    / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40"
    / "stageQ_CAE_INP_handoff"
    / "stageQ_v03_CAE_INP_handoff_manifest.json"
)
STAGEQ_COMMAND_FILE = (
    PROJECT_ROOT
    / "outputs"
    ...
    / "stageQ_CAE_INP_handoff"
    / "commands"
    / "stageQ_v03_batch32_abqjobpilot_commands_READY_TO_RUN.txt"
)
STAGEQ_REPORT_MD = (
    PROJECT_ROOT
    / "docs"
    / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40"
    / "PPO_V03_STAGEQ_CAE_INP_HANDOFF_REPORT.md"
)

OUTPUT_DIR = PROJECT_ROOT / "outputs" / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40" / "stageR_ODB_teacher_metric_extraction"
REPORT_MD = PROJECT_ROOT / "docs" / "stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40" / "PPO_V03_STAGER_ODB_TEACHER_METRIC_EXTRACTION_REPORT.md"
MANIFEST_PATH = OUTPUT_DIR / "stageR_v03_odb_teacher_metric_extraction_manifest.json"

AUDIT_CSV = OUTPUT_DIR / "stageR_v03_solver_completion_audit.csv"
AUDIT_JSON = OUTPUT_DIR / "stageR_v03_solver_completion_audit_summary.json"
AUDIT_MD = OUTPUT_DIR / "stageR_v03_solver_completion_audit.md"
EXTRACTION_SUMMARY_CSV = OUTPUT_DIR / "stageR_v03_odb_extraction_summary.csv"
EXTRACTION_SUMMARY_JSON = OUTPUT_DIR / "stageR_v03_odb_extraction_summary.json"
TEACHER_METRICS_CSV = OUTPUT_DIR / "stageR_v03_teacher_metrics.csv"
TEACHER_METRICS_JSON = OUTPUT_DIR / "stageR_v03_teacher_metrics.json"
SUMMARY_JSON = OUTPUT_DIR / "stageR_v03_extraction_summary.json"
...
STAGER_NOTES = {
    ...
    "includes_N40": True,
    "generated_CAE_INP_JNL_only_in_stageQ": True,
    "final_step_required_for_metrics": "step_final_cooling",
    "targeted_batch": "PPO v03 lex-primary N24/N40 batch32",
    "partial_training_caveat": "PPO v03 training was partial: N24 seed 20260627 reached 100000 timesteps from an interrupted checkpoint; N40 seed 20260627 reached 61440 timesteps. Training verdict: WARNING_V03_PPO_TRAINING_PARTIAL_REVIEW.",
}
...
def load_manifest_rows() -> list[dict[str, str]]:
    with STAGEQ_GENERATION_MANIFEST_CSV.open("r", encoding="utf-8-sig", newline="") as handle:
    ...
    if len(rows) != TOTAL_EXPECTED:
        raise RuntimeError(f"Expected {TOTAL_EXPECTED} Stage Q v03 batch32 manifest rows, got {len(rows)}")
    if any(row.get("batch_name") != BATCH_NAME for row in rows):
        raise RuntimeError("Manifest contains rows outside Stage Q v03 batch32 batch name")
    ...
    for row in rows:
        ...
        merged["selection_tag"] = selected.get("selected_by", "")
        merged["batch_index"] = ""
        ...
        merged["duplicate_role"] = ""
        merged["candidate_source"] = selected.get("candidate_source", "")
        merged["ppo_v03_checkpoint"] = selected.get("ppo_v03_checkpoint", "")
        merged["ppo_seed"] = selected.get("ppo_seed", "")
        merged["selected_by"] = selected.get("selected_by", "")
        merged["predicted_lex_primary_score"] = selected.get("predicted_lex_primary_score", "")
        merged["predicted_u2_guarded_score"] = selected.get("predicted_u2_guarded_score", "")
        merged["final_v03_score"] = selected.get("final_v03_score", "")
        merged["partial_training_caveat"] = selected.get("partial_training_caveat", STAGER_NOTES["partial_training_caveat"])
        normalized_rows.append(merged)
    if any(row["handoff_strategy_name"] not in selected_by_strategy for row in normalized_rows):
        raise RuntimeError("Stage Q manifest contains strategies missing from selected v03 batch32 CSV")
...
def audit_case(row: dict[str, str]) -> dict[str, object]:
    ...
    return {
        ...
        "ppo_v03_checkpoint": row.get("ppo_v03_checkpoint", ""),
        "ppo_seed": row.get("ppo_seed", ""),
        "selected_by": row.get("selected_by", ""),
        "predicted_lex_primary_score": row.get("predicted_lex_primary_score", ""),
        "predicted_u2_guarded_score": row.get("predicted_u2_guarded_score", ""),
        "final_v03_score": row.get("final_v03_score", ""),
        "partial_training_caveat": row.get("partial_training_caveat", ""),
...
def extract_case(audit_row: dict[str, object]) -> dict[str, object]:
    base = {
        ...
        "ppo_v03_checkpoint": audit_row["ppo_v03_checkpoint"],
        "ppo_seed": audit_row["ppo_seed"],
        "selected_by": audit_row["selected_by"],
        "predicted_lex_primary_score": audit_row["predicted_lex_primary_score"],
        "predicted_u2_guarded_score": audit_row["predicted_u2_guarded_score"],
        "final_v03_score": audit_row["final_v03_score"],
        "partial_training_caveat": audit_row["partial_training_caveat"],
...
def build_audit_summary(audit_rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    if failed or not expected_ok:
        verdict = "FAIL_STAGER_V03_SOLVER_COMPLETION_INCOMPLETE"
    elif warnings:
        verdict = "WARNING_STAGER_V03_SOLVER_COMPLETION_WITH_NONFATAL_WARNINGS"
    else:
        verdict = "PASS_STAGER_V03_SOLVER_COMPLETION_32_OF_32"
    return {
        ...
        "stageR_notes": STAGER_NOTES,
        "stageQ_report": str(STAGEQ_REPORT_MD),
        "stageQ_command_file": str(STAGEQ_COMMAND_FILE),
        "by_N": by_n,
    }
...
def build_extraction_summary(rows: list[dict[str, object]]) -> dict[str, object]:
    ...
    return {
        "timestamp": datetime.now(timezone.utc).isoformat(),
        "verdict": "PASS_STAGER_V03_ODB_TEACHER_METRIC_EXTRACTION_32_OF_32" if not failed and len(rows) == TOTAL_EXPECTED else "FAIL_STAGER_V03_ODB_TEACHER_METRIC_EXTRACTION",
        "total_expected": TOTAL_EXPECTED,
...
def write_report(audit_summary: dict[str, object], extraction_summary: dict[str, object] | None, audit_rows: list[dict[str, object]], extraction_rows: list[dict[str, object]]) -> None:
    ...
    lines.extend(["", "## Selection Tags", ""])
    for tag in sorted({str(row.get("selected_by", "")) for row in audit_rows if row.get("selected_by")}):
        tag_count = sum(1 for row in audit_rows if row.get("selected_by") == tag)
        lines.append(f"- `{tag}`: `{tag_count}` cases")
    incomplete = [row for row in audit_rows if row["completion_status"] == "FAIL_INCOMPLETE_OR_ABORTED"]
                ...
                extraction_rows.append(
                    {
                        ...
                        "partial_training_caveat": row["partial_training_caveat"],
                        "job_name": row["job_name"],
        ...
        json.dumps(
            {
                ...
                "inputs": {
                    "stageQ_generation_manifest_csv": str(STAGEQ_GENERATION_MANIFEST_CSV),
                    "stageQ_case_manifest_csv": str(STAGEQ_CASE_MANIFEST_CSV),
                    "selected_batch_csv": str(SELECTED_BATCH_CSV),
                    "stageQ_handoff_manifest_json": str(STAGEQ_HANDOFF_MANIFEST_JSON),
                    "stageQ_command_file": str(STAGEQ_COMMAND_FILE),
                    "stageQ_report_md": str(STAGEQ_REPORT_MD),
                },
                "stageR_notes": STAGER_NOTES,
                "outputs_written": [
                    str(AUDIT_CSV),
