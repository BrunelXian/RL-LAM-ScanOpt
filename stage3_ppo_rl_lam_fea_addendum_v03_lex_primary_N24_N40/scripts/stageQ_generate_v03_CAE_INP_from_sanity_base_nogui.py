"""Generate Stage Q PPO v03 lex-primary N24/N40 CAE/INP files.

Run with:
abaqus cae noGUI="E:\Projects\RL-LAM-ScanOpt\stage3_ppo_rl_lam_fea_addendum_v03_lex_primary_N24_N40\scripts\stageQ_generate_v03_CAE_INP_from_sanity_base_nogui.py"
...
RUN_ID = "stageQ_v03_CAE_INP_handoff"
...
GENERATION_SUMMARY = OUTPUT_ROOT / "manifest" / "stageQ_v03_generation_summary.json"
GENERATION_MANIFEST_CSV = OUTPUT_ROOT / "manifest" / "stageQ_v03_CAE_generation_manifest.csv"
GENERATION_MANIFEST_JSON = OUTPUT_ROOT / "manifest" / "stageQ_v03_CAE_generation_manifest.json"
FAILURE_LOG = OUTPUT_ROOT / "reports" / "stageQ_v03_generator_failure.json"
...
def validate_paths(rows: list[dict[str, object]]) -> None:
    for row in rows:
        ...
        if case_dir.parent != CASE_ROOT:
            raise StageLGenerationError(f"case_dir not under Stage Q root: {case_dir}")
...
def ensure_output_fields(model) -> dict[str, object]:  # noqa: ANN001
    ...
    if not status["updated_requests"]:
        ...
        if creator is None:
            ...
            try:
                creator(name="F-Output-PPO-v03-TeacherMetrics", createStepName="step_scan_00", variables=OUTPUT_VARIABLES)
...
def write_case_files(row: dict[str, object]) -> None:
    ...
    Path(str(row["expected_jnl"])).write_text(
        f"Stage Q PPO v03 generation placeholder for {row['strategy_name']}.\nCAE/INP generated only; no solver/datacheck/enqueue/ODB.\n",
        encoding="utf-8",
    )
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "Stage Q PPO v03 CAE/INP generation only; no teacher metrics yet.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_STAGEQ_V03_GENERATION_COMPLETE",
...
def main() -> int:
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_STAGEQ_V03_GENERATION_COMPLETE")
        ...
        print("FAIL_STAGEQ_V03_GENERATION")
        print(str(exc))
