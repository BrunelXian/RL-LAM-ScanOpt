...
BASE_AUDIT_SUMMARY = OUTPUT_DIR / "run70_base_mesh_audit_summary.json"
CASE_ROOT = PROJECT_ROOT / "cae_model" / BATCH_NAME
FAILURE_LOG = OUTPUT_DIR / "run70_generator_failure.json"
GENERATION_SUMMARY = OUTPUT_DIR / "run70_generation_summary.json"
MANIFEST_CSV = OUTPUT_DIR / "stage3_run70_smallN_recovery_focused_batch40_cae_generation_manifest.csv"
MANIFEST_JSON = OUTPUT_DIR / "stage3_run70_smallN_recovery_focused_batch40_cae_generation_manifest.json"
...
def load_plan() -> list[dict[str, object]]:
    if not PLAN_CSV.exists():
        raise run40GenerationError("missing run70 generation plan: {}".format(PLAN_CSV))
    ...
    with PLAN_CSV.open(newline="", encoding="utf-8-sig") as f:
        for row in csv.DictReader(f):
            ...
            if row.get("selected_batch") != "run68_smallN_recovery_focused_batch40":
                raise run40GenerationError("non-run68_smallN_recovery_focused_batch40 row selected: {}".format(row.get("handoff_strategy_name")))
...
def write_case_files(model, row: dict[str, object]) -> None:  # noqa: ANN001
    ...
    metadata_path = case_dir / "run70_case_metadata.json"
    metadata_keys = [
...
def write_generation_manifest(rows: list[dict[str, object]]) -> None:
    ...
    for row in rows:
        manifest_rows.append({
            ...
            "notes": "run70 constrained N24/N40 batch32 CAE/INP generation only; not teacher-validated.",
        })
...
def generate_all(rows: list[dict[str, object]]) -> None:
    ...
    summary = {
        "verdict": "PASS_RUN70_SMALLN_RECOVERY_FOCUSED_BATCH40_GENERATION_COMPLETE",
...
def main() -> int:
    ...
    try:
        ...
        validate_paths(rows)
        generate_all(rows)
        print("PASS_RUN70_SMALLN_RECOVERY_FOCUSED_BATCH40_GENERATION_COMPLETE")
        ...
        print("FAIL_RUN70_SMALLN_RECOVERY_FOCUSED_BATCH40_GENERATION")
        print(str(exc))
